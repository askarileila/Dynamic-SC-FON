		...
		for (itr=pCon->m_SC->SCPath.begin(); itr!=pCon->m_SC->SCPath.end(); itr++){
			for(int i=0;i<(*itr)->abs_m_nW;i++){
				if((*itr)->abs_m_pChannel[i].m_nCapacity>=pCon->m_eBandwidth){
					...
					if((*itr)->m_linkCap<0 || (*itr)->m_linkCap>AB_LINK_CAPACITY){
						cout<<"\n Capacity of the link is lower than link capacity";
						cin.get();
					}
			...
			for(int i=0;i<(*itr)->abs_m_nW;i++){
				...
				//	break;				
					if(i==pCon->m_Con_Wl[j]){
						...
						(*itr)->m_linkCap_Array[i]+= pCon->m_eBandwidth;
						if((*itr)->m_linkCap>AB_LINK_CAPACITY){
							cout<<"\n Capacity of the link is higher than link capacity";
							cin.get();
						}
