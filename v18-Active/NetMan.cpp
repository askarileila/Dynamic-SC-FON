...
NetMan::NetMan(): m_dTxThreshold(1.0), m_nNumberOfPaths(1), m_hCostSlack(0),
	...
	NumNAT(0),NumFW(0),NumTM(0),NumWOC(0),NumIDPS(0),NumVOC(0),m_NumPVNF(0),VNFCounter(0),m_NumActVNF(0),SameNodeVNFs(0),DiffNodeVNFs(0)
{	
	...
	*p_block += (double)(block/nRunConnections);	//-B: since stats are computed every N_RUN_CONNECTIONS (m_nRunConnections is set in Simulator class)
	...
	switch (m_eStatActive){
		...
		{												//ALMENO 300000 connessioni..
			...
			if((p_block->isconfsatisfied(stop_perc , conf) == true /*&& (connection >= 300000)*/) || (p_block->mean() == 0.0 && (connection) == 10000000))   //tolto per evitare che una volta raggiunta la confidenza si fermi la simulazione
			{
						...
						const char* UnAv) // M//ANDREA
{
	...
	//LA: i should check that no nfv is enabled here: not needed as it messed up the destructor of OXCnode
	/*OXCNode *oxcnode; 
	if (oxcnode->m_NumNFV>0)
		oxcnode->m_NumNFV=0;
	*/
...
bool NS_OCH::NetMan::checkAvailabilitySimplexLink(UINT srcId, UINT dstId, int w, double nBW)
{
	SimplexLink*sLink;
	sLink = m_hGraph.lookUpSimplexLink(srcId, Vertex::VT_Channel_Out, w, dstId, Vertex::VT_Channel_In, w);
...
void NetMan::releaseLinkBandwidth(Lightpath*pLightpath, double BWToRealease)
{
	list<UniFiber*>::const_iterator itrFiber;
#ifdef DEBUGB
...
void NS_OCH::NetMan::invalidateSimplexLinkDueToCapOrStatus(double bwd)
{
	list<AbstractLink*>::const_iterator itr, itrL;
	SimplexLink*pLink;
	UniFiber*pUniFiber;
		...
		if ((*itr)->getValidity())
		{
				...
				else
				{
					for(int i=0;i<(*itr)->abs_m_nW;i++){
						if((*itr)->abs_m_pChannel[i].m_nCapacity>=bwd){
							NoCap=false;
							break;
						}
					}
...
void NetMan::releaseVirtualLinkBandwidth(double nBWToRelease, Lightpath*pLightpath)
{
...
float NetMan::computeLatency(list<AbstractLink*>&path)
{
	...
	/* not needed for OTN version just propagation delay and grooming
	...
	while (itr != path.end())
	{
		...
		if(counter==0)
			latency+=(float) ELSWITCHLATENCY+(float) ELSWITCHLATENCY;
		else
...
void NetMan::BuildNFVNodesList()
{
	...
	for (itr = m_hWDMNet.m_hNodeList.begin(); itr != m_hWDMNet.m_hNodeList.end(); itr++)
	{
		...
		if(pNode->m_NFVFlag==1)
			this->NFVCapnodes.push_back(pNode);
...
cout << "->DVNF_ProvisionVNF" << endl;
	...
	bool NodeExist=false,provisioned=false,linkRedun=false;	
...
cout << "\nProvisioning VNF of type :" << VN->VNFName<< endl;
	...
	if(checkVar!=0){
						...
						NFVnodesCap=DVNF_CreateVNFNodes(VN->VNFName);
							...
							//LA:to avoid using the nfv-nodes already on SCpath
						for(itr=pCon->m_SC->SCPath.begin();itr!=pCon->m_SC->SCPath.end();itr++){
							AbstractLink *pLink = (AbstractLink*)(*itr);
							OXCNode * VDstpath=(OXCNode*)  pLink->getDst();
							OXCNode * VSrcpath= (OXCNode*) pLink->getSrc();
							if(VSrcpath->m_NFVnode && find(NodeOnPath.begin(),NodeOnPath.end(),VSrcpath)==NodeOnPath.end())
								NodeOnPath.push_back(VSrcpath);
							if(find(NodeOnPath.begin(),NodeOnPath.end(),VDstpath)==NodeOnPath.end() && VDstpath->m_NFVnode)
								NodeOnPath.push_back(VDstpath);
						}
						...
						for(int j=0;j<NFVnodesCap.size();j++){
							...
							if(((NFVnodesCap[j]->CPURes))>=(VN->CpuUsage)*(pCon->m_SC->NumofUsers)){
								if(find(pCon->m_SC->SCnodes.begin(),pCon->m_SC->SCnodes.end(),NFVnodesCap[j])==pCon->m_SC->SCnodes.end())
									if(find(NodeOnPath.begin(),NodeOnPath.end(),NFVnodesCap[j])==NodeOnPath.end()){
										
										//LA:check if there is redundant link for reaching this node from con src
										linkRedun=false;
										pVSrc = m_hWDMNet.m_hNodeList.find(SrcID);
										pVDst = m_hWDMNet.m_hNodeList.find(NFVnodesCap[j]->getId());
										NFVnodesCap[j]->hFromSrcPath.clear();
										NFVnodesCap[j]->NFVnDstReachCost=m_hWDMNet.Dijkstra(NFVnodesCap[j]->hFromSrcPath,pVSrc,pVDst,AbstractGraph::LCF_ByOriginalLinkCost);
										for(itrSC=pCon->m_SC->SCPath.begin();itrSC!=pCon->m_SC->SCPath.end();itrSC++){
											for(itr=NFVnodesCap[j]->hFromSrcPath.begin();itr!=NFVnodesCap[j]->hFromSrcPath.end();itr++){
												AbstractLink *pLink =(AbstractLink*) (*itr);
												AbstractNode * VDstpath=  (AbstractNode*)pLink->getDst();
												AbstractNode * VSrcpath=(AbstractNode*) pLink->getSrc();
												AbstractLink *pLinkSC = (*itrSC);
												if(pLink->getSrc()==pLinkSC->getSrc() && pLink->getDst()==pLinkSC->getDst()&&VDstpath!=VSrcpath)
													linkRedun=true;
									...
									if(NFVnodesCap[j]->getId()==pCon->m_nDst && !linkRedun)
										NFVnodes.push_back(NFVnodesCap[j]);
								}
						}
								NFVnodesCap[j]->hFromSrcPath.clear();
						...
						if(NFVnodes.size()==0){
							...
							if(status){
								#ifdef DEBUGLA
									cout<<"\nSuccessfully created an instance of VNF:"<<VN->VNFName<<" on node :"<<pCon->m_SC->SCnodes.back()<<endl;
								#endif
						...
						//LA: there is more than one already activated VNF instance  in the network with enough capacity
							...
							for (itr=NFVnodes[j]->htoDstPath.begin(); itr!=NFVnodes[j]->htoDstPath.end(); itr++){
								AbstractLink *pLink = (AbstractLink*)(*itr);
								AbstractNode * VDstpath=(AbstractNode*)pLink->m_pDst;
							...
							if(NodeExist)
								continue;
								//LA:to prevent insertion of redundant links in scpath
						if(pCon->m_SC->SCPath.size()!=0){
							...
							for (itr=NFVnodes[j]->htoDstPath.begin(); itr!=NFVnodes[j]->htoDstPath.end(); itr++){
								...
								AbstractNode * VSrcpath= (AbstractNode *) pLink->getSrc();
								for(itrSC=pCon->m_SC->SCPath.begin(); itrSC!=pCon->m_SC->SCPath.end(); itrSC++){
									AbstractLink *pLinkSC = (AbstractLink*)(*itrSC);
									if(pLink->getSrc()==pLinkSC->getSrc() && pLink->getDst()==pLinkSC->getDst()&&VDstpath!=VSrcpath){
										linkRedun=true;
										break;
									}
								...
								for (itr=NFVnodes[j]->htoDstPath.begin(); itr!=NFVnodes[j]->htoDstPath.end(); itr++){
									AbstractLink *pLink = (AbstractLink*)(*itr);
									AbstractNode * VDstpath=  pLink->getDst();
									AbstractNode * VSrcpath=  pLink->getSrc();
								for(itrSC=NFVnodes[j]->hFromSrcPath.begin(); itrSC!=NFVnodes[j]->hFromSrcPath.end(); itrSC++){
									AbstractLink *pLinkSC = (AbstractLink*)(*itrSC);
									if(pLink->getSrc()==pLinkSC->getSrc() && pLink->getDst()==pLinkSC->getDst()&&VDstpath!=VSrcpath){
										linkRedun=true;
										break;
						...
						DstRed=false;
					if(candidatenode!=NULL && candidatenode->getId()!=pCon->m_nDst){
						//LA:if in htodst path there is redundant nodes among srcs of links in scpath choose dst
						if(pCon->m_SC->SCPath.size()!=0){
							for(itr=candidatenode->htoDstPath.begin();itr!=candidatenode->htoDstPath.end();itr++){
								AbstractLink *plink=(AbstractLink*)(*itr);
								for(itrSC=pCon->m_SC->SCPath.begin();itrSC!=pCon->m_SC->SCPath.end();itrSC++){									
									AbstractLink *plinkPath=(AbstractLink*)(*itrSC);
									if(plink->getDst()==plinkPath->getSrc()&& plink->getSrc()==plinkPath->getDst())
									{
										//LA:choose Dst as NFV-node
										candidatenode= (OXCNode*)m_hWDMNet.m_hNodeList.find(pCon->m_nDst);
										pVSrc=m_hWDMNet.m_hNodeList.find(SrcID);
										pVDst=m_hWDMNet.m_hNodeList.find(candidatenode->getId());
										candidatenode->hFromSrcPath.clear();
										candidatenode->htoDstPath.clear();
										candidatenode->NFVnSrcReachCost=m_hWDMNet.Dijkstra(candidatenode->hFromSrcPath,pVSrc,pVDst,AbstractGraph::LCF_ByOriginalLinkCost);
										DstRed=true;
										break;
									}
									//LA:same link in scpath and htodst path
									else 
										if(plink->getDst()==plinkPath->getDst()&& plink->getSrc()==plinkPath->getSrc())
									{
										//LA:choose Dst as NFV-node
										candidatenode= (OXCNode*)m_hWDMNet.m_hNodeList.find(pCon->m_nDst);
										pVSrc=m_hWDMNet.m_hNodeList.find(SrcID);
						...
						//LA:if it is the first node to insert in scnodes check also hfromsrc path with htodstpath
						//else{
						//LA:check if the link causing trombone effect exists in hfromsrc and htodst of candidatenode
							for(itrSC=candidatenode->hFromSrcPath.begin();itrSC!=candidatenode->hFromSrcPath.end();itrSC++){
								AbstractLink *plinkPath=(AbstractLink*)(*itrSC);
								for(itr=candidatenode->htoDstPath.begin();itr!=candidatenode->htoDstPath.end();itr++){
									AbstractLink *plink=(AbstractLink*)(*itr);									
									if(plink->getDst()==plinkPath->getSrc()&& plink->getSrc()==plinkPath->getDst())
									{
										...
										//LA:if the link with the same source and dst exists in both hfromsrc and htodst
										if( plink->getDst()==plinkPath->getDst() && plink->getSrc()==plinkPath->getSrc()) 
										{
											//LA:choose Dst as NFV-node
											candidatenode= (OXCNode*)m_hWDMNet.m_hNodeList.find(pCon->m_nDst);
							...
							if(!DstRed){
							//LA:Check hfromsrc and scpath for trombone
								for(itrSC=candidatenode->hFromSrcPath.begin();itrSC!=candidatenode->hFromSrcPath.end();itrSC++){
									AbstractLink *plinkPath=(AbstractLink*)(*itrSC);
									for(itr=pCon->m_SC->SCPath.begin();itr!=pCon->m_SC->SCPath.end();itr++){
										AbstractLink *plink=(AbstractLink*)(*itr);									
				...
				//LA:if candidatenode is not dst yet 
						if(candidatenode->getId()!=pCon->m_nDst){
							//LA:if dst is already on the way to get to candidatenode, choose dst as candidatenode
							for(itrnew=candidatenode->hFromSrcPath.begin();itrnew!=candidatenode->hFromSrcPath.end();itrnew++){
								AbstractLink *plink=(AbstractLink*)(*itrnew);
								AbstractNode * LinkDst=  plink->getDst();
								if (LinkDst->getId()==pCon->m_nDst&& ((OXCNode*)m_hWDMNet.m_hNodeList.find(pCon->m_nDst))->CPURes>=(VN->CpuUsage)*(pCon->m_SC->NumofUsers)){
							//LA>choosing connection dst as candidatenode and provision VNF on that 
								//candidatenode=(OXCNode*)m_hWDMNet.m_hNodeList.find(pCon->m_nDst);
								candidatenode= (OXCNode*)m_hWDMNet.m_hNodeList.find(pCon->m_nDst);
										pVSrc=m_hWDMNet.m_hNodeList.find(SrcID);
										pVDst=m_hWDMNet.m_hNodeList.find(candidatenode->getId());
...
bool NetMan:: DVNF_ActivateNewInst(VNF* VN,Connection * pCon)
{
	...
	LINK_COST SP_Cost;	
	list<AbstractLink*>::const_iterator itr,itrSC, itrSP;
	list<AbstractLink*> ShortestPath;
	list<AbstractLink*>ShortestPathLA;
	vector <OXCNode*> ClosestNodes,TempNodes,SPnodes,NodeOnPath;
	UINT nodeID,SrcID;
	...
	if(SP_Cost==UNREACHABLE){
		...
		for (itrSP=ShortestPath.begin(); itrSP!=ShortestPath.end(); itrSP++){
			AbstractLink *pLink = (*itrSP);
			...
			if(VDst->m_NFVnode){
				...
				if((VDst->CPURes)>=(VN->CpuUsage)*(pCon->m_SC->NumofUsers)){		//if it has enough capacity add it to the list otherwise no
						...
						for(itrSC=pCon->m_SC->SCPath.begin();itrSC!=pCon->m_SC->SCPath.end();itrSC++){
							AbstractLink *pLinkSC = (AbstractLink*)(*itrSC);
							OXCNode * VDstpath= (OXCNode *) pLinkSC->getDst();
							OXCNode * VSrcpath= (OXCNode *) pLinkSC->getSrc();
							if(VSrcpath->m_NFVnode)
								NodeOnPath.push_back(VSrcpath);
							if(find(NodeOnPath.begin(),NodeOnPath.end(),VDstpath)==NodeOnPath.end() && VDstpath->m_NFVnode)
								NodeOnPath.push_back(VDstpath);
						}
					...
					if(find(pCon->m_SC->SCnodes.begin(),pCon->m_SC->SCnodes.end(),VDst)==pCon->m_SC->SCnodes.end() || VDst->getId()==pCon->m_nDst)
						if(	find(SPnodes.begin(),SPnodes.end(),VDst)==SPnodes.end() )	
							if(find(NodeOnPath.begin(),NodeOnPath.end(),VDst)==NodeOnPath.end()){
								linkRedun=false;
							//	while(!linkRedun){		
								//LA:check if there is redundant link for reaching this node from con src
										pVSrc = m_hWDMNet.m_hNodeList.find(SrcID);
										pVDst = m_hWDMNet.m_hNodeList.find(VDst->getId());
										VDst->hFromSrcPath.clear();
										VDst->NFVnDstReachCost=m_hWDMNet.Dijkstra(VDst->hFromSrcPath,pVSrc,pVDst,AbstractGraph::LCF_ByOriginalLinkCost);
										for(itrSC=pCon->m_SC->SCPath.begin();itrSC!=pCon->m_SC->SCPath.end();itrSC++){
											for(itr=VDst->hFromSrcPath.begin();itr!=VDst->hFromSrcPath.end();itr++){
												AbstractLink *pLink = (AbstractLink*)(*itr);
												AbstractNode * VDstpath= (AbstractNode *) pLink->getDst();
												AbstractNode * VSrcpath= (AbstractNode *) pLink->getSrc();
												AbstractLink *pLinkSC = (AbstractLink*)(*itrSC);
		...
		for(int k=0;k<SPnodes.size();k++)
		{
			...
			//if(SPnodes[k]->CPURes>=VN->CpuUsage){
				...
				//LA:calculate htoDSt
					...
					pVSrc = m_hWDMNet.m_hNodeList.find(SPnodes[k]->getId());
					pVDst = m_hWDMNet.m_hNodeList.find(pCon->m_nDst);
					SP_Cost= m_hWDMNet.Dijkstra(SPnodes[k]->htoDstPath,pVSrc,pVDst,AbstractGraph::LCF_ByOriginalLinkCost);

			
				if(pCon->m_SC->SCPath.size()!=0){
					linkRedun=false;
						//LA:check htoDStpath
					for (itr=SPnodes[k]->htoDstPath.begin(); itr!=SPnodes[k]->htoDstPath.end(); itr++){
						AbstractLink *pLink = (AbstractLink*)(*itr);
						AbstractNode * VDstpath= (AbstractNode *) pLink->getDst();
						AbstractNode * VSrcpath= (AbstractNode *) pLink->getSrc();
						for(itrSC=pCon->m_SC->SCPath.begin(); itrSC!=pCon->m_SC->SCPath.end(); itrSC++){
					...
					//LA:compare htoDst and hfromsrc
						for (itr=SPnodes[k]->htoDstPath.begin(); itr!=SPnodes[k]->htoDstPath.end(); itr++){
						AbstractLink *pLink = (AbstractLink*)(*itr);
						AbstractNode * VDstpath= (AbstractNode *) pLink->getDst();
						AbstractNode * VSrcpath= (AbstractNode *) pLink->getSrc();
						for(itrSC=SPnodes[k]->hFromSrcPath.begin(); itrSC!=SPnodes[k]->hFromSrcPath.begin(); itrSC++){
							AbstractLink *pLinkSC = (AbstractLink*)(*itrSC);
							if(pLink->getSrc()==pLinkSC->getSrc() && pLink->getDst()==pLinkSC->getDst() && VDstpath!=VSrcpath){
								linkRedun=true;
								//continue;
					...
					//enable VNF
					if(!linkRedun && SPnodes[k]->NFVnReachCost!=UNREACHABLE){
					DVNF_EnableVNF(SPnodes[k],VN,pCon->m_SC);
					SPnodes[k]->hFromSrcPath.clear();
					return true;
					}
				...
				//	if(SC->SCnodes.back()!=SPnodes[k]){	 //LA: the node chosen for provisioning of vnf is not chosen before in provisioning of previous VNF
					...
					SP_Cost= m_hWDMNet.Dijkstra(NewVNFCand->htoDstPath,pVSrc,pVDst,AbstractGraph::LCF_ByOriginalLinkCost);
						...
						//LA:to prevent insertion of redundant links in scpath
					if(pCon->m_SC->SCPath.size()!=0 && NewVNFCand->htoDstPath.size()!=0){
						linkRedun=false;
						...
						AbstractNode * VDstpath= (AbstractNode *) pLink->getDst();
						AbstractNode * VSrcpath= (AbstractNode *) pLink->getSrc();
						for(itrSC=NewVNFCand->hFromSrcPath.begin(); itrSC!=NewVNFCand->hFromSrcPath.begin(); itrSC++){
							AbstractLink *pLinkSC = (AbstractLink*)(*itrSC);
							if(pLink->getSrc()==pLinkSC->getSrc() && pLink->getDst()==pLinkSC->getDst() && VDstpath!=VSrcpath){
								linkRedun=true;
								//continue;
					...
					if(!linkRedun&& SP_Cost!=UNREACHABLE){				
			...
			//LA: if the list of nodes on the shortest path with enough capacity is empty
			//LA:commented as it needs to be checked in any case 
			//if(SPnodes.size()==0)
			//{
				for(int l=0;l<NFVCapnodes.size();l++){
				...
				if(pCon->m_SC->SCPath.size()!=0){
					...
					for (itr=NFVCapnodes[l]->htoDstPath.begin(); itr!=NFVCapnodes[l]->htoDstPath.end(); itr++){
						...
						AbstractNode * VSrcpath= (AbstractNode *) pLink->getSrc();
						for(itrSC=pCon->m_SC->SCPath.begin(); itrSC!=pCon->m_SC->SCPath.end(); itrSC++){
							AbstractLink *pLinkSC = (SimplexLink*)(*itrSC);
							if(pLink->getSrc()==pLinkSC->getSrc() && pLink->getDst()==pLinkSC->getDst()&& VSrcpath!=VDstpath){
								linkRedun=true;
								break;
							}
						...
						AbstractNode * VDstpath= (AbstractNode *) pLink->getDst();
						AbstractNode * VSrcpath= (AbstractNode*) pLink->getSrc();
						for(itrSC=NFVCapnodes[l]->hFromSrcPath.begin(); itrSC!=NFVCapnodes[l]->hFromSrcPath.begin(); itrSC++){
							AbstractLink *pLinkSC = (AbstractLink*)(*itrSC);
							if(pLink->getSrc()==pLinkSC->getSrc() && pLink->getDst()==pLinkSC->getDst() && VDstpath!=VSrcpath){
								linkRedun=true;
								//continue;
			...
			//	}
				...
				for(itrSC=pCon->m_SC->SCPath.begin();itrSC!=pCon->m_SC->SCPath.end();itrSC++){
					for(itr=NFVCapnodes[l]->hFromSrcPath.begin();itr!=NFVCapnodes[l]->hFromSrcPath.end();itr++){
						AbstractLink *pLink = (SimplexLink*)(*itr);
						AbstractNode * VDstpath= (AbstractNode *) pLink->getDst();
						AbstractNode * VSrcpath= (AbstractNode *) pLink->getSrc();
						AbstractLink *pLinkSC = (AbstractLink*)(*itrSC);
						if(pLink->getSrc()==pLinkSC->getSrc() && pLink->getDst()==pLinkSC->getDst()&&VDstpath!=VSrcpath)
			...
			/*
				...
				/*//LA:not needed as NFV-node is checked for sure before
				if(ClosestNodes.size()==0){
				...
				*/		
				...
				{	
					////LA:7-2- if it's last vnf to provision add the shortest path from that node to dst to SCpath 
					//if(this->VNFCounter==(pCon->m_SC->LengthOfSCs)-1) {
					//	pVSrc =m_hGraph.lookUpVertex(CapCandNode->getId(),Vertex::VT_Access_Out,-1);
					//	pVDst = m_hGraph.lookUpVertex(pCon->m_nDst,Vertex::VT_Access_In,-1);
					//	SP_Cost= m_hGraph.Dijkstra(CapCandNode->htoDstPath,pVSrc,pVDst,AbstractGraph::LCF_ByOriginalLinkCost);
	...
	for(int i=0;i<pCon->m_SC->SCnodes.size();i++){
		...
		VNF *VN= pCon->m_SC->VNFList[i];
			...
			if(pox->VNF[j]->NumSC==0){
				...
				for(int k=0;k<VNFNodes.size();k++){
					if(pox==VNFNodes[k]){
						VNFNodes.erase(VNFNodes.begin()+k);
						break;
					}
				}
...
outfile.open("SCPaths.txt", std::ios_base::app);
	...
	outfile<<"\n The connection sequence number is"<<SC->con->m_nSequenceNo<<endl;
	outfile<<"\nThe number of users for this SC is "<<SC->NumofUsers<<endl;
	outfile<<"The connection is: ";
	if(SC->con->m_eStatus==Connection::SETUP) 
		outfile<<"Provisioned";
	else{
		if(SC->con->m_eStatus==Connection::DROPPED)
			outfile<<"Blocked!";
			outfile<<"\nThe location of blocking is: "<<SC->BlckingLocation;
	}
	...
	outfile<<" ,Dst:"<<SC->con->m_nDst<<endl;	
		...
		if(SC->Grouped==true)
			outfile<<"Grouped"<<endl;
		else
			outfile<<"Not grouped"<<endl;
		
		/*for(int K=0;K<VNFNodes.size();K++)
		{	
			outfile<<"For node "<<VNFNodes[K]->getId()<<" CPU capacity is: "<<VNFNodes[K]->CPURes<<endl;
		}
		*/
	/*	if(SC->con->LogReprov)
			outfile<<"\n Connection is reprovisioned!"<<endl;*/
	outfile.close();
}
bool NetMan::DVNF_GroupingVNF(ServiceChain *SC, Connection * pCon)
{
	...
	UINT PrevDstID=-1;
	int index;
	bool firsttime=true, linkRedun=false;;
	int countercase3=0;
	...
	//LA:case 3: if link is located neither at the begining nor at the end
		...
		//LA:just if the node has capacity for both VNFs it can be chosen as candidate node 
			if(SC->SCnodes[maxdst]->CPURes>=(VNFsrc->CpuUsage)*(SC->NumofUsers) && SC->SCnodes[maxdst]->CPURes>=(VNFdst->CpuUsage)*(SC->NumofUsers))
				candnode=SC->SCnodes[maxdst];
			else
				if(SC->SCnodes[maxsrc]->CPURes>=(VNFsrc->CpuUsage)*(SC->NumofUsers) && SC->SCnodes[maxsrc]->CPURes>=(VNFdst->CpuUsage)*(SC->NumofUsers))
				candnodedst=SC->SCnodes[maxsrc];
	...
	cout<<"\nBuilding the path between Src of the connection and new node for the first VNF"<<endl;
			...
			for(itrSC=pCon->m_SC->SCPath.begin(); itrSC!=pCon->m_SC->SCPath.end(); itrSC++){
				...
				if(pLink->getSrc()==pLinkSC->getSrc() && pLink->getDst()==pLinkSC->getDst()&&VDstpath!=VSrcpath){
					...
					break;
						...
						}//LA: chech if there is redundant link 
		...
		//for (itr=ShortestPath.begin(); itr!=ShortestPath.end(); itr++){
		//	AbstractLink *pLink = (AbstractLink*)(*itr);
		//	Vertex *VSrc = (Vertex*)pLink->getSrc();
		//	Vertex *VDst = (Vertex*)pLink->getDst();
		...
		//		PrevSrcID=VSrc->m_pOXCNode->getId(); //LA: keep the last id of src to prevent inserting duplicate links
		//		PrevDstID=VDst->m_pOXCNode->getId();
		//		}
		//	//}
		//}
		SC->SCPath.insert(SC->SCPath.end(),ShortestPath.begin(),ShortestPath.end());
		index=i;
	...
	#endif
		pVSrc =m_hWDMNet.m_hNodeList.find(SC->SCnodes.back()->getId());
		pVDst =  m_hWDMNet.m_hNodeList.find(pCon->m_nDst);
		linkcost= m_hWDMNet.Dijkstra(SC->SCnodes.back()->htoDstPath,pVSrc,pVDst,AbstractGraph::LCF_ByOriginalLinkCost);
		//LA:if the path is not found 
		if(linkcost==UNREACHABLE){
		//pCon->m_bBlockedDueToLatency=true;
				if(!firsttime)
...
latency=computeLatency(pCon->m_SC->SCPath);	
pCon->m_ConLat=latency;
firsttime=false;
}//end of while

//LA:remove conn from all links in old scpath and add it to new scpath 
		list<AbstractLink*>::const_iterator itrConPath;
		list<Connection*>::const_iterator itrc;

		/*for (itrConPath=SCpath.begin(); itrConPath!=SCpath.end(); itrConPath++){
			for(itrc=(*itrConPath)->m_ConsOnLP.begin();itrc!=(*itrConPath)->m_ConsOnLP.end();itrc++){
				if((*itrc)->m_nSequenceNo==pCon->m_nSequenceNo){
					(*itrConPath)->m_ConsOnLP.erase(itrc);
					break;
				}
			}
		}

		for (itr=SC->SCPath.begin(); itr!=SC->SCPath.end(); itr++){
			(*itr)->m_ConsOnLP.push_back(pCon);		
		}*/

pCon->m_SC->Grouped=true;
return true;
}
...
vector <int> NetMan::DVNF_BuildLocalNodes(ServiceChain* SC){
	vector <int> NodesID;
	//LA:DVNF topology
	/*int SC1nodes[]={80,10,75,46};
	int SC2nodes[]={14,4,58,46};
	int SC3nodes []={34,25,56,46};*/
	
	
//int SC1nodes[]={6,8,5,2,1,12};
//	int SC2nodes[]={1,5,2,43};
//	int SC3nodes []={5,6,8,2,3,4};

	/*int SC1nodes[]={6,8,5,2,1,12,43,25};
	int SC2nodes[]={52,1,5,2,43,48,25};
	int SC3nodes []={5,6,8,2,3,45,30,48};
*/
	/*int SC1nodes[]={8,14,43,2,1,35,37,41,52,14,16,46,39,44};
	int SC2nodes[]={8,3,2,1,41,43,14,16,30,35,52,39,50,44};
	int SC3nodes []={14,8,3,2,1,16,30,43,48,15,52,13,39,44};*/

	
	int SC1nodes[]={1,4,49,40,37,31,23,13};
	int SC2nodes[]={9,6,13,19,27,34,44,49};
	int SC3nodes []={46,6,49,44,31,13,27,23};

	if(SC->SCtype==MIoT)
		NodesID.insert(NodesID.end(),SC1nodes,SC1nodes+8);
	else
		//LA:webservice
		if(SC->SCtype==AugmentedReality)
			NodesID.insert(NodesID.end(),SC2nodes,SC2nodes+8);
		else 
			if(SC->SCtype==SmartFactory)
				NodesID.insert(NodesID.end(),SC3nodes,SC3nodes+8);
	return NodesID;
...
//LA:to see where is the problem that dijkstra couldn't find a path
char* NetMan::DVNF_BottleNeck(AbstractNode* Src,AbstractNode* Dst)
{
	 char* bNeck="Null";
	 bool Destination=false;
	 list<AbstractLink*>::const_iterator itr ;
	 list<UniFiber*>::const_iterator itrPath ;
	 AbstractNode *pVSrc, *pVDst;
	 LINK_COST linkcost;
	 list<AbstractLink*> path;
	 AbstractLink *pLink ;
	 ...
	 if (Src==BottelNeck)
		...
		if(Dst==BottelNeck)
			...
			//LA:it can be either the dst or the intermediate node so should check the outgoing links of the bottleneck
			for(itr=BottelNeck->m_hOLinkList.begin();itr!=BottelNeck->m_hOLinkList.end();itr++)
			{
				if(((*itr)->getDst())==Dst){
					bNeck="Dst";
					Destination=true;
			...
			//LA:check the invalidated links entering the Dst to see if the bottleneck is in the list of incoming links of Dst or not
			for(itr=Dst->m_hILinkList.begin();itr!=Dst->m_hILinkList.end();itr++)
			{
				if((*itr)->getSrc()==BottelNeck && !(*itr)->m_bValid){
					bNeck="Dst";
					Destination=true;
					break;
				}
		...
		//LA:check if the Dst is on the way to get to the Bottleneck node 
		if(bNeck!="Dst"){
			
			if(BeforeBNeck.back()==BottelNeck)
				BBNeck=BeforeBNeck[BeforeBNeck.size()-2];
			else
				BBNeck=BeforeBNeck[BeforeBNeck.size()-1];
			

		pVSrc =m_hWDMNet.m_hNodeList.find(BBNeck->getId());
		pVDst = m_hWDMNet.m_hNodeList.find(BottelNeck->getId());
		linkcost= m_hWDMNet.Dijkstra(path,pVSrc,pVDst,AbstractGraph::LCF_ByOriginalLinkCost);
		for(itr=path.begin();itr!=path.end();itr++)
			{
				pLink = (AbstractLink*)(*itr);
				if(((*itr)->getSrc())==BottelNeck){
					bNeck="Dst";
					//LA:otherwise the path in reprovision function will be a mess
					BottelNeck=pVDst;
					Destination=true;
					break;
				}
...
//bool NetMan::DVNF_Reprovision(Connection* pCon,AbstractNode* Src,AbstractNode* Dst )
//{
//	#ifdef DEBUGLA
//		cout<<"\n DVNF_Reprovision"<<endl;
//	#endif
//	AbstractNode *pVSrc, *pVDst;
//	LINK_COST linkcost,linkcostSrc,linkcostDst,SPCost;
//	list<AbstractLink*> path,SrcPath,LinksToInval;
//	list<AbstractLink*>::const_iterator itr,itrPath,itrDst,itrNew ;
//	list<Connection*>::const_iterator itrC ;
//	Connection * Conn, *MinConn;
//	AbstractLink *pLink ,*pLinkDst;
//
//	double BwToPro=0,hPQbW=0;
//	int i=0,counter=0,location=0,wavelength=0;
//	BinaryHeap<Connection*, vector<Connection*>, Connection::PConnComp> hPQ;
//	list<AbstractLink*> PrevPath,SPPath;
//	list<AbstractLink*>::reverse_iterator itrR;
//	list <int> indices;
//	list<int>::const_iterator h;
//	int WlSrc=-1;
//	bool FirstTime=true;
//	bool bwprov=false;
//	//list<Connection*> ConnsToMove;
//	vector<double>  WL_array;
//	//LA:reset the counter for the moved connections
//	ConnCounter=0;
//	vector <bool>bwproArray;
//	int CounterMove=0;
//	bool allbwprov=false;
//	list <Connection*> ConsNeedToMove;
//	
//
//	//LA:keep a copy of bottleneck and before from abstractgraph
//	BottelNeck=m_hWDMNet.BottelNeck;
//	BeforeBNeck=m_hWDMNet.BeforeBNeck;
//
//	//LA:1-:check where is the bottleneck
//	pCon->m_SC->BlckingLocation=DVNF_BottleNeck(Src,Dst);
//	
//	//LA:it doesn't matter where is the bottleneck src,mid,dst have the same procedure
//	//if(pCon->m_SC->BlckingLocation!="Src"){
//
//	//validate all the links
//	m_hWDMNet.resetLinks();
//
//	//invalidate links with capacity less than min capacity required for SC
//Calculate: invalidateSimplexLinkDueToCapOrStatus(pCon->m_SC->MinBw);
//	path.clear();
//	SrcPath.clear();
//	SPPath.clear();
//	linkcostDst=linkcostDst=SPCost=UNREACHABLE;
//	//calculate shortest path between bottleneck and dst
//	pVSrc =m_hWDMNet.m_hNodeList.find(BottelNeck->getId());
//	pVDst = m_hWDMNet.m_hNodeList.find(pCon->m_nDst);
//	linkcostSrc= m_hWDMNet.Dijkstra(path,pVSrc,pVDst,AbstractGraph::LCF_ByOriginalLinkCost);
//
//	//LA:calculate shortestpath between src and bottleneck
//	pVSrc =m_hWDMNet.m_hNodeList.find(pCon->m_nSrc);
//	pVDst = m_hWDMNet.m_hNodeList.find(BottelNeck->getId());
//	linkcostDst= m_hWDMNet.Dijkstra(SrcPath,pVSrc,pVDst,AbstractGraph::LCF_ByOriginalLinkCost);
//
//	linkcost=linkcostSrc+linkcostDst;
//	//LA:calculate shortestpath between src and dst of connection
//	pVSrc=m_hWDMNet.m_hNodeList.find(pCon->m_nSrc);
//	pVDst=m_hWDMNet.m_hNodeList.find(pCon->m_nDst);
//	SPCost=m_hWDMNet.Dijkstra(SPPath,pVSrc,pVDst,AbstractGraph::LCF_ByOriginalLinkCost);
//
//	//LA:if the cost of shortestpath between src and dst is lower than the cost of shortestpath between bottlenech and dst use that path instead
//	if(linkcost>=SPCost /*&& SPCost!=0 */||linkcostSrc==0 )
//		path=SPPath;
//	
//	//LA:clear connections to move
//		for(itrR=path.rbegin();itrR!=path.rend();itrR++){
//			pLink = (AbstractLink*)(*itrR);
//			pLink->m_ConsToMove.clear();
//		}
//
//	/*if (linkcost==0)
//		cin.get();*/
//
//
//	//try to move connection on the shortest path to new lightpath along the same link till the capacity is enough for the new connection on all the links
//	if(linkcost!=UNREACHABLE||SPCost!=UNREACHABLE){
//		
//		#ifdef DEBUGLA
//		if(path.size()>1)
//			cout<<"\n 2 paths"<<endl;
...
//				
//				#ifdef DEBUGLA
//					cout<<"\n Starting of the for cycle"<<endl;
//				#endif
//				//LA:Counter to check if there are more than one lightpath in the way to get to the Dst
//			/*	i++;
//				
//				//LA: there are more than one lightpaths to reach dst from bottelneck
//				if(i>1)
//				//LA:reset the value of bw provisioned
//					BwToPro=0;*/
//	
//				pLink = (AbstractLink*)(*itrR);
//				pLink->m_ConsToMove.clear();
//				pLink->m_Src_WLs.clear();
//				pLink->m_Dst_WLs.clear();
//				pLink->m_Loc_Wls.clear();
//				
//				
//				//LA: this does not clear the hPQ need to build it each time
//				//hPQ.empty();
//				BinaryHeap<Connection*, vector<Connection*>, Connection::PConnComp> hPQ;
//				if(pLink->m_linkCap<pCon->m_eBandwidth){
//					//validate all the links
//					m_hWDMNet.resetLinks();
//
//					//invalidate links in current 2 shortest paths
//					LinksToInval.push_back(pLink);
//					//LA:just 2 shortest path 
//					if(LinksToInval.size()>1)
//					{	invalidateSimplexLinkDueToCapOrStatus(pCon->m_eBandwidth);
//						return false;
//					}
//					for(itrNew=LinksToInval.begin();itrNew!=LinksToInval.end();itrNew++){
//						AbstractLink* pLinkInval=(*itrNew);
//						pLinkInval->invalidate();
//					}
//					
//					goto Calculate;
//					
...
//				invalidateSimplexLinkDueToCapOrStatus(pCon->m_eBandwidth);
//				if(!(pLink->m_bValid)){
//					//LA:keep the array of wls on critical link to see when pcon can be provisioned	
//					WL_array=pLink->m_linkCap_Array;
//					indices.clear();
//					bwprov=false;
//					bwproArray.clear();
//					CounterMove=0;
...
//
//						#ifdef DEBUGLA
//								cout<<"\n building the connections"<<endl;
//						#endif
//						for(itrC=pLink->m_ConsOnLP.begin();itrC!=pLink->m_ConsOnLP.end();itrC++)
//						{
//							
//							Conn = (Connection*)(*itrC);
//							if(Conn->m_eBandwidth<pCon->m_eBandwidth){
//								//LA: insert the item according to increasing connection bw
//								hPQ.insert(*itrC);
//								hPQbW+=Conn->m_eBandwidth;
//							}
//						}
//					}
//					else
//						continue;
//
//					//LA:if the connections can't provide bw requested by the connection
//					if(pCon->m_eBandwidth>hPQbW && pLink->m_linkCap<pCon->m_eBandwidth)
//						return false;
//
//					if(pCon->m_eBandwidth>hPQbW && pLink->m_linkCap>pCon->m_eBandwidth)
//						continue;
//
//					//LA:1- find wls with enough capacity to accommodate at least one connection
//					#ifdef DEBUGLA
//					cout<<"\n Finding wls with enough capacity"<<endl;
//					#endif
//ForArray:			WlSrc=-1;
//					for (int k=0; k<WL_array.size();k++)
//					{ 
//						if(WL_array[k]>=pCon->m_SC->MinBw){
//							indices.push_back(k);
//							if(WlSrc==-1)
//								WlSrc=k;
//							else
//								if(WL_array[k]>WL_array[WlSrc])
//									WlSrc=k;						
//								
//						}
//					}
//
//					//LA:remove the src wl from the list of indices
//					indices.remove(WlSrc);
//
//
//
//					//for(itrC=pLink->m_pLightpath->m_ConsOnLP.begin();itrC!=pLink->m_pLightpath->m_ConsOnLP.end();itrC++)
//					while(!hPQ.empty())
//					{
//						if(bwprov)
//							break;
...
//						hPQ.popMin();
//
//						//LA:reset everything->seems that it us destroying everything alreay calculated
//						if(!(find(ConsNeedToMove.begin(),ConsNeedToMove.end(),MinConn)==ConsNeedToMove.end()))
//							continue;
//						else{
//						MinConn->NewPath.clear();
//						MinConn->SrcWl=-1;
//						MinConn->DstWl=-1;
//						MinConn->NeedsPath=false;
//						}
//						 
//						/*if(MinConn->DstWl!=-1 || MinConn->NeedsPath)
//							continue;*/
//						
//
//						//LA:2- find wl related to this connection on plink and put it in candidatewl
//
//						/*that part for mid is more generic so these two are not needed
//						//LA:if bottleneck is dst last con wl is the plink wl 
//						if(pCon->m_SC->BlckingLocation=="Dst")
//							candidatewl=Conn->m_Con_Wl.back();
//
//						else 
//							if(pCon->m_SC->BlckingLocation=="Src")
//								candidatewl=Conn->m_Con_Wl.front();
//
//						else
//							if(pCon->m_SC->BlckingLocation=="Mid"){*/
//								int ConCount=0;
//								for(itrPath=MinConn->m_SC->SCPath.begin();itrPath!=MinConn->m_SC->SCPath.end();itrPath++){
//									if(((AbstractLink*)(*itrPath))->getId() ==pLink->getId()){
//										MinConn->SrcWl=MinConn->m_Con_Wl[ConCount];
//										break;
//									}
//									ConCount++;
//								}
//								if(ConCount==MinConn->m_SC->SCPath.size()){
//									cout<<"\n Reprovision function same wl:"<<endl;
//									cin.get();
//								}
//								/*#ifdef DEBUGLA
//								cout<<"Srcwl connection: "<<MinConn->m_nSequenceNo<<" is :"<<MinConn->SrcWl<<endl;
//								#endif*/
//							//LA:check the wl of conn to be same as wlsrc
//							//LA:if it is not the same choose another connection 
//								if(WlSrc!=MinConn->SrcWl && hPQ.empty())
//									goto ForArray;
//
//								if(WlSrc!=MinConn->SrcWl)
//									continue;
//								
//							//}
//						if(indices.size()<2){
//							
//							//LA:try to find a new path for minconn that connects last nfv node to dst
//
//							//validate all the links
//FindNewPath:				m_hWDMNet.resetLinks();
//							#ifdef DEBUGLA
//							cout<<"\n FindPath for connection:"<<MinConn->m_nSequenceNo<<endl;
//							#endif
//							//invalidate links with capacity less than min capacity required for SC
//							invalidateSimplexLinkDueToCapOrStatus(MinConn->m_eBandwidth);
//							//Invalidate plink 
//							pLink->m_bValid=false;
//
//							//if(pLink->m_pSrc->m_nNodeId!=(MinConn->m_SC->SCPath.back())->m_pSrc->m_nNodeId)
//							//	cin.get();
//							//pVSrc=m_hWDMNet.m_hNodeList.find(MinConn->m_SC->SCnodes.back()->getId());
//							//LA:instead of building new link between src and dst just replace the link in mincon path
//							//pVSrc=m_hWDMNet.m_hNodeList.find(MinConn->m_nSrc);
//							
//							//LA:just if plink is last link on Minconn path do these things
//							if(pLink->getDst()->getId()==MinConn->m_nDst){
//								pVSrc=m_hWDMNet.m_hNodeList.find(MinConn->m_SC->SCnodes.back()->getId());
//								pVDst=m_hWDMNet.m_hNodeList.find(MinConn->m_nDst);
//							}
//							linkcostDst= m_hWDMNet.Dijkstra(MinConn->NewPath,pVSrc,pVDst,AbstractGraph::LCF_ByOriginalLinkCost);
//						
//							if(MinConn->NewPath.size()!=0 ){
//								#ifdef DEBUGLA
//								cout<<"\n A path is found for MinConn"<<endl;
//							#endif
//								MinConn->NeedsPath=true;
...
//						
//					/*	if(!FirstTime){
//							//LA:check the wl of conn to be same as candidatewl
//							//LA:if it is not the same choose another connection 
//							if(Conn->SrcWl!=WlSrc)
//								continue;
//						}
...
//							WlSrc=Conn->SrcWl;*/
//	
//							/*LA: not needed since I do everything at one step
//							pLink->m_pLightpath->m_ConsToMove.push_front(Conn->m_nSequenceNo,Conn);*/
//					
//					//LA:move the connections
//					//LA:invalidate the lightpaths on the path then try to find path for the connections that need to be moved  
//					//LA:check all incoming links of path Dst with src same as path src
//				/*	for(itrDst=pLink->m_pDst->m_hILinkList.begin();itrDst!=pLink->m_pDst->m_hILinkList.end();itrDst++)
//					{
//						pLinkDst = (AbstractLink*)(*itr);
//						if((pLinkDst->getDst())==(pLink->getDst()) && (pLinkDst->getSrc())==(pLink->getSrc())&& Conn->m_eBandwidth<=pLinkDst->m_linkCap){
//							Conn->NewPath.push_back(pLinkDst);
//							
//							break;
//						}
//					
...
//					}*/
//				
//					//LA:3-move the connections from one of the indeces to free some space
//#ifdef DEBUGLA
//						cout<<"\n Size of indices is:"<<indices.size()<<endl;
//#endif
//						
//						for(h=indices.begin();h!=indices.end();h++){												
//							//LA:then move this conn to new wl the one with enough capacity-> Policy:first fit
//							if(WL_array[(*h)]>=MinConn->m_eBandwidth){
//								MinConn->DstWl=(*h);
//								//cout<<"\n h in indices is: "<<*h;
//								#ifdef DEBUGLA
//								cout<<"\n DStwl of minconn:"<<MinConn->m_nSequenceNo<<" is:"<<MinConn->DstWl;
//								#endif
//								break;
//							}
//					
//						}
//					//	while(BwToPro<=pCon->m_eBandwidth) 
//
//						
//					/*LA:if can't find a path even for a connection it should return false 
//					if(Conn->NewPath.size()==0)*/
//						//LA:if no dst wl is found for the connection return false
//						if(MinConn->DstWl==-1)
//							goto FindNewPath;
//
//						//cout<<"\n After for loop for indices Dstwl is:"<<MinConn->DstWl;
//
//						//LA:4-change the wl for the plink for the connection
//						//4-1:release bw on the wl of conn->wlsrc 
//						/*for(int k=0;k<pLink->abs_m_nW;k++)
//						{
//							if(k==Conn->SrcWl){
//								pLink->abs_m_pChannel[k].m_nCapacity += Conn->m_eBandwidth;						
//								pLink->m_linkCap_Array[k]+= Conn->m_eBandwidth;
//								break;
//							}
//
//						}*/
//#ifdef DEBUGLA
//						cout<<"\n SrcWl is:"<<MinConn->SrcWl<<endl;
//#endif
//						//LA:add the connection to list of connections that need to be moved later
//
//						pLink->m_Src_WLs.push_back(MinConn->SrcWl);
//						pLink->m_Dst_WLs.push_back(MinConn->DstWl);
//						
//#ifdef DEBUGLA
//						cout<<"\nPutting mincon in list of connections to move: "<<MinConn->m_nSequenceNo; 
//#endif
//						pLink->m_Loc_Wls.push_back(ConCount);
//
//IncreaseCounter:		ConnCounter++;
//					
//						pLink->m_ConsToMove.push_back(MinConn);
//						ConsNeedToMove.push_back(MinConn);
//						
//
//							
//					//// log this provisioning, increasing counter of accepted or blocked connections-->not needed
//					//MinConn->log(m_runLog);	// Statistical Log Add by Andrea  
//					//MinConn->log(m_hLog);
//
//					/*LA:add the connection bandwidth to the bw provided till now-not really working
//					BwToPro+=Conn->m_eBandwidth;*/
//					for(int k=0;k<pLink->abs_m_nW;k++)
//					{ 
//						if(k==MinConn->DstWl)
//							WL_array[k]-=MinConn->m_eBandwidth;
//					
//						if(k==MinConn->SrcWl){
//							WL_array[k]+=MinConn->m_eBandwidth;
//							if(WL_array[k]>=pCon->m_eBandwidth){
//								bwprov=true;
//								//break;
//							}
//							//break;
...
//					}
//					if(bwprov)
//						bwproArray.push_back(bwprov);
//				
//					if(ConnCounter>=MAX_Con_Moved)
//								break;
//				
//				
//					/*LA:reset the prevpath variable
//					PrevPath.clear();*/
//					//FirstTime=false;
//#ifdef DEBUGLA
//						cout<<"\n End of while in reprovision"<<endl;
//#endif
//					}//end of while
//					
//					if(MinConn->DstWl==-1 && !MinConn->NeedsPath){
//						for(itrR=path.rbegin();itrR!=path.rend();itrR++){
//					for(itrC=(*itrR)->m_ConsToMove.begin();itrC!=(*itrR)->m_ConsToMove.end();itrC++){
//				(*itrC)->NewPath.clear();
//						(*itrC)->SrcWl=-1;
//						(*itrC)->DstWl=-1;
//						(*itrC)->NeedsPath=false;
//						}
//				}
//						return false;
//					}
...
//						
//					
//				}//end of for cycle for each link 
//				
//		if(ConnCounter<=MAX_Con_Moved && bwproArray.size()==CounterMove && CounterMove!=0){
//					for(itrR=path.rbegin();itrR!=path.rend();itrR++){
//						int WlCounter=0;
//						if(!((*itrR)->m_bValid)){
//							#ifdef DEBUGLA
//											cout<<"\n it is a new link"<<endl;
//											
//										#endif
//							for(itrC=(*itrR)->m_ConsToMove.begin();itrC!=(*itrR)->m_ConsToMove.end();itrC++){
//								/*if((*itrC)->DstWl==-1)
//									cin.get();*/
//								if((*itrC)->NewPath.size()==0 || !(*itrC)->NeedsPath||(*itrC)->DstWl!=-1){
//										#ifdef DEBUGLA
//											cout<<"\n size of newpath is:"<<(*itrC)->NewPath.size()<<endl;
//											cout<<"\n Does it need path: "<<(*itrC)->NeedsPath<<endl;
//										#endif
//											if((*itrC)->DstWl==-1)
//												cin.get();
//									DVNF_MoveConnections((*itrC),(*itrR),WlCounter);
//									WlCounter++;
//								}
//								else
//								{	
...
//if(allbwprov){
//	#ifdef DEBUGLA
//		cout<<"\n Reprovisioning is successfull"<<endl;
//	#endif
//		m_hLog.m_CounterConn+=ConnCounter;
//		if(ConnCounter>m_hLog.m_Highest_count)
//			m_hLog.m_Highest_count=ConnCounter;
//	return true;
//}
//	else{
//			for(itrR=path.rbegin();itrR!=path.rend();itrR++){
...
//void NetMan::DVNF_MoveConnections(Connection* MinConn, AbstractLink* pLink,int WlCounter){
//	#ifdef DEBUGLA
//	cout<<"\n ->DVNF_move connections with wl for connection:"<<MinConn->m_nSequenceNo<<endl;
//	#endif
//	float latency;
//	#ifdef DEBUGLA
//	cout<<"\n DstWl is:"<<MinConn->DstWl<<" WlCounter is: " <<WlCounter<<endl;
//	#endif
//
//	pLink->abs_m_pChannel[pLink->m_Src_WLs[WlCounter]].m_nCapacity += MinConn->m_eBandwidth;						
//	pLink->m_linkCap_Array[pLink->m_Src_WLs[WlCounter]]+= MinConn->m_eBandwidth;
//	
//	
//	//4-2:consume bw on the wl of conn->dstwl
//	#ifdef DEBUGLA
//	cout<<"\n before channel"<<endl;
//	#endif
//	
//	pLink->abs_m_pChannel[pLink->m_Dst_WLs[WlCounter]].m_nCapacity -= MinConn->m_eBandwidth;		
//	#ifdef DEBUGLA
//	cout<<"\n before mlinkcap array"<<endl;
//	#endif
//	
//	pLink->m_linkCap_Array[pLink->m_Dst_WLs[WlCounter]]-= MinConn->m_eBandwidth;
//							
//	//4-3:update conn->m_Con_Wl at concount location and put as dstwl
//	/*#ifdef DEBUGLA
//		cout<<"\n updating con_wl concount is:"<<ConCount<<endl;
//	#endif*/
//	#ifdef DEBUGLA
//	cout<<"\n before m con wl"<<endl;
//	#endif
//	
//	MinConn->m_Con_Wl[pLink->m_Loc_Wls[WlCounter]]=pLink->m_Dst_WLs[WlCounter];
//	/*//LA:change the path for the connection 
//	//LA:1-1:save the path for the connection-not needed as i will copy link by link later
//	//PrevPath=Conn->m_SC->SCPath;
//	//LA:1-2: Build the new path
//		for(itrPath=Conn->m_SC->SCPath.begin();itrPath!=Conn->m_SC->SCPath.end();itrPath++)
//		{
//			PrevPath.push_back((*itrPath));
//			if(((*itrPath)->getSrc())==Conn->NewPath.front()->getSrc() && (*itrPath)->getDst()==Conn->NewPath.front()->getDst() )
//			{
//				//PrevPath.remove((*itrPath));
//				//LA: cant insert abstractlink in a list of lightpath
//				PrevPath.pop_back();
//				PrevPath.push_back(*itrPath);
//				location=counter;
//			}
//			counter++;					
//		}*/
//	//LA:find a wavelength for the newpath
//	/*commented as it will be done in updatelinkbw function
//	for(int k=0;k<(Conn->NewPath.front())->abs_m_nW;k++){
//		if(Conn->NewPath.front()->abs_m_pChannel[k].m_nCapacity>Conn->m_eBandwidth){
//			wavelength=k;
//			break;
//		}
//	}
//	*/
//	/*LA:deprovision connection on the previous path but don't touch VNFs and locations		
//	Conn->Reprov=true;		
//	DVNF_Deprovision(Conn);
//	Conn->Reprov=false;		*/
//	//LA:provision on the new path (like what is done in DVNF_provisionSC) 
//	//Conn->m_eStatus = Connection::SETUP;
//	#ifdef DEBUGLA
//		cout<<"\n Printing new path for service chain in Reprovision function"<<endl;
//	#endif
//	MinConn->LogReprov=true;
//	DVNF_Dump_SC(MinConn->m_SC);
//	/*LA:Clear SC path before inserting something new
//	Conn->m_SC->SCPath.clear();
//	*/
//	/* commented as it will be done in updatelinkbw function
//	//LA:replace the wl in location with the new wl that is found for the path
//	Conn->m_Con_Wl[location]=wavelength;*/
//	/*Conn->m_SC->SCPath=PrevPath;
//	//LA:consume bw on the new path
//	m_hWDMNet.DVNF_updateLinkBW(Conn);*/
//	// Assign connection routing time
//	//pCon->m_dRoutingTime = Conn->m_pPCircuit->latency;
//	//LA:calculate latency again for the connection and check if the latency is violated or not					
//	#ifdef DEBUGLA
//		cout<<"\nCalculating latency for the new path of SC"<<endl;
//	#endif		
//	/*latency=computeLatency(MinConn->m_SC->SCPath);
//	if (latency>MinConn->m_SC->latency)
//		m_hLog.m_Latency_Violated++;*/
//		MinConn->NewPath.clear();
//		MinConn->SrcWl=-1;
//		MinConn->DstWl=-1;
//		MinConn->NeedsPath=false;
...
//}
////
//void NetMan::DVNF_MoveConnections(Connection* Conn){
//	#ifdef DEBUGLA
//	cout<<"\n -> DVNF_move connections for connection: "<<Conn->m_nSequenceNo<<endl;
//	#endif
//	int NodesID,SrcID=-1,DstID=-1;
//	float latency;
//	//vector <int> DstNodesID,NFVnodes;
//	//vector <int> CandID;
//	//int dstlocalid=UNREACHABLE;
//	AbstractNode* pVSrc;
//	AbstractNode* pVDst;
//	LINK_COST Dijkstracost=UNREACHABLE,DstCost=UNREACHABLE;
//	list<AbstractLink*> ShortestPathLA,SP;
//	list<AbstractNode*>::const_iterator itr;
//	Vertex* pNode;
//	int TempDst=UNREACHABLE;
//	list<Connection*>::const_iterator itrC ;
//	//list<SimplexLink*>::const_iterator itrPath;
//	list<AbstractLink*>::const_iterator itrDst,itrPath,itrnew,itrSC;
//	//list<Connection*> ConnsToMove;
//	vector <AbstractNode*> OldNodes;
//	BinaryHeap<Connection*, vector<Connection*>, Connection::PConnComp> hPQ;
//	
//	double BwPro=-1;
//	list<SimplexLink*> PrevPath;
//	
//	bool bwprov=false,found=false;
//
//				//LA:save nodes on scpath to build the new path later using them
//					for(itrDst=Conn->m_SC->SCPath.begin();itrDst!=Conn->m_SC->SCPath.end();itrDst++){
//						AbstractLink * pLinkDst = (AbstractLink*)(*itrDst);
//							OldNodes.push_back(pLinkDst->getSrc());
//							if(pLinkDst->m_pSrc->getId()==Conn->m_SC->SCnodes.back()->getId()){
//								OldNodes.push_back(pLinkDst->getDst());
//								break;
//							}
//						}
//				
//
//					/*LA:find the location of link to be removed and replace it with shortestpathLA->this cause problem of lightpath assertion fail
//					found=false;
//					for(itrDst=Conn->m_SC->SCPath.begin();itrDst!=Conn->m_SC->SCPath.end();itrDst++)
//					{	
//						pLinkDst = (SimplexLink*)(*itrDst);
//						if(Conn->m_SC->SCnodes.back()->getId()==((Vertex*)pLinkDst->getDst())->m_pOXCNode->getId() && pLinkDst->m_hFreeCap<=CHANNEL_CAPACITY){
//							found=true;
//							NewPath.push_back(pLinkDst);
//						}
//						if(!found)
//							NewPath.push_back(pLinkDst);							
//					}
//
//					
//					NewPath.insert(NewPath.end(),ShortestPathLA.begin(),ShortestPathLA.end());*/
//						
//					//LA:deprovision connection on the previous path but don't touch VNFs and locations 
//					Conn->Reprov=true;		
//					DVNF_Deprovision(Conn);
//					Conn->Reprov=false;	
//
//					//Provision on new path 
//					//LA:Clear SC path before inserting something new
//					Conn->m_SC->SCPath.clear();
//					this->m_hWDMNet.resetLinks();
//					invalidateSimplexLinkDueToCapOrStatus(Conn->m_eBandwidth);
//					ShortestPathLA.clear();
//					for(int k=0;k<OldNodes.size()-1;k++)
//					{
//						if(OldNodes[k]->getId()==Conn->m_SC->SCnodes.back()->getId())
//							break;
//						pVSrc=m_hWDMNet.m_hNodeList.find(OldNodes[k]->getId());
//						pVDst=m_hWDMNet.m_hNodeList.find(OldNodes[k+1]->getId());
//						
//						Dijkstracost= m_hWDMNet.Dijkstra(ShortestPathLA,pVSrc,pVDst,AbstractGraph::LCF_ByOriginalLinkCost);
//						Conn->m_SC->SCPath.insert(Conn->m_SC->SCPath.end(),ShortestPathLA.begin(),ShortestPathLA.end());			
//					}
//			//LA:build conn path one by one->this will cause redundant links in scpath
//			//ShortestPathLA.clear();
//			//this->m_hGraph.resetLinks();
//			//invalidateSimplexLinkDueToCapOrStatus(Conn->m_eBandwidth);
//			//
//			//for(int k=0;k<Conn->m_SC->SCnodes.size();k++)
//			//{
//			//	ShortestPathLA.clear();
//			//	if(k==0)
//			//			SrcID=Conn->m_nSrc;
//			//	else
//			//		SrcID=Conn->m_SC->SCnodes[k-1]->getId();
//			//
//			//DstID=Conn->m_SC->SCnodes[k]->getId();
//			//pVSrc=m_hGraph.lookUpVertex(SrcID, Vertex::VT_Access_Out,-1);
//			//pVDst=m_hGraph.lookUpVertex(DstID, Vertex::VT_Access_In,-1);
//			//Dijkstracost= m_hGraph.Dijkstra(ShortestPathLA,pVSrc,pVDst,AbstractGraph::LCF_ByOriginalLinkCost);
//			///*for(itrDst=ShortestPathLA.begin();itrDst!=ShortestPathLA.end();itrDst++)
//			//{ 
//			//	/*if((*itrNew)->getSrc()==LinkToReplace->getSrc()&&(*itrNew)->getDst()==LinkToReplace->getDst()&&(*itrNew)->getId()!=LinkToReplace->getId())
//			//		Conn->m_SC->SCPath.push_back(LinkToReplace);
//			//	else*//*
//			//		Conn->m_SC->SCPath.push_back((*itrDst));
//			//}*/
//			//Conn->m_SC->SCPath.insert(Conn->m_SC->SCPath.end(),ShortestPathLA.begin(),ShortestPathLA.end());
//			//	
//			//}
//			//
//			//ShortestPathLA.clear();
//			//SrcID=Conn->m_SC->SCnodes.back()->getId();
//			//DstID=TempDst;
//			//pVSrc=m_hGraph.lookUpVertex(SrcID, Vertex::VT_Access_Out,-1);
//			//pVDst=m_hGraph.lookUpVertex(DstID, Vertex::VT_Access_In,-1);
//			//Dijkstracost= m_hGraph.Dijkstra(ShortestPathLA,pVSrc,pVDst,AbstractGraph::LCF_ByOriginalLinkCost);
...
//
//
//					/*LA:converting lightpath to abstractlink to pass to NewCircuit function
//					for(itrPath=NewPath.begin();itrPath!=NewPath.end();itrPath++)
//						Conn->m_SC->SCPath.push_back((*itrPath));*/
//				
//					/* LA:causes the problem of lightpath not being equal 
//					//LA:converting lightpath to abstractlink to pass to NewCircuit function
//					for(itrPath=NewPath.begin();itrPath!=NewPath.end();itrPath++)
//						Conn->m_SC->SCPath.push_back((SimplexLink*)(*itrPath));*/
//		
//			
//			//LA:replace the newpath in connection path -> it will not work because links may not have enough capacity by now
//			//Conn->m_SC->SCPath.insert(Conn->m_SC->SCPath.end(),Conn->NewPath.begin(),Conn->NewPath.end());
//
//			int PathCount=0;
//				for(itrDst=Conn->NewPath.begin();itrDst!=Conn->NewPath.end();itrDst++){
//					ShortestPathLA.clear();
//					Dijkstracost=UNREACHABLE;
//					AbstractLink* SLink=(AbstractLink*)(*itrDst);
//					if(PathCount==0){
//						SrcID=(Conn->m_SC->SCPath.back())->getDst()->getId();
//						DstID=SLink->getDst()->getId();
//						itrDst=Conn->NewPath.begin();
//					}
//					else{
//						SrcID=SLink->getSrc()->getId();
//						DstID=SLink->getDst()->getId();
//
//					}
//
...
//					Dijkstracost=m_hWDMNet.Dijkstra(ShortestPathLA,pVSrc,pVDst,AbstractGraph::LCF_ByOriginalLinkCost);
//					PathCount++;
//					if(Dijkstracost==UNREACHABLE)
//						cin.get();
//					Conn->m_SC->SCPath.insert(Conn->m_SC->SCPath.end(),ShortestPathLA.begin(),ShortestPathLA.end());
//				}
//			//bool first=true;
//			//for(itrDst=Conn->NewPath.begin();itrDst!=Conn->NewPath.end();itrDst++){
//			//	SimplexLink * pLinkDst = (SimplexLink*)(*itrDst);
//			//	if(pLinkDst->m_hFreeCap>CHANNEL_CAPACITY)
//			//		continue;
//			//	int SrcID,DstID;
//			//	/*if(first){
//			//		SrcID=OldNodes.back()->getId();
//			//		DstID=((Vertex *) pLinkDst->getSrc())->m_pOXCNode->getId();
//			//		first=false;
//			//		itrD--;
//			//	}
//			//	else{*/
//			//		SrcID=((Vertex *) pLinkDst->getSrc())->m_pOXCNode->getId();
//			//		DstID=((Vertex *) pLinkDst->getDst())->m_pOXCNode->getId();
//			//	//}
//			//	pVSrc=m_hGraph.lookUpVertex(SrcID, Vertex::VT_Access_Out,-1);
//			//	pVDst=m_hGraph.lookUpVertex(DstID, Vertex::VT_Access_In,-1);
//			//	Dijkstracost= m_hGraph.Dijkstra(ShortestPathLA,pVSrc,pVDst,AbstractGraph::LCF_ByOriginalLinkCost);
...
//
//			/*//LA:build circuit 
//				DVNF_NewCircuit(*(Conn->m_pPCircuit),Conn->m_SC->SCPath,-1,Conn);
//		
//				//LA:consume Bw
//				Conn->m_pPCircuit->Unprotected_setUpCircuit(this);*/
//			
//			m_hWDMNet.DVNF_updateLinkBW(Conn);
//			Conn->LogReprov=true;
//			DVNF_Dump_SC(Conn->m_SC);
//					//LA:calculate latency again for the connection and check if the latency is violated or not					
//					latency=computeLatency(Conn->m_SC->SCPath);
//					if (latency>Conn->m_SC->latency)
//						m_hLog.m_Latency_Violated++;
//
//				//LA:reset em all	
//					Conn->NewPath.clear();
