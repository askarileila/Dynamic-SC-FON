...
bool TopoReader::readTopo(WDMNetwork &hNetwork, const char *pTopoFile)
{
	...
	//LA:commented: related to DC topology
	//bSuccess = readTopoHelper(hNetwork, fin);
	//LA: for HetNet topology
	bSuccess = this->DVNFReadTopoHelper(hNetwork, fin);
	
	fin.close();
	return bSuccess;
}
//LA:for DC (fat-tree) topology
...
//Salvataggio di tutti i parametri dei nodi e della fibre presi dal file di topologia passato in input
{
	...
	const char *pUniFiberFormat = "%u, %u, %u, %u, %u, %u, %u";  //LA: format changed
#define _CRT_SECURE_NO_DEPRECATE //
	...
	hNetwork.nDC = nDC;
	/*LA:commented: it's not used in my project
	assert(fin && (nDC > 0));
	...
	*/
	...
	UINT nCount = 0;
	int nNodeId, nConversion, nGrooming, nTxAtNode, nRxAtNode, nFlag, nNFVFlag;
	...
	while ((nCount < nOXCNodes) && fin.getline(pBuf, MAX_LINE_LENGTH, cStartDelimiter))
	{
		...
		sscanf(pBuf, pOXCNodeFormat, &nNodeId, &nConversion, &nGrooming, &nTxAtNode, &nRxAtNode, &nFlag, &nNFVFlag);
		if (nTx > 0)
			nTxAtNode = nTx;
		if (nRx > 0)
			nRxAtNode = nRx;
		hNetwork.addNode(new OXCNode(nNodeId,
			...
			(OXCNode::GroomingCapability)nGrooming, nTxAtNode, nRxAtNode, nWavelengths, nFlag, nNFVFlag));
		nCount++;
	// salvataggio del Dummy Node (flag -1 nel file di topologia)	
        /* LA: commented as I don't use this flag for datacenter purposes in my topology file 
	...
	// riempimento vettore con ID dei Data Center
      ...
      /* LA: this part is for using the nflag(core,aggregate,edge) 
			Don't know where I can make use of this flag
	...
	nCount = 0;
	int nFiberId, nSrc, nDst, nLinkCap, nW;
	int nLength;
	//double hCost; //-B: commented
	double hCostOld=1;
	//double nLatency; //LA: added but later commented as it's defined in AbstractGraph

	//acquisizione dati UniFibers
	while ((nCount < nUniFibers) 
		&& fin.getline(pBuf, MAX_LINE_LENGTH, cStartDelimiter)) {
		fin.getline(pBuf, MAX_LINE_LENGTH, cEndDelimiter);
		sscanf(pBuf, pUniFiberFormat, &nFiberId, &nSrc, &nDst, &nW, &hCostOld, &nLinkCap, &nLength);
		...
		//LA: as I'm not useing number of wavelength commented the following lines
		/*
		if (nWavelengths > 0)	
			nW = nWavelengths;	
		*/
	//	hNetwork.addUniFiberDCTopo(nFiberId, nSrc, nDst, nW, (LINK_COST)hCostOld, nLinkCap, nLength); //LA: modified
		nCount++;
	}
...
//LA:for HetNet topology
bool TopoReader::DVNFReadTopoHelper(WDMNetwork &hNetwork, ifstream &fin)
//Salvataggio di tutti i parametri dei nodi e della fibre presi dal file di topologia passato in input
{
#ifdef DEBUGB
	cout << "\t-> DVNF ReadTopoHelper" << endl;
#endif // DEBUGB
	const int MAX_LINE_LENGTH = 1024;
	const char *pWavelengthCount = "NumberOfWavelengths = ";
	const char *pChannelCapacity = "ChannelCapacity = ";
	...
	const char cStartDelimiter = '<';
	const char cEndDelimiter = '>';
	const char *pUniFiberFormat = "%u, %u, %u, %u, %lf, %f, %u";
#define _CRT_SECURE_NO_DEPRECATE //
	...
	UINT nCount = 0;
	int nNodeId, nConversion, nGrooming, nTxAtNode, nRxAtNode, nFlag, NFVFlag, cellFlag, nArea;

	//acquisizione dati OXCNodes
	while ((nCount < nOXCNodes) && fin.getline(pBuf, MAX_LINE_LENGTH, cStartDelimiter))
	{
		fin.getline(pBuf, MAX_LINE_LENGTH, cEndDelimiter);
		sscanf(pBuf, pOXCNodeFormat, &nNodeId, &nConversion, &nGrooming, &nTxAtNode, &nRxAtNode, &nFlag, &NFVFlag, &cellFlag, &nArea);
		if (nTx > 0)
			nTxAtNode = nTx;
		if (nRx > 0)
			nRxAtNode = nRx;
		//  Core CO (flag -1 of toplogy file)- LA:core co is dummmy node	
		if (nFlag < 0) {
			if (jj > 0) {
			...
			int	DummyNode = nNodeId;
			hNetwork.DummyNode = DummyNode;
			NFVFlag = 1; //-LA: the core CO is always a NFV enabled node
			jj++;
		}
		hNetwork.addNode(new OXCNode(nNodeId,
				(OXCNode::WConversionCapability)nConversion,
				(OXCNode::GroomingCapability)nGrooming, nTxAtNode, nRxAtNode, nWavelengths, nFlag, NFVFlag, cellFlag, (OXCNode::UrbanArea)nArea));
		nCount++;
	...
	while ((nCount < nUniFibers)
		&& fin.getline(pBuf, MAX_LINE_LENGTH, cStartDelimiter)) {
		fin.getline(pBuf, MAX_LINE_LENGTH, cEndDelimiter);
		sscanf(pBuf, pUniFiberFormat, &nFiberId, &nSrc, &nDst, &nW, &hCostOld, &nLength,&tree);
		//-B: comment this kind of binary cost assignment so that it will save the cost given in the input topology file
		/*if (nSrc == hNetwork.DummyNode || nDst == hNetwork.DummyNode){    // se è Dummy Node
		hCost=0;           // tutti i link tra Dummy Node e i DC
		}
		...
		if (nWavelengths > 0){
			nW = nWavelengths;
		hNetwork.NumberOfWavelength=nWavelengths;
		}
		hNetwork.addUniFiber(nFiberId, nSrc, nDst, nW, (LINK_COST)hCostOld, nLength,tree);
		nCount++;
	}
