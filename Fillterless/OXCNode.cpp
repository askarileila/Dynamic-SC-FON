...
#include "OXCNode.h"
//#include "NetMan.h"  //LA:added to support NumNFV
...
OXCNode::OXCNode(UINT nNodeId, WConversionCapability eConversion,
	GroomingCapability eGrooming, UINT nTx, UINT nRx, int Nchan, int nFlag, int nNFVFlag) :
	AbstractNode(nNodeId, Nchan), m_eConversion(eConversion), m_eGrooming(eGrooming), m_dTrafficProcessed(0), m_dExtraLatency(0),
	m_nTx(nTx), m_nRx(nRx), m_nMaxW(0), m_nBBUNodeIdsAssigned(0), m_nNetStage(UNREACHABLE), m_nBetweennessCentrality(0),
	...
	m_pStateWDM(NULL), m_bNewLP(false), m_Flag(nFlag), m_NFVFlag(nNFVFlag), m_dCostMetric(UNREACHABLE), m_dTrafficGen(0),m_NumVNF(0),
	ChosenAsSrc(false)
{
	//LA:core co has unlimited cpu Capacity
	if(m_Flag==-1)
		CPURes=UNREACHABLE;
	else
		CPURes=24;
...
OXCNode::OXCNode(UINT nNodeId, WConversionCapability eConversion,
	GroomingCapability eGrooming, UINT nTx, UINT nRx, int Nchan, int nFlag, int nNFVFlag, int cellFlag, UrbanArea nArea) :
	AbstractNode(nNodeId, Nchan), m_eConversion(eConversion), m_eGrooming(eGrooming), m_nBetweennessCentrality(0),
	m_nTx(nTx), m_nRx(nRx), m_nMaxW(0), m_nBBUNodeIdsAssigned(0), m_cellFlag(cellFlag), m_nNetStage(UNREACHABLE),
	m_nFreeTx(nTx), m_nFreeRx(nRx), m_nBBUReachCost(UNREACHABLE), m_nReachabilityDegree(0), m_nProximityDegree(0),
	m_pStateWDM(NULL), m_bNewLP(false), m_Flag(nFlag), m_NFVFlag(nNFVFlag), m_dCostMetric(UNREACHABLE),
	m_nArea(nArea), m_fDistanceFromCoreCO(0), m_dTrafficGen(0), m_dTrafficProcessed(0), m_dActivityTime(0),
	m_dExtraLatency(0), m_uNumOfBBUSupported(0),m_NumVNF(0),ChosenAsSrc(false)
{
	...
	{
		m_NFVnode = true;
		//LA: commented as it's moved to net man
		//m_NumNFV++;
...
/*OXCNode::OXCNode():CPURes(64) //LA:added to support oxcnode built in netman for storing numnfv
{
	
}*/
 bool NS_OCH::operator< (const OXCNode& rhs, const OXCNode& rhl){
	return rhs.m_NumVNF< rhl.m_NumVNF;
}
...
UINT OXCNode::getNumberOfSources()
{
	...
	if (this->isMacroCell())
	{
		...
		numOfSources += SMALLCELLS_PER_MC * floor(double(MAXTRAFFIC_SMALLCELL / BWDGRANULARITY));
	}
...
//LA: to see how many VNF instances are activated on the node
/*
UINT OXCNode::getNumberOfActivatedVNFS()
{
	UINT activatedVNFs =0;
	too har node bayad ye araye as vnf ha bashe harkodoom active shod 1 beshe harkodoom deactive 0
	pas bayad class e vnf az classe oxcnode be ers bebare 
	va tooye oxcnode ham ye property e public tarif konam ke vnf instance e active shode bashe 
	yani hamon araye eee az vnf ha 
	...
	return activatedVNFs;
}
*/
//LA: to see how many vnf instances a nfv enabled node can host 
/*
UINT OXCNode::getNumberOfVNFs()
{
#ifdef DEBUGB
	cout << "-> getNumberOfSources" << endl;
#endif // DEBUGB
