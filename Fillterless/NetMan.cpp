...
//	int SC3nodes []={5,6,8,2,3,4};
	...
	int SC3nodes []={10,5};
*/
	//filterless dst 25 nodes: scnodes+8
	//int SC1nodes[]={1,4,49,40,37,31,23,13};
	//int SC2nodes[]={9,6,13,19,27,34,44,49};
	//int SC3nodes []={46,6,49,44,31,13,27,23};

	//filterless dst 16 nodes+6
	int SC1nodes[]={1,4,49,40,37,31};
	int SC2nodes[]={9,6,13,19,27,34};
	int SC3nodes []={46,6,49,44,27,23};
	/*int SC1nodes[]={1,4,9,40,37,31,23,13,51,34,19};
	int SC2nodes[]={9,6,13,19,27,34,44,49,4,40,25};
	int SC3nodes []={46,6,49,44,31,13,27,23,4,40,33,25};*/

	if(SC->SCtype==MIoT)
		NodesID.insert(NodesID.end(),SC1nodes,SC1nodes+6);
	else
		//LA:webservice
		if(SC->SCtype==AugmentedReality)
			NodesID.insert(NodesID.end(),SC2nodes,SC2nodes+6);
		else 
			if(SC->SCtype==SmartFactory)
				NodesID.insert(NodesID.end(),SC3nodes,SC3nodes+6);
...
bool NetMan::DVNF_Reprovision(Connection* pCon,AbstractNode* Src,AbstractNode* Dst )
{
	...
	list<AbstractLink*> path,SrcPath;
	list<AbstractLink*>::const_iterator itr,itrPath,itrDst ;
	list<Connection*>::const_iterator itrC ;
	Connection * Conn;
	AbstractLink *pLink ,*pLinkDst;
	list<Connection*> ConToMove;
	...
	invalidateSimplexLinkDueToCapOrStatus(pCon->m_SC->MinBw);*/
				...
				/*	for(itrDst=pLink->m_pDst->m_hILinkList.begin();itrDst!=pLink->m_pDst->m_hILinkList.end();itrDst++)
					{
						...
						//LA:find wls with enough capacity to accomodate at least one connection
						for (int k=0; k<pLink->m_linkCap_Array.size();k++)
						{ 
							if(pLink->m_linkCap_Array[k]>pCon->m_SC->MinBw)
								indices.push_back(k);
					...
					*/
					...
					// log this provisioning, increasing counter of accepted or blocked connections
					Conn->log(m_runLog);	// Statistical Log Add by Andrea  
					Conn->log(m_hLog);

					//LA:add the connection bandwidth to the bw provided till now 
