...
AbstractLink::AbstractLink(int nId, AbstractNode* pSrc, AbstractNode* pDst,
			 LINK_COST hCost, float nLength, UINT m_nW,int tree): m_nLinkId(nId), 
			 m_pSrc(pSrc), m_pDst(pDst), m_hCost(hCost), m_nLength(nLength),
			 m_bValid(true), m_bCostSaved(false), m_dLinkLoad(0),
			 m_nBackupVHops(0), m_pBackupCost(NULL),
			 m_pCSet(NULL), m_nCSetSize(0), m_nBChannels(0),m_linkCap(AB_LINK_CAPACITY),abs_m_pChannel(NULL),abs_m_nW(m_nW),LinkTree(tree)
{
	m_latency = m_nLength * (float)PROPAGATIONLATENCY;
}									
...
double AbstractLink::getCapacity  () const //LA:added for the capacity of link
{
	return m_linkCap;
	//return m_hBand;
}
...
bool AbstractGraph::addLink(AbstractLink *pLink)
{
	...
	for (w = 0; w < nW; w++)
	{
		pLink->m_linkCap_Array.push_back(CHANNEL_CAPACITY);
...
AbstractLink* AbstractGraph::addLink(int nId, UINT nSrc, UINT nDst, 
							LINK_COST hCost, float nLength, float nLinkCap,int tree)  //LA:modified to add nLinkCap
{
	AbstractNode *pSrc = m_hNodeList.find(nSrc);
	AbstractNode *pDst = m_hNodeList.find(nDst);
	if ((NULL == pSrc) || (NULL == pDst))
	...
	AbstractLink *pLink = new AbstractLink(nId, pSrc, pDst, hCost, nLength, nLinkCap,tree);
	pSrc->addOutgoingLink(pLink);
	pDst->addIncomingLink(pLink);
	m_hLinkList.push_front(pLink->getId(), pLink);
...
AbstractLink* AbstractGraph::addLink(int nId, UINT nSrc, UINT nDst, 
							LINK_COST hCost, float nLength, float nLinkCap)  //LA:modified to add nLinkCap
{
	AbstractNode *pSrc = m_hNodeList.find(nSrc);
	AbstractNode *pDst = m_hNodeList.find(nDst);
	if ((NULL == pSrc) || (NULL == pDst))
												  ...
												  AbstractNode *pDst)
{
	...
	while (pPrevLink && (nCount++ < nMaxCount))
	{
		...
		cout << " - link: ";
		if (pSLink->getSimplexLinkType() == SimplexLink::LT_Rx)
		{
			AbstractNode*pVertex = pSLink->getSrc();
			cout << pVertex->getId() << "->";
			pVertex = pSLink->getDst();
			cout << pVertex->getId();
			cout<<endl;
...
bool AbstractGraph:: DVNF_updateLinkBW(  Connection* pCon)
{
	
	int counter=0;
	int WlSelected=-1;
	list<AbstractLink*>::const_iterator itr,itrj;
	AbstractLink* Link;
	vector<int> FreeWl;
	//Link=pCon->m_SC->SCPath.back();
	bool found=false;

	/*//LA:find free wls of the links on path
		for (itr=pCon->m_SC->SCPath.begin(); itr!=pCon->m_SC->SCPath.end(); itr++){
			(*itr)->FreeWl.clear();
			for(int i=0;i<(*itr)->abs_m_nW;i++){
				if((*itr)->abs_m_pChannel[i].m_nCapacity>=pCon->m_eBandwidth)				
					(*itr)->FreeWl.push_back(i);
			}
		}
	itr=pCon->m_SC->SCPath.begin();
		for(itrj=itr++; itrj!=pCon->m_SC->SCPath.end(); itrj++){
			for(int j=0;j<(*itr)->FreeWl.size();j++){
				if((*itr)->FreeWl[j]==(*itrj)->FreeWl[j] && (*itr)->LinkTree==(*itrj)->LinkTree){
					(*itrj)->abs_m_pChannel[j].m_nCapacity=-1;
					(*itrj
				}
	...
	*/
	/*LA:not useful when we have trees 
	for (itr=pCon->m_SC->SCPath.begin(); itr!=pCon->m_SC->SCPath.end(); itr++){
		for(int i=0;i<(*itr)->abs_m_nW;i++){
			if((*itr)->abs_m_pChannel[i].m_nCapacity>=pCon->m_eBandwidth){				
				counter++;			
	...
	//LA:if links on scpath are on the same tree counter can not be incremented
	//for (itr=pCon->m_SC->SCPath.begin(); itr!=pCon->m_SC->SCPath.end(); itr++){
	

	//if(counter==pCon->m_SC->SCPath.size()){
		for (itr=pCon->m_SC->SCPath.begin(); itr!=pCon->m_SC->SCPath.end(); itr++){
			found=false;
			for(int i=0;i<(*itr)->abs_m_nW;i++){
				//cout<< (*itr)->abs_m_pChannel[i].m_nCapacity << endl;
									
					
					if((*itr)->LinkTree!=-1){
						if((*itr)->abs_m_pChannel[i].m_nCapacity==CHANNEL_CAPACITY){
							found=true;
						/*(*itr)->abs_m_pChannel[i].consumeCh((*itr),i,-1);
						(*itr)->m_linkCap_Array[i]=-1;*/
						DVNF_OccupyTree(i,(*itr)->LinkTree,(*itr)->getId());
						}
						else 
							continue;
					}
						...
						if((*itr)->abs_m_pChannel[i].m_nCapacity>=pCon->m_eBandwidth){
							found=true;
							//LA:update the array of link capacity
							(*itr)->m_linkCap_Array[i]-=pCon->m_eBandwidth;
							(*itr)->abs_m_pChannel[i].consumeCh((*itr),i,pCon->m_eBandwidth);

						}
						else
							continue;
					}
					(*itr)->m_linkCap =((*itr)->m_linkCap - pCon->m_eBandwidth);
					pCon->m_Con_Wl.push_back(i);
					//LA:add the connection to list of connections on this link
					(*itr)->m_ConsOnLP.push_back(pCon);
				
					//LA:check if it is working properly
					if((*itr)->m_linkCap<-1 || (*itr)->m_linkCap>AB_LINK_CAPACITY)
						cin.get();

					//LA:exit the for cycle as a wl is found
					if(found)
						break;
					else{
						//LA:recover channels that are set to -1 in the calculations above
						itr=pCon->m_SC->SCPath.begin();
						for(int K=0;K<pCon->m_Con_Wl.size();K++){
							(*itr)->abs_m_pChannel[pCon->m_Con_Wl[K]].m_nCapacity=CHANNEL_CAPACITY;
							(*itr)->m_linkCap =((*itr)->m_linkCap + pCon->m_eBandwidth);
							if((*itr)->LinkTree!=-1)
								DVNF_ReleaseTree(pCon->m_Con_Wl[K],(*itr)->LinkTree,(*itr)->getId());				
							itr++;
						}
					pCon->m_Con_Wl.clear();
					return false;
			...
			//LA;if uncomment this it will check only one wl 
				/*	else{
						//LA:recover channels that are set to -1 in the calculations above
						itr=pCon->m_SC->SCPath.begin();
						for(int K=0;K<pCon->m_Con_Wl.size();K++){
		...
		pCon->m_Num_link=pCon->m_SC->SCPath.size();
		return true;
...
void AbstractGraph::DVNF_OccupyTree(int wavelength , int tree, int LinkID){
		
	list<AbstractLink*>::const_iterator itr;
	//LA:for all the links that are on the same tree set the capacity of the wl to -1 if they are not the link we already devoted capacity to
	for (itr = m_hLinkList.begin(); itr != m_hLinkList.end(); itr++){
		if((*itr)->LinkTree==tree /*&& (*itr)->getId()!=LinkID*/){
			for(int i=0;i<(*itr)->abs_m_nW;i++){
				if(i==wavelength){
					(*itr)->abs_m_pChannel[i].m_nCapacity=-1;
					(*itr)->m_linkCap_Array[i]=-1;					
					break;
				}
...
void AbstractGraph::DVNF_ReleaseTree(int wavelength , int tree, int LinkID){
	...
	for (itr = m_hLinkList.begin(); itr != m_hLinkList.end(); itr++){
		if((*itr)->LinkTree==tree /*&& (*itr)->getId()!=LinkID*/){
			for(int i=0;i<(*itr)->abs_m_nW;i++){
				if(i==wavelength){
					(*itr)->abs_m_pChannel[i].m_nCapacity=CHANNEL_CAPACITY;
					(*itr)->m_linkCap_Array[i]=CHANNEL_CAPACITY;
					break;
				}
...
void AbstractGraph::DVNF_deprovLinkBW(Connection* pCon)
{
	bool found;
		list<AbstractLink*>::const_iterator itr;
		...
		for (itr=pCon->m_SC->SCPath.begin(); itr!=pCon->m_SC->SCPath.end(); itr++){
			j++;
		//for(int j=0;j<pCon->m_SC->SCPath.size();j++){
		//found=false;

			for(int i=0;i<(*itr)->abs_m_nW;i++){
				//if(found)
				//	break;				
					if(i==pCon->m_Con_Wl[j]){
						
						(*itr)->m_linkCap += pCon->m_eBandwidth;
						
						if((*itr)->LinkTree!=-1){
							(*itr)->abs_m_pChannel[i].m_nCapacity=CHANNEL_CAPACITY;
							(*itr)->m_linkCap_Array[i]=CHANNEL_CAPACITY;
							DVNF_ReleaseTree(i,(*itr)->LinkTree,(*itr)->getId());
						}
						else{
							(*itr)->m_linkCap_Array[i]+= pCon->m_eBandwidth;
							(*itr)->abs_m_pChannel[i].m_nCapacity += pCon->m_eBandwidth;
						}
						for(itrc=(*itr)->m_ConsOnLP.begin();itrc!=(*itr)->m_ConsOnLP.end();itrc++){
							if((*itrc)->m_nSequenceNo==pCon->m_nSequenceNo){
								(*itr)->m_ConsOnLP.erase(itrc);
								break;
							}
						}
						//found=true;
		...
		//LA:for wavlength continutity
		/*for (itr=pCon->m_SC->SCPath.begin(); itr!=pCon->m_SC->SCPath.end(); itr++){
			if((*itr)->abs_m_pChannel[pCon->m_SC->Wavelength].m_nCapacity+bw<=CHANNEL_CAPACITY)
				(*itr)->abs_m_pChannel[pCon->m_SC->Wavelength].m_nCapacity=(*itr)->abs_m_pChannel[pCon->m_SC->Wavelength].m_nCapacity+bw;
			else 
				cin.get();
