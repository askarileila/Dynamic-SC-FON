...
SimplexLink::SimplexLink(int nLinkId, Vertex* pSrc, Vertex* pDst, 
	...
	m_hFreeCap(double(hCap)), m_nBackupCap(0), m_nBWToBeAllocated(0)
{
...
void SimplexLink::consumeBandwidth(double nBW)
{
	if(m_hFreeCap < nBW){
		cout<<"\nin simplex link the m_hFreeCap is:" << m_hFreeCap << " and nBw: "<< nBW;
		cin.get();
	}

	//assert(m_hFreeCap >= nBW);
	consumeBandwidthHelper(0 - (double)nBW);
}

void SimplexLink::releaseBandwidth(double nBW)
{
	consumeBandwidthHelper(nBW);
}

void SimplexLink::consumeBandwidthHelper(double nBW)
{
#ifdef DEBUGB
	cout << "->consumeBandwidthHelper" << endl;
	cout << "\tReleasing bandwidth(+)/ Consuming Bandwidth(-) " << nBW << " on simplex link corresponding to channel " << this->m_nChannel << endl;
#endif // DEBUGB
	m_hFreeCap += nBW;
}
