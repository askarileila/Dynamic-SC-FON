...
public:
	...
	std::string				LatencyClass; //Stringent, intermediate,relaxed
	...
	bool					Grouped;
	ServiceType				 SCtype;
	std::string						BlckingLocation;			//LA:reason for the SC that is blocked 
	int						MinNumUsers;				//LA:min number of users
	double					MinBw;						//LA:min bw required by SCs
	ServiceChain();
	~ServiceChain();
