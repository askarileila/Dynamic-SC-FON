...
public:
	...
	void validateSimplexLinkDueToCap(double residualBWD);
	...
	void preferWavelChannel();
	void removeSimplexLinkLightpath(Lightpath * pLightpath);
	void removeSimplexLinkLightpath();
	void releaseSimplexLinkLightpathBwd(double nBWToRelease, Lightpath*pLightpath);
	...
	SimplexLink* addSimplexLink(UINT nLinkId, Vertex *pSrc, Vertex *pDst,
			...
			int nChannel, double hCap = INFINITE_CAP);
...
protected:
	...
	void invalidateVNode(list<AbstractNode*>&, Vertex*);
