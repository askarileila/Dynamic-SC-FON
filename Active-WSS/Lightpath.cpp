...
Lightpath::~Lightpath()
{
	...
	for (itr=m_ConsOnLP.begin(); itr!=m_ConsOnLP.end(); itr++)
		delete (*itr);*/
}
...
double Lightpath::getFreeCapacity() const
{
...
{
  //if(m_nFreeCapacity>m_nCapacity){
		//cout<<"\n "<<m_nFreeCapacity <<" > "<< m_nCapacity;
		//cin.get();
  //}
...
void Lightpath::increaseConflictSet(const Circuit& hBCircuit)
{
    ...
    double eBW = hBCircuit.m_pCircuit->m_eBW;
...
#ifdef DEBUGB
	cout << "\tLightpath #" << this->getId() << " free capacity: " << m_nFreeCapacity << endl << endl;
#endif // DEBUGB
...
void Lightpath::Unprotected_setUpLightpath(NetMan* pNetMan, Circuit *pCircuit,bool tempReprov)
{
#ifdef DEBUGB
	cout << "-> Unprotected_setUpLightpath" << endl;
	cout << "\tLightpath route size: " << m_hPRoute.size() << endl;
	...
	cout << "\tLightpath #" << this->getId() << " free capacity: " << m_nFreeCapacity << endl << endl;
#endif // DEBUGB
	updateSimplexLinkLpCapacity(this, pNetMan, m_nBWToBeAllocated,tempReprov);
	// reset value
	m_nBWToBeAllocated = 0;
...
void Lightpath::updateSimplexLinkLpCapacity(Lightpath*pLightpath, NetMan*pNetMan, double BWToBeAllocated,bool tempReprov )
{
	pNetMan->consumeBandwOnSimplexLink(pLightpath, BWToBeAllocated,tempReprov);
}
void Lightpath::updateSimplexLinkLpCapacity(Lightpath*pLightpath, NetMan*pNetMan, double BWToBeAllocated)
{
...
#endif // DEBUGB
	//LA:changed to double to support double bw
	//UINT nBWToRelease;
	double nBWToRelease;
	switch (pCircuit->m_eCircuitType)
	{
...
#ifdef DEBUGB
	cout << "\tFreeCap del lightpaht " << this->getId() << " = " << m_nFreeCapacity << " (ho rilasciato " << nBWToRelease << ")" << endl;
#endif // DEBUGB
