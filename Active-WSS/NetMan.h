...
public:
	...
	Graph*					m_BackupGraph;			//LA:to save temporary state of the network in case of reprovisioning
	UINT					VNFCounter;
	std::ofstream			outfile;				//LA:to save sc chain path in a file
	float					TotSimTime;
	UINT					ConnCounter;			//LA:number of connections moved 
	...
	void NetMan::DVNF_NewCircuit(Circuit& , const list<AbstractLink*>& , UINT , Connection*,Graph m_BackupGraph);//LA:for reprovisioning
	...
	char* NetMan::DVNF_BottleNeck(Vertex*,Vertex*);
	
	void invalidateSimplexLinkDueToCapOrStatus(Graph*,double );

	void NetMan::DVNF_UpdateGraphBeforeMove(Connection* ,bool,AbstractLink*); //LA:update graph links before moving connections
	int SameNodeVNFs; //LA:all VNFs of the SC are on the same node
	int DiffNodeVNFs;	//LA:not all the VNFs of the SC are on the same node
...
//////////////////////////// -B: ADDED BY ME
	...
	bool checkAvailabilitySimplexLink(UINT srcId, UINT dstId, int w, double nBW);
	...
	void releaseLinkBandwidth(Lightpath*, double); //-B
	...
	void invalidateSimplexLinkDueToCapOrStatus(double bwd);
	...
	void releaseVirtualLinkBandwidth(double nBWToRelease, Lightpath * pLightpath);
	...
	float computeLatency(list<AbstractLink*>& path, Connection*);

	void clearPrecomputedPath();

	void setPrecomputedPath(Circuit*);

	void consumeBandwOnSimplexLink(Lightpath * pLightpath, double BWToBeAllocated);
	void consumeBandwOnSimplexLink(Lightpath * pLightpath, double BWToBeAllocated,bool); //LA:to support reprov
