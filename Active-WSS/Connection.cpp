...
Connection::Connection(): m_eStatus(REQUEST), 
	...
	grooming(0),m_SC(NULL),m_bBlockedDueToCapacity(false),m_ConLat(0)
{
}
//LA:correct constructor
Connection::Connection(UINT nSeqNo, UINT nSrc, UINT nDst,
	SimulationTime tArrivalTime, SimulationTime tHoldingTime,
	double eBW, ProtectionClass ePC) :
	...
	m_bBlockedDueToTx(false), m_bBlockedDueToLatency(false), m_nHopCount(0),m_bBlockedDueToCapacity(false),m_ConLat(0),Reprov(false),m_Num_link(0)/*,SrcWl(-1),DstWl(-1),NeedsPath(false),LogReprov(false)*/
{
	m_dRoutingTime = 0.0000; //-B: max latency CPRI
}
	/*LA:added to support double type bw for SC bw-commented as i changed the above constructor
	Connection::Connection(UINT nSeqNo, UINT nSrc, UINT nDst,
	SimulationTime tArrivalTime, SimulationTime tHoldingTime,
	double eBW, ProtectionClass ePC) :
	m_nSequenceNo(nSeqNo), m_nSrc(nSrc), m_nDst(nDst), grooming(0),
	m_dArrivalTime(tArrivalTime), m_dHoldingTime(tHoldingTime),
	m_ServiceChainBw(eBW), m_eCPRIBandwidth(OC0), m_eProtectionClass(ePC), m_eStatus(REQUEST),
	m_pPCircuit(NULL), m_pBCircuit(NULL), m_bBlockedDueToUnreach(false), m_pCircuits(NULL),
	m_bBlockedDueToTx(false), m_bBlockedDueToLatency(false), m_nHopCount(0)
{
...
Connection::Connection(UINT nSeqNo, UINT nSrc, UINT nDst,
	...
	m_pPCircuit(NULL), m_pBCircuit(NULL), m_bBlockedDueToUnreach(false), m_pCircuits(NULL),
	m_bBlockedDueToTx(false), m_bBlockedDueToLatency(false), m_nHopCount(0), m_eConnType(connType),m_bBlockedDueToCapacity(false),m_ConLat(0),Reprov(false)
{
	m_dRoutingTime = 0.0000; //-B: max latency CPRI
}
...
const char* Connection::toString() const
{
	...
	{
		sprintf(pStr, ">Conn %5d | %3d->%3d | BW = %3d  | ArrTime = %4.4f | HoldTime = %1.4f | RoutingTime = %1.9f | ",
			m_nSequenceNo, m_nSrc, m_nDst, m_eBandwidth,  m_dArrivalTime, m_dHoldingTime, m_dRoutingTime);
		/*LA:modifed
...
void Connection::log(Log &hLog)
{
	...
	else if (m_eStatus == DROPPED)
	{
		...
		//LA:added to support blocking due to capacity shortage on nodes
		else if (this->m_bBlockedDueToCapacity)
		{
			hLog.m_nBlockedConDueToCapacity++;
		}
		switch (this->m_eConnType)
		{
			case MOBILE_FRONTHAUL:
