...
///////////////////////////////////////////////////////////////////////////////
//
template<class Key, class T> class MappedLinkList;   //LA: MappedLinkList is a template class with two parameters defined in mappedlinklist.h
class AbstractLink;
...
public:
	...
	int					m_pPrevWL;			//LA:wavelength of previous link
//	AbstractNode*		m_pPrevHop;			// previous node since there might be
	...
	//LA: commented to be able to use the links with more than candidate prevlink->as it is used somewhere else I can not comment it
	valarray<LINK_COST> costi;
	vector<vector <LINK_COST>> costiNew;
	...
	vector <vector<AbstractLink*>> prevNew;
...
public:
	...
	//AbstractLink(int, AbstractNode*, AbstractNode*, LINK_COST, float , UINT); //LA:added to support link cap
	~AbstractLink();
	virtual void dump(ostream&) const;
	...
	UINT getCapacity() const;		//LA: added to return the capacity of each link
...
public:
	...
	UINT			abs_m_nW;		// # of channels on this fiber
	// N.B: may need to use later on
	// list<int>	m_hSRGList;
	Channel			*abs_m_pChannel;
    ...
    int             m_used;
	float			m_latency;	//in seconds
	UINT			m_linkCap;  //LA: link capacity in G
	...
	valarray<LINK_COST>  wlOccupation;    //valori che indicano le wavelength libere
	...
	bool			m_bValid; //LA:moved to make it accessible in netman
	list<Connection*> ConsToMove;
...
public:
	...
	LINK_COST Dijkstra(list<AbstractLink*>&, AbstractNode*, AbstractNode*,
				LinkCostFunction hLCF = LCF_ByHop);	
	LINK_COST DijkstraWP(list<AbstractLink*>&, AbstractNode*, AbstractNode*,
				LinkCostFunction hLCF = LCF_ByHop);	
	...
	LINK_COST recordMinCostPathWP(list<AbstractLink*>& , AbstractNode*,AbstractNode*);
	...
	//LINK_COST DijkstraDVNF(list<AbstractLink*>&, AbstractNode * pSrc, AbstractNode * pDst, LinkCostFunction hLCF, NetMan *);
	...
	void DijkstraHelper(AbstractNode*, AbstractNode*,LinkCostFunction hLCF = LCF_ByHop);
	void DijkstraHelperWP(AbstractNode* , AbstractNode* ,LinkCostFunction hLCF = LCF_ByHop );
	//void DijkstraHelperDVNF(NetMan *,AbstractNode*, AbstractNode*, LinkCostFunction hLCF = LCF_ByHop);
	...
	AbstractNode* lookUpNodeByIdGraph(UINT) const;
	...
	//AbstractLink* addLink(int, UINT, UINT, LINK_COST, float, UINT);  //LA:modified 
	...
	void updateLinkBW(LINK_CAPACITY bw); //LUCA
	AbstractNode*	BottelNeck;	//LA:node causing dijkstra fail
	vector <AbstractNode*>	BeforeBNeck;	//LA:node before bottleneck dijkstra fail
	bool DVNF_updateLinkBW( LINK_CAPACITY , ServiceChain*); //LA: for the lighter version
	void DVNF_deprovLinkBW(LINK_CAPACITY , ServiceChain*); //LA: for the lighter version
	void deprovLinkBW(LINK_CAPACITY bw, int linkIDlist[]); //LUCA
	void restoreLinkBW(); //LUCA
