...
//LA:updated to work with aux graph 
bool AbstractNode::updateOutLinks(set<int>& chan)
{
	...
	for (itrLink = m_hOLinkList.m_hList.begin(); itrLink != m_hOLinkList.m_hList.end(); itrLink++)
	{
		...
		//updates2 = updates;
		//LA:commented as they are not useful for auxiliary graph 
		srcId = (*itrLink)->getSrc()->m_nNodeId;
		dstId = (*itrLink)->getDst()->m_nNodeId;
		/*srcId = ((Vertex*)(*itrLink)->getSrc())->m_pOXCNode->m_nNodeId;		
		dstId = ((Vertex*)(*itrLink)->getDst())->m_pOXCNode->m_nNodeId;*/
		if (!(*itrLink)->valid())
			continue;
		...
		for (ii = 0; ii < updates.size(); ii++)
		{
			...
			if(((SimplexLink*)(*itrLink))->m_hFreeCap==UNREACHABLE)
				newCost = costi[ii] + 0;
			else
				newCost = costi[ii] + ((*itrLink)->getCost()*((*itrLink)->wlOccupation[ii]));
			//aggiornamento costi
...
//LA:constructor working
AbstractNode::AbstractNode(UINT nNodeId,int Nchan): m_nNodeId(nNodeId), 
	m_pPrevLink(NULL), m_hCost(5), m_bValid(true), m_dLatency(0),
	m_nBackupVHops(0), m_pBackupCost(NULL),forceUpdate(true),costiParzialiFlag(false),m_pPrevWL(-1)
{
	prev = vector<AbstractLink*>(Nchan);
	costi = valarray<LINK_COST> (UNREACHABLE,Nchan);
	
	vector <vector<LINK_COST>> costiNew (Nchan, vector<LINK_COST>(Nchan,UNREACHABLE));
...
///////////////////////////////////////////////////////////////////////////////
// ABSTRACT LINK
//LA:modified it for the lighter version
/*AbstractLink::AbstractLink(): m_pSrc(NULL), m_pDst(NULL), m_bValid(true),
	...
	m_pCSet(NULL), m_nCSetSize(0), m_nBChannels(0),m_linkCap(AB_LINK_CAPACITY)
{
...
//LA: added to support capacity for abstract link
/*AbstractLink::AbstractLink(int nId, AbstractNode* pSrc, AbstractNode* pDst,
			 LINK_COST hCost, float nLength, UINT m_linkCap): m_nLinkId(nId), 
			 m_pSrc(pSrc), m_pDst(pDst), m_hCost(hCost), m_nLength(nLength),
			 m_bValid(true), m_bCostSaved(false), m_dLinkLoad(0),
			 m_nBackupVHops(0), m_pBackupCost(NULL),
			 m_pCSet(NULL), m_nCSetSize(0), m_nBChannels(0)
...
/*
//LA: added to support # of wavelength in abstractlink- lighter version
AbstractLink::AbstractLink(int nId, AbstractNode* pSrc, AbstractNode* pDst,
			 LINK_COST hCost, float nLength, UINT m_nW): m_nLinkId(nId), 
			 m_pSrc(pSrc), m_pDst(pDst), m_hCost(hCost), m_nLength(nLength),
			 m_bValid(true), m_bCostSaved(false), m_dLinkLoad(0),
			 m_nBackupVHops(0), m_pBackupCost(NULL),
			 m_pCSet(NULL), m_nCSetSize(0), m_nBChannels(0),m_linkCap(AB_LINK_CAPACITY),abs_m_pChannel(NULL),abs_m_nW(m_nW)
{
	m_latency = m_nLength * (float)PROPAGATIONLATENCY;
}									
...
UINT AbstractLink::getCapacity  () const //LA:added for the capacity of link
{
	return m_linkCap;
	//return m_hBand;
}
...
AbstractNode* AbstractGraph::lookUpNodeByIdGraph(UINT nNodeId) const
{
	list<AbstractNode*>::const_iterator itr;
	for(itr=m_hNodeList.begin();itr!=m_hNodeList.end();itr++)
	{
		if(((Vertex*)(*itr))->m_pOXCNode->getId()==nNodeId)
			return ((Vertex*)(*itr))->m_pOXCNode;
	}
...
bool AbstractGraph::addLink(AbstractLink *pLink)
{
	...
	//LA: not needed for grooming version
	/*
		int nW=pLink->abs_m_nW;
	Channel *pChannel = new Channel[nW];
	//Lightpath *pLightpath = new Lightpath[(OCLightpath / BWDGRANULARITY)];
	UINT  w;
	for (w = 0; w < nW; w++)
	{
		pChannel[w] = Channel(pLink, w, CHANNEL_CAPACITY, true, UP);
...
//AbstractLink* AbstractGraph::addLink(int nId, UINT nSrc, UINT nDst, 
//							LINK_COST hCost, float nLength, UINT nLinkCap)  //LA:modified to add nLinkCap
//{
//	AbstractNode *pSrc = m_hNodeList.find(nSrc);
//	AbstractNode *pDst = m_hNodeList.find(nDst);
//	if ((NULL == pSrc) || (NULL == pDst))
...
//		return NULL;
//
//	AbstractLink *pLink = new AbstractLink(nId, pSrc, pDst, hCost, nLength, nLinkCap);
//	pSrc->addOutgoingLink(pLink);
//	pDst->addIncomingLink(pLink);
//	m_hLinkList.push_front(pLink->getId(), pLink);
//
...
void AbstractGraph::reset4ShortestPathComputation()
{
	...
	for (itr = m_hNodeList.begin(); itr != m_hNodeList.end(); itr++)
	{
		//LA:reset also costi and prev values 
	((Vertex*)(*itr))->m_pOXCNode->costi=valarray<LINK_COST> (UNREACHABLE,NumWL);
	((Vertex*)(*itr))->m_pOXCNode->prev = vector<AbstractLink*>(NumWL);

	((Vertex*)(*itr))->m_pOXCNode->costiNew.clear();
												  ...
												  AbstractNode *pDst)
{
	...
	while (pPrevLink && (nCount++ < nMaxCount))
	{
		...
		/*LA:commented
		Vertex*VSrc = (Vertex*)pPrevLink->getSrc();
		Vertex*VDst = (Vertex*)pPrevLink->getDst();
		if(VSrc->m_pOXCNode->getId()!=VDst->m_pOXCNode->getId()) */
		if(pPrevLink->m_bValid)
			hPath.push_front(pPrevLink);
#ifdef DEBUGBB
		cout << "\tInserting simplex link -> ch: " << pSLink->m_nChannel
...
//LA:modified so much does not work properly 
 LINK_COST AbstractGraph::recordMinCostPathWP(list<AbstractLink*>& hPath, 
												  AbstractNode *pDst,AbstractNode *pSrc)
{
	bool First=true;
	int WLSelected=-1;
	list<AbstractLink*>::const_iterator itr, itrhPath, itrl;
	list<AbstractNode*>::const_iterator itrNode;
	//list<AbstractLink*>::reverse_iterator itrR;
	vector <int> WlsTried;
	vector <int> WlsChecked;
	list <UniFiber*> PrevPhys;
	AbstractLink* FirstPrevLink;
	vector <AbstractLink*> TriedPaths;
	bool Updated=false;
	bool done=false;
	bool redundant=false;
	AbstractLink *OldPrevLink;

#ifdef DEBUGB
	cout << "->recordMinCostPath WP*&" << endl;
#endif // DEBUGB
	...
	//-B: +++++++++++++ BUILD THE PATH: from destination to source, according to previous link poitner ++++++++++++++
	AbstractLink *pPrevLink = pDst->m_pPrevLink;
	if(pPrevLink  /*&&((SimplexLink*)(pPrevLink))->m_nChannel!=-1*/){
			//LA:to save first prevlink and compare it later
		if(((SimplexLink*)pPrevLink)->m_eSimplexLinkType== SimplexLink::LT_Lightpath)
			PrevPhys=((SimplexLink*)pPrevLink)->m_pLightpath->m_hPRoute;
		FirstPrevLink=pPrevLink;
	}

Begin:	while (pPrevLink && (nCount++ < nMaxCount))
	{

		#ifdef DEBUGLA
			cout << "\n Beginning of while"<<endl;
		#endif
		if(hPath.size()>0 && ((Vertex*)(hPath.back()->getSrc()))->m_pOXCNode==((Vertex*)(pSrc))->m_pOXCNode)
			break;
		...
		Vertex*VDst = (Vertex*)pPrevLink->getDst();
		if(VSrc->m_pOXCNode->getId()!=VDst->m_pOXCNode->getId()) */
		if(((SimplexLink*)(pPrevLink))->m_nChannel==-1){
			hPath.push_front(pPrevLink);
		//#ifdef DEBUGLA
		//		//cout << " \nChannnel is -1 "<<endl;
		//	if(hPath.size()>18)
		...
		//	#endif
			...
			if(First && pPrevLink->m_bValid){
				hPath.push_front(pPrevLink);
			if(((SimplexLink*)(pPrevLink))->m_nChannel!=-1){
				WLSelected=((SimplexLink*)(pPrevLink))->m_nChannel;
				if(((SimplexLink*)(FirstPrevLink))->m_nChannel==-1)
					FirstPrevLink=pPrevLink;
					OldPrevLink=pPrevLink;
				#ifdef DEBUGLA
				cout << "\n wavelength is selected as: "<<WLSelected<<endl;
			#endif
				First=false;
			}
		}
		...
		{
CheckWl:	//AbstractNode* DstN=hPath.front()->getDst();
			#ifdef DEBUGLA
				cout << "\n Begining of checkwl"<<endl;
			#endif
				if((((SimplexLink*)(pPrevLink))->m_nChannel==WLSelected||WLSelected==-1)&&pPrevLink->m_bValid && 
					((Vertex*)pPrevLink->m_pSrc)->m_pOXCNode->costi[WLSelected]<=((Vertex*)pPrevLink->m_pSrc)->m_pOXCNode->costi[WLSelected]&&pPrevLink->m_pSrc->m_hCost<=pPrevLink->m_pDst->m_hCost)
			{
			#ifdef DEBUGLA
				cout << "\n link will be inserted in the path and WLselected is:"<<WLSelected<<endl;
			#endif
			if(find(hPath.begin(),hPath.end(),pPrevLink)==hPath.end())
				redundant=false;
				if(hPath.size()!=0){
					for(itrhPath=hPath.begin();itrhPath!=hPath.end();itrhPath++){
						SimplexLink* LinkhPath= (SimplexLink*)(*itrhPath);	
						if(((((Vertex*)LinkhPath->getDst())->m_pOXCNode==((Vertex*)pPrevLink->getSrc())->m_pOXCNode)||
							(((Vertex*)LinkhPath->getSrc())->m_pOXCNode==((Vertex*)pPrevLink->getSrc())->m_pOXCNode))&& LinkhPath->m_nChannel!=-1){
								redundant=true;
								break;
						}
			...
			/*#ifdef DEBUGLA
				cout << "\n link is on the same wl or with wl as -1:"<<endl;
			#endif*/
			}
			else{
				#ifdef DEBUGLA
				cout << "\n Searching for a link on wlselcted among incoming links of dst"<<endl;
			#endif
				
jump:	 Updated=false;
				//for(itr=pPrevLink->getSrc()->m_hILinkList.begin();itr!=pPrevLink->getSrc()->m_hILinkList.end();itr++)
				
				/*for(int j=0;j<((Vertex*)pPrevLink->m_pDst)->m_pOXCNode->prev.size();j++)
				{
					if(((Vertex*)pPrevLink->m_pDst)->m_pOXCNode->costi[j]==UNREACHABLE)
						continue;*/
				SimplexLink* pLink;
				 SimplexLink* CostiPrev;
				 bool CostiCe=false;
				 Vertex * CandVertex;
				 //LA:commented as it will lead to wrong paths
				
				if(((Vertex*)pPrevLink->m_pDst)->m_pOXCNode->costi[WLSelected]<UNREACHABLE){
					CostiPrev=(SimplexLink*)(((Vertex*)pPrevLink->m_pDst)->m_pOXCNode->prev[WLSelected]);
				//if(CostiPrev)
					if(find(TriedPaths.begin(),TriedPaths.end(),CostiPrev)==TriedPaths.end() &&
						((Vertex*)CostiPrev->m_pSrc)->m_pOXCNode->costi[WLSelected]<=((Vertex*)CostiPrev->m_pDst)->m_pOXCNode->costi[WLSelected] && CostiPrev->m_pSrc->m_hCost<=CostiPrev->m_pDst->m_hCost){
						pLink=CostiPrev;
						CostiCe=true;
				}
			}
					if(!CostiCe/*||(((Vertex*)pPrevLink->m_pDst)->m_pOXCNode->costi[WLSelected]==UNREACHABLE)*/){
					
				//done=false;
					//if(((Vertex*)pPrevLink->m_pDst)->m_pOXCNode->costi[WLSelected]==UNREACHABLE)
					//	//LA:choose another wl
					//	goto Updating; 
					//LA:try to find new prevlink
Checkcost:			done=false;	
						
								#ifdef DEBUGLA
									cout << "\n before file cycle in checkcost"<<endl;
								#endif
								
					//LA:does it work? src can be different -> remove that condition->better to keep it->in case prevlink is wrong it will not reach the src so removed
					for(itr=pPrevLink->getDst()->m_hILinkList.begin();itr!=pPrevLink->getDst()->m_hILinkList.end();itr++){
							pLink = (SimplexLink*)(*itr);
							if(pLink->m_nChannel==WLSelected &&pLink->m_bValid && ((Vertex*)pLink->m_pSrc)->m_pOXCNode->costi[WLSelected]<=((Vertex*)pLink->m_pSrc)->m_pOXCNode->costi[WLSelected]&&
							pLink->m_pSrc->m_hCost<=pLink->m_pDst->m_hCost/*&&((Vertex*)pLink->m_pSrc)->m_pOXCNode==((Vertex*)pPrevLink->m_pSrc)->m_pOXCNode*/){
								if(find(TriedPaths.begin(),TriedPaths.end(),pLink)==TriedPaths.end()){
									//TriedPaths.insert(TriedPaths.end(),pLink);
									done=true;
									break;
								}
							}
					...
					//LA:look for dst': shuld give priority to lp
					#ifdef DEBUGLA
									cout << "\n before checking for CandVertex"<<endl;
								#endif
					if(!done){
						//CandVertex=NS_OCH::Graph::lookUpVertex(((Vertex*)pPrevLink->m_pDst)->m_pOXCNode->getId(), Vertex::VT_Access_Out,-1);
							for(itrNode=m_hNodeList.begin();itrNode!=m_hNodeList.end();itrNode++){
								CandVertex=(Vertex*)((AbstractNode*)(*itrNode));
								if(CandVertex->m_pOXCNode->getId()>((Vertex*)pPrevLink->m_pDst)->m_pOXCNode->getId())
									break;
								if(CandVertex->m_pPrevLink){
									
									if(((CandVertex->m_nChannel==WLSelected && CandVertex->m_eVType==Vertex::VT_Channel_In)|| (CandVertex->m_nChannel==-1 && CandVertex->m_eVType==Vertex::VT_Access_In && ((SimplexLink*)CandVertex->m_pPrevLink)->m_eSimplexLinkType==SimplexLink::LT_Lightpath)) 
										&& CandVertex->m_pOXCNode==((Vertex*)pPrevLink->m_pDst)->m_pOXCNode /*&&( (SimplexLink*)CandVertex->m_pPrevLink)->m_nChannel==WLSelected*/){
									//pLink=(SimplexLink*)CandVertex->m_pPrevLink;
										for(itrl=CandVertex->m_hILinkList.begin();itrl!=CandVertex->m_hILinkList.end();itrl++){
											pLink=(SimplexLink*)(*itrl);
											if(find(TriedPaths.begin(),TriedPaths.end(),pLink)==TriedPaths.end()&& find(hPath.begin(),hPath.end(),pLink)==hPath.end()
												&& ((Vertex*)pLink->m_pSrc)->m_pOXCNode->costi[WLSelected]<=((Vertex*)pLink->m_pSrc)->m_pOXCNode->costi[WLSelected]&& pLink->m_bValid
												&&pLink->m_nChannel==WLSelected &&pLink->m_pSrc->m_hCost<=pLink->m_pDst->m_hCost ){
												done=true;
												break;
											}
										}
...
NewWL:							/*if(!done){
								for(itr=pDst->m_hILinkList.begin();itr!=pDst->m_hILinkList.end();itr++){
								pLink = (SimplexLink*)(*itr);
								if(find(TriedPaths.begin(),TriedPaths.end(),pLink)==TriedPaths.end()&&pLink->m_bValid&&pLink->m_nChannel==WLSelected&& find(hPath.begin(),hPath.end(),pLink)==hPath.end())
								{
									TriedPaths.insert(TriedPaths.end(),hPath.begin(),hPath.end());
									hPath.clear();
									done=true;
									break;								
								}
					...
					//LA:try to find new firstlink 
					//LA:first look in m_hIlinklist of dst then search for another node on the same layer

				/*LA:commented all these sections related to dst seems not necessariy 29/03
				bool DstDone=false;	
				#ifdef DEBUGLA
									cout << "\n before checking incoming links of dst"<<endl;
								#endif
				if(!done){
					for(itr=pDst->m_hILinkList.begin();itr!=pDst->m_hILinkList.end();itr++){
						#ifdef DEBUGLA
									cout << "\n beginning of for cycle to check incoming link of dst"<<endl;
								#endif
							pLink = (SimplexLink*)(*itr);
							if(pLink->m_nChannel==WLSelected &&pLink->m_bValid //&&((Vertex*)pLink->m_pSrc)->m_pOXCNode==((Vertex*)pPrevLink->m_pSrc)->m_pOXCNode
								){
								if(find(TriedPaths.begin(),TriedPaths.end(),pLink)==TriedPaths.end()&& find(hPath.begin(),hPath.end(),pLink)==hPath.end()
									&& pLink->m_pSrc->m_hCost<=pLink->m_pDst->m_hCost){
									TriedPaths.insert(TriedPaths.end(),pLink);
									
									for(itrhPath=hPath.begin();itrhPath!=hPath.end();itrhPath++){
										SimplexLink* pLinkPath = (SimplexLink*)(*itrhPath);
										if(find(TriedPaths.begin(),TriedPaths.end(),pLinkPath)==TriedPaths.end())
											TriedPaths.insert(TriedPaths.end(),pLinkPath);
									}
									bool redundant=false;
									#ifdef DEBUGLA
									cout << "\n before checking redundant links in hpath wrt plink"<<endl;
								#endif
									for(itrhPath=hPath.begin();itrhPath!=hPath.end();itrhPath++){
										SimplexLink* LinkhPath= (SimplexLink*)(*itrhPath);	
										if(((((Vertex*)LinkhPath->getDst())->m_pOXCNode==((Vertex*)pLink->getSrc())->m_pOXCNode)||
											(((Vertex*)LinkhPath->getSrc())->m_pOXCNode==((Vertex*)pLink->getSrc())->m_pOXCNode))&&
											LinkhPath->m_nChannel!=-1&& pLink->m_nChannel!=-1)
											//(((Vertex*)LinkhPath->getDst())->m_pOXCNode==((Vertex*)pLink->getDst())->m_pOXCNode)
											//||
											//(((Vertex*)LinkhPath->getSrc())->m_pOXCNode==((Vertex*)pLink->getDst())->m_pOXCNode))
										{
											redundant=true;
											break;
										}
									...
									#ifdef DEBUGLA
									cout << "\n before checking if plink was redundant"<<endl;
								#endif
									if(!redundant&&((Vertex*)pLink->m_pSrc)->m_pOXCNode!=((Vertex*)pDst)->m_pOXCNode){
										#ifdef DEBUGLA
									cout << "\n plink is redundant"<<endl;
									#endif
										DstDone=true;
										hPath.clear();
										pPrevLink=pLink;
				...
				/*	if(!done&&!DstDone){
						#ifdef DEBUGLA
									cout << "\n before checking all the nodes in nodelist"<<endl;
								#endif
						for(itrNode=m_hNodeList.begin();itrNode!=m_hNodeList.end();itrNode++){
								CandVertex=(Vertex*)((AbstractNode*)(*itrNode));
								if(CandVertex->m_pOXCNode->getId()>(((Vertex*)pDst)->m_pOXCNode)->getId())
									break;
								if(CandVertex->m_pPrevLink){
									if(((CandVertex->m_nChannel==WLSelected && CandVertex->m_eVType==Vertex::VT_Channel_In)|| (CandVertex->m_nChannel==-1 && CandVertex->m_eVType==Vertex::VT_Access_In && ((SimplexLink*)CandVertex->m_pPrevLink)->m_eSimplexLinkType==SimplexLink::LT_Lightpath))
										&& CandVertex->m_pOXCNode==((Vertex*)pDst)->m_pOXCNode ){
										pLink=(SimplexLink*)CandVertex->m_pPrevLink;
										if(find(TriedPaths.begin(),TriedPaths.end(),pLink)==TriedPaths.end()&& find(hPath.begin(),hPath.end(),pLink)==hPath.end()
											&& pLink->m_pSrc->m_hCost<=pLink->m_pDst->m_hCost){
											//break;
											for(itr=hPath.begin();itr!=hPath.end();itr++){
												SimplexLink* pLinkPath = (SimplexLink*)(*itr);
												if(find(TriedPaths.begin(),TriedPaths.end(),pLinkPath)==TriedPaths.end())
													TriedPaths.insert(TriedPaths.end(),pLinkPath);
											}
		...
		//	}
						//LA:makes it wrong when costice==true or cost<UNREACHABLE->commented
						/*else
							goto Updating;*/

				
					/*if(((SimplexLink*)pPrevLink)->m_eSimplexLinkType==SimplexLink::LT_Channel){
						if(((Vertex*)(pPrevLink->m_pDst))->m_pOXCNode->getId()==((Vertex*)(pLink->m_pDst))->m_pOXCNode->getId()&&
							((Vertex*)(pPrevLink->m_pSrc))->m_pOXCNode->getId()==((Vertex*)(pLink->m_pSrc))->m_pOXCNode->getId()&&
							pPrevLink->m_bValid && WLSelected!=-1)
						{*/
							
							/*if(pLink->m_nChannel==WLSelected)
							{*/	
								
								#ifdef DEBUGLA
									cout << "\n found the link"<<endl;
								#endif
								//LA:check if the cost of src in plink is greater than the cost of dst should choose another prevlink
								/*	if(pLink->m_pSrc->m_hCost>pLink->m_pDst->m_hCost){
										//LA:renmove prevlink from hpath and choose another link
										#ifdef DEBUGLA
									cout << "\n before while for removing links in hpath with hpath size:"<<hPath.size()<<endl;
								#endif
										if(hPath.size()==0)
										{
											if(find(TriedPaths.begin(),TriedPaths.end(),pLink)==TriedPaths.end())
										...
										goto Checkcost;
											...
											while(hPath.front()!=OldPrevLink){
												if(find(TriedPaths.begin(),TriedPaths.end(),hPath.front())==TriedPaths.end())
													TriedPaths.insert(TriedPaths.end(),hPath.front());
												hPath.pop_front();
											}
											pPrevLink=OldPrevLink;
											if(find(TriedPaths.begin(),TriedPaths.end(),pLink)==TriedPaths.end())
												TriedPaths.insert(TriedPaths.end(),pLink);
											goto Checkcost;	
											}
									...
									if(find(hPath.begin(),hPath.end(),pLink)==hPath.end() && pLink->m_nChannel!=-1){
									bool redundant=false;
									for(itrhPath=hPath.begin();itrhPath!=hPath.end();itrhPath++){
										SimplexLink* LinkhPath= (SimplexLink*)(*itrhPath);	
										if((((((Vertex*)LinkhPath->getDst())->m_pOXCNode==((Vertex*)pLink->getSrc())->m_pOXCNode)||
											(((Vertex*)LinkhPath->getSrc())->m_pOXCNode==((Vertex*)pLink->getSrc())->m_pOXCNode)) )&& LinkhPath->m_nChannel!=-1 /*
											(((Vertex*)LinkhPath->getDst())->m_pOXCNode==((Vertex*)pPrevLink->getDst())->m_pOXCNode)||
											||
											(((Vertex*)LinkhPath->getSrc())->m_pOXCNode==((Vertex*)pPrevLink->getDst())->m_pOXCNode)*/){
											redundant=true;
											break;
										}
									...
									#ifdef DEBUGLA
									cout << "\n checking for redundant links"<<endl;
								#endif
									if(!redundant&&((Vertex*)pLink->m_pSrc)->m_pOXCNode!=((Vertex*)pDst)->m_pOXCNode){
										hPath.push_front(pLink);
										OldPrevLink=pLink;
										pPrevLink=pLink;
										Updated=true;
									}
										...
										if(hPath.size()!=0){
											if(((Vertex*)hPath.back()->getDst())->m_pOXCNode==((Vertex*)pDst)->m_pOXCNode&&((Vertex*)hPath.front()->getSrc())->m_pOXCNode==((Vertex*)pSrc)->m_pOXCNode)
												return pDst->m_hCost;
										}
										if(find(TriedPaths.begin(),TriedPaths.end(),pLink)==TriedPaths.end()){
											if(pLink->m_nChannel==WLSelected)
									...
									if(((Vertex*)hPath.back()->getDst())->m_pOXCNode==((Vertex*)pDst)->m_pOXCNode&&((Vertex*)hPath.front()->getSrc())->m_pOXCNode==((Vertex*)pSrc)->m_pOXCNode)
										return pDst->m_hCost;
											...
											/*for(itr=hPath.begin();itr!=hPath.end();itr++){
												SimplexLink* LinkhPath= (SimplexLink*)(*itr);
												if(LinkhPath->m_nChannel==WLSelected){*/
											//pPrevLink=hPath.front();
													//break;
												//}
					...
					//else 
					//	if(((SimplexLink*)pPrevLink)->m_eSimplexLinkType==SimplexLink::LT_Lightpath){
					//		AbstractNode* TempSrc=(((SimplexLink*)pPrevLink)->m_pLightpath->m_hPRoute.back())->m_pSrc;
					//		AbstractNode* TempDst=(((SimplexLink*)pPrevLink)->m_pLightpath->m_hPRoute.back())->m_pDst;
					//		if(TempDst->getId()==((Vertex*)(pLink->m_pDst))->m_pOXCNode->getId()&&
					//			TempSrc->getId()==((Vertex*)(pLink->m_pSrc))->m_pOXCNode->getId()&&pPrevLink->m_bValid && WLSelected!=-1)
					//		{
					//			
					//			/*if(pLink->m_nChannel==WLSelected)
					//			{*/	
					...
					//			}
					//			else 
					//				if(((Vertex*)hPath.back()->getDst())->m_pOXCNode==((Vertex*)pDst)->m_pOXCNode)
					//					return pDst->m_hCost;
					//					else
					//						goto Updating;
					//				break;
				...
				//}
					//LA:selected wl is not found free on one of the links: need to find another low cost wl
Updating:				if(!Updated){
					#ifdef DEBUGLA
						cout << "\n !updated"<<endl;
					#endif
							int minCostWl=UNREACHABLE;
							int SelectedIndex=-1;
							AbstractNode* DstOXC=((Vertex*)pDst)->m_pOXCNode;
							if(WlsTried.size()==NumWL)
								goto CheckhPath;
							if(find(WlsTried.begin(),WlsTried.end(),WLSelected)==WlsTried.end())
								WlsTried.push_back(WLSelected);
							for (int i=0;i<DstOXC->costi.size();i++)
							{
							/*	#ifdef DEBUGLA
									cout << "\n costi cycle"<<endl;
								#endif*/
								if(WlsTried.size()!=0 && find(WlsTried.begin(),WlsTried.end(),i)!=WlsTried.end())
									continue;
								if(DstOXC->costi[i]<minCostWl){
									minCostWl=DstOXC->costi[i];
									SelectedIndex=i;
							...
							if(SelectedIndex!=-1){
								if(find(WlsTried.begin(),WlsTried.end(),SelectedIndex)==WlsTried.end())
									WlsTried.push_back(SelectedIndex);
								WLSelected=SelectedIndex;

						#ifdef DEBUGLA
							cout << "\n Did not find any suitable wl so need to change wl of first link: "<<endl;
						#endif
							
						for(itr=hPath.begin();itr!=hPath.end();itr++){
							SimplexLink* pLinkPath = (SimplexLink*)(*itr);
							if(find(TriedPaths.begin(),TriedPaths.end(),pLinkPath)==TriedPaths.end())
									TriedPaths.push_back(pLinkPath);
						}
								hPath.clear();
								//LA:find a new prevlink for mdst
								//for(int j=0;j<((Vertex*)FirstPrevLink->m_pDst)->m_pOXCNode->prev.size();j++)
								
								/*for (int j=0;j<((Vertex*)pDst)->m_pOXCNode->prev.size();j++)
								{*/
								SimplexLink* pLink;
									/*if(((Vertex*)pDst)->m_pOXCNode->costi[j]==UNREACHABLE)
										continue;
									if(WLSelected==j)*/
									if(((Vertex*)pDst)->m_pOXCNode->costi[WLSelected]<UNREACHABLE)					 
										pLink=(SimplexLink*)((((Vertex*)pDst))->m_pOXCNode->prev[WLSelected]);
									else 
										goto Updating;
										//continue;
									/*#ifdef DEBUGLA
										cout << "\n in the for cycle to find an incoming link and select a wl"<<endl;
									#endif*/
									//SimplexLink* pLink=(SimplexLink*)(*itr);
									//LA:check if there is another lightpath with the same src on another wl 
									if(pLink->m_bValid /*&& pLink->m_pSrc->m_hCost<=pLink->m_pDst->m_hCost
										((Vertex*)pLink->m_pSrc)->m_pOXCNode==((Vertex*)FirstPrevLink->m_pSrc)->m_pOXCNode &&*/ ){
									/*	#ifdef DEBUGLA
											cout << "\n searching among SimplexLinks:"<<endl;
										#endif*/
										OldPrevLink=pLink;
										pPrevLink=pLink;
										//break;->wrong
								...
								/*	else
										goto Checkcost;*/
									//LA:if it is a lightpath check the physical path and check with other incoming links to dst
									//if(pLink->m_eSimplexLinkType==SimplexLink::LT_Lightpath && PrevPhys.size()!=0){
									//	if(/*(PrevPhys.back())->m_pSrc==((Vertex*)pLink->m_pSrc)->m_pOXCNode &&*/ PrevPhys.back()->m_bValid )
									//	{
									//		#ifdef DEBUGLA
									//			cout << "\n searching on physical path of lightpath:"<<endl;
									//		#endif
									//		OldPrevLink=pLink;
									//		pPrevLink=pLink;
									//		//break;->wrong
							...
							//LA:if no wl is found it should return unreachable
							//else{
							//	
							//	if( WlsTried.size()< NumWL){
							//		for(int k=0;k<NumWL;k++){
							//			#ifdef DEBUGLA
							//				cout << "\n Finding new wavelength"<<endl;
							//			#endif
							//			SelectedIndex=k;
							//			if(find(WlsTried.begin(),WlsTried.end(),SelectedIndex)==WlsTried.end()){
							//				WlsTried.push_back(SelectedIndex);
							...
							//	}
									...
									#ifdef DEBUGLA
								cout << "\n no suitable wl found"<<endl;
								#endif
CheckhPath:							for(itr=hPath.begin();itr!=hPath.end();itr++){
										SimplexLink* pLinkPath = (SimplexLink*)(*itr);
									if(find(TriedPaths.begin(),TriedPaths.end(),pLinkPath)==TriedPaths.end())
										TriedPaths.push_back(pLinkPath);
									}
								hPath.clear();
								for(int l=0;l<NumWL;l++){
									for(itr=pDst->m_hILinkList.begin();itr!=pDst->m_hILinkList.end();itr++){
										pLink = (SimplexLink*)(*itr);
										if((find(TriedPaths.begin(),TriedPaths.end(),pLink)==TriedPaths.end()&&pLink->m_bValid&&pLink->m_nChannel==l
											&& pLink->m_pSrc->m_hCost<=pLink->m_pDst->m_hCost)||( pLink==FirstPrevLink))
										{
											if(find(WlsChecked.begin(),WlsChecked.end(),l)==WlsChecked.end()){
												WlsChecked.push_back(l);
												OldPrevLink=pLink;
												pPrevLink=pLink;
												WLSelected=l;
												goto Begin;							
								...
								if(CandVertex->m_pOXCNode->getId()>(((Vertex*)pDst)->m_pOXCNode)->getId())
									break;
									if(CandVertex->m_pPrevLink){
										if(((CandVertex->m_nChannel==l && CandVertex->m_eVType==Vertex::VT_Channel_In)|| (CandVertex->m_nChannel==-1 && CandVertex->m_eVType==Vertex::VT_Access_In && ((SimplexLink*)CandVertex->m_pPrevLink)->m_eSimplexLinkType==SimplexLink::LT_Lightpath))
										&& CandVertex->m_pOXCNode==((Vertex*)pDst)->m_pOXCNode )
										pLink=(SimplexLink*)CandVertex->m_pPrevLink;
										if((find(TriedPaths.begin(),TriedPaths.end(),pLink)==TriedPaths.end()&&pLink->m_bValid&&pLink->m_nChannel==l
											&& ((Vertex*)pLink->m_pSrc)->m_pOXCNode->costi[WLSelected]<=((Vertex*)pLink->m_pSrc)->m_pOXCNode->costi[WLSelected]) &&pLink->m_pSrc->m_hCost<=pLink->m_pDst->m_hCost
											||( pLink==FirstPrevLink))
										{
											if(find(WlsChecked.begin(),WlsChecked.end(),l)==WlsChecked.end()){
												WlsChecked.push_back(l);
	...
	//-B: we already have computed the cost in DijkstraHelper method, so it simply returns it
	if(hPath.size()==0 && pDst->m_hCost!=0)
		return UNREACHABLE;
	else
	if(hPath.size()!=0 &&( ((Vertex*)hPath.back()->getDst())->m_pOXCNode!=((Vertex*)pDst)->m_pOXCNode||((Vertex*)hPath.front()->getSrc())->m_pOXCNode!=((Vertex*)pSrc)->m_pOXCNode)){
			if(!pPrevLink && WlsTried.size()< NumWL){
				Updated=false;
				goto Updating;
			}
			else{
...
//LA: does not work properly because of lightpath section
// LINK_COST AbstractGraph::recordMinCostPathWP(list<AbstractLink*>& hPath, 
//												  AbstractNode *pDst,AbstractNode *pSrc)
//{
//	bool First=true;
...
//					if(((Vertex*)(pPrevLink->m_pDst))->m_pOXCNode->getId()==((Vertex*)(pLink->m_pDst))->m_pOXCNode->getId()&&
//						((Vertex*)(pPrevLink->m_pSrc))->m_pOXCNode->getId()==((Vertex*)(pLink->m_pSrc))->m_pOXCNode->getId()&&
//						pLink->m_nChannel==WLSelected && WLSelected!=-1)
//					{
//						pPrevLink=pLink;
//						if(pPrevLink->m_bValid)
//						{	
...
//									SimplexLink* pLink=(SimplexLink*)(*itr);
//									//LA:check if there is another lightpath with the same src on another wl 
//									if(((Vertex*)pLink->m_pSrc)->m_pOXCNode==((Vertex*)FirstPrevLink->m_pSrc)->m_pOXCNode && pLink->m_bValid){
//										#ifdef DEBUGLA
//											cout << "\n searching among SimplexLinks:"<<endl;
//										#endif
//										pPrevLink=pLink;
...
LINK_COST AbstractGraph::DijkstraWP(list<AbstractLink*>& hMinCostPath, 
			AbstractNode* pSrc, AbstractNode* pDst, LinkCostFunction hLCF)
{
	DijkstraHelperWP(pSrc, pDst, hLCF); //FABIO 4 dic: modifico 

	// record minimum-cost path
	#ifdef DEBUGLA
	cout<<"\Source is:"<<((Vertex*)pSrc)->m_pOXCNode->getId();
				#endif
	return recordMinCostPathWP(hMinCostPath, pDst,pSrc);
}
...
//LINK_COST AbstractGraph::DijkstraDVNF(list<AbstractLink*>& hMinCostPath, 
//			AbstractNode* pSrc, AbstractNode* pDst, LinkCostFunction hLCF,NetMan* Net )
//{
//	DijkstraHelperDVNF(Net,pSrc, pDst, hLCF); //FABIO 4 dic: modifico 
//
//	// record minimum-cost path
//	return recordMinCostPath(hMinCostPath, pDst);
//}
										  ...
										  LinkCostFunction hLCF)
{
	...
	{
#ifdef DEBUGB
		cout << "\tvertex src and vertex dst are on this node -> Found!" << endl;
#endif // DEBUGB
		pDst->m_hCost = 0;
		return;
	}
	...
	while (!hPQ.empty())
	{
		  ...
		  {
#ifdef DEBUGB
			  cout << "\tWe reached to the destination:--> Path Found" << endl;
#endif // DEBUGB
			  BottelNeck=pLinkSrc;
			  return; // FABIO 4 dic
		  }
		...
		if(BeforeBNeck.size()==0)
			BeforeBNeck.push_back(pLinkSrc);

		else{
		  if(((Vertex*)BeforeBNeck.back())->m_pOXCNode!=((Vertex*)pLinkSrc)->m_pOXCNode)
			  BeforeBNeck.push_back(pLinkSrc);
		}
...
//			}
 ...
 inline void AbstractGraph::DijkstraHelperWP(AbstractNode* pSrc, AbstractNode* pDst,
																  LinkCostFunction hLCF)
{
#ifdef DEBUGB
	cout << "-> DijkstraHelperWP" << endl;
#endif // DEBUGB
	...
	//-B: source node has zero reachability cost
	pSrc->m_hCost = 0;
	((Vertex*)pSrc)->m_pOXCNode->costi=valarray<LINK_COST> (Zero,NumWL);
	((Vertex*)pSrc)->m_pOXCNode->prev = vector<AbstractLink*>(NumWL);
	BeforeBNeck.clear();
	BottelNeck=NULL;
	
	//LA:in order not to make a problem when the link is not a unifiber and does not have m_hchannel->does not work well
	int index=-1;
	...
	while (!hPQ.empty())
	{
		  ...
		  hPQ.popMin();
		  //-B: if this vertex is the destination node, we have found what we are looking for, so return
		//LA:does not find all the path to dst in wp case
		  /*
		  if (pLinkSrc == pDst)
		  {
#ifdef DEBUGB
		...
		if(BeforeBNeck.size()==0)
		  ...
		  for (itr = pLinkSrc->m_hOLinkList.begin(); itr != pLinkSrc->m_hOLinkList.end(); itr++) 
		  {
				...
				if ((!(*itr)->valid()) || (UNREACHABLE == (*itr)->getCost()))
					continue;	
				/*LA: shouldnt be here
				(*itr)->m_pDst->prev = vector<AbstractLink*>(this->numberOfChannels);
				(*itr)->m_pDst->costi = valarray<LINK_COST> (UNREACHABLE,numberOfChannels);*/
...
//			}
				...
				//if(hLinkCost>0){	
					index=-1;
				 //LA: made all prev and costi equal
					/*index=((Vertex*)(*itr)->m_pDst)->m_nChannel;
					if(index==-1){*/
					for(int i=0;i<((Vertex*)(*itr)->m_pDst)->m_pOXCNode->costi.size();i++)
					{
						if(((SimplexLink*)(*itr))->m_nChannel==i){
							index=i;
							break;
						}
					}
					//}
					//LA:update oxc array cost
					if(index!=-1 &&(((Vertex*)(*itr)->m_pDst)->m_pOXCNode->costi[index]>=(pLinkSrc->m_hCost + hLinkCost)) )
					{

						((Vertex*)(*itr)->m_pDst)->m_pOXCNode->costi[index]=pLinkSrc->m_hCost + hLinkCost;
						((Vertex*)(*itr)->m_pDst)->m_pOXCNode->prev[index]=*itr;
				...
				//double c1 = (*itr)->m_pDst->m_hCost;
					...
					//-B: if destination node's cost > reachability cost
					if (c1 > c2 )
						/*if(((((Vertex*)(*itr)->m_pSrc)->m_pOXCNode->getId()!=pNode2->m_pOXCNode->getId()&&((Vertex*)(*itr)->m_pSrc)->m_pOXCNode->getId()!=((Vertex*)(*itr)->m_pDst)->m_pOXCNode->getId()))||
							(((Vertex*)(*itr)->m_pSrc)->m_pOXCNode->getId()==((Vertex*)(*itr)->m_pDst)->m_pOXCNode->getId()))*/
					{
					//-B: set destination node's cost = reachability cost
					(*itr)->m_pDst->m_hCost = pLinkSrc->m_hCost + hLinkCost;
					...
					//(*itr)->m_pDst->m_pPrevWL=((SimplexLink*)(*itr))->m_nChannel; 
					/*if((*itr)->m_pDst->m_hCost<(*itr)->m_pSrc->m_hCost)
						cin.get();*/
...
bool AbstractGraph:: DVNF_updateLinkBW( LINK_CAPACITY bw, ServiceChain* SC)
{
	int counter=0;
	int WlSelected=-1;
	list<AbstractLink*>::const_iterator itr;
	...
	vector<int> FreeWl;
	Link=SC->SCPath.back();
	//LA:scan all the wavelength of first link and put the free one in a vector
	for(int i=0;i<Link->abs_m_nW;i++){
		if(Link->abs_m_pChannel[i].m_nCapacity>=bw)
			FreeWl.push_back(i);
	}
	
	//LA:check if there is a wl that is free in all the links 
	for(int j=0;j<FreeWl.size();j++){
		WlSelected=FreeWl[j];
		for (itr=SC->SCPath.begin(); itr!=SC->SCPath.end(); itr++){
			if ((*itr)->abs_m_pChannel[WlSelected].m_nCapacity>=bw)
				counter++;
		}

	if(counter==SC->SCPath.size()){
		for (itr=SC->SCPath.begin(); itr!=SC->SCPath.end(); itr++){
				(*itr)->abs_m_pChannel[WlSelected].consumeCh((*itr),WlSelected,bw);
				(*itr)->m_linkCap = (*itr)->m_linkCap - bw;
				if((*itr)->m_linkCap<0)
					cin.get();
		}
		
		SC->Wavelength=WlSelected;
		return true;
		}
	counter=0;
	}
...
void AbstractGraph::updateLinkBW(LINK_CAPACITY bw)
	{
		//cout<<bw<<endl;
		list<AbstractLink*>::const_iterator itr;
		for (itr=m_hLinkList.begin(); itr!=m_hLinkList.end(); itr++)
		{
			...
			//cout<<(*itr)->m_hBand<<"\n";
			(*itr)->m_linkCap = (*itr)->m_linkCap - bw;
			(*itr)->usedLink=0;  //così mi da pb=0,6 cmq blocked>prov, se invece commento qsr riga fa tutte prov
			//cout<<(*itr)->m_nLinkId<<"-";
			//cout<<(*itr)->m_hBand<<"\n";
			}
			//if ((*itr)->m_hBand == 0)
...
void AbstractGraph::DVNF_deprovLinkBW(LINK_CAPACITY bw,ServiceChain* SC)
{
		list<AbstractLink*>::const_iterator itr;
		for (itr=SC->SCPath.begin(); itr!=SC->SCPath.end(); itr++){
			if((*itr)->abs_m_pChannel[SC->Wavelength].m_nCapacity+bw<=CHANNEL_CAPACITY)
				(*itr)->abs_m_pChannel[SC->Wavelength].m_nCapacity=(*itr)->abs_m_pChannel[SC->Wavelength].m_nCapacity+bw;
			else 
				cin.get();
				if((*itr)->m_linkCap + bw<=AB_LINK_CAPACITY)
					(*itr)->m_linkCap = (*itr)->m_linkCap + bw;
				else
					cin.get();
		}
...
void AbstractGraph::deprovLinkBW(LINK_CAPACITY bw, int linkIDlist[])
{
	//cout<<"Deprov"<<endl;
	//cout<<bw<<endl;
	list<AbstractLink*>::const_iterator itr;
	...
	for (itr=m_hLinkList.begin(); itr!=m_hLinkList.end(); itr++)
	{
		...
		for (i=0; i<10; i++)
		{
			if ((*itr)->m_nLinkId == linkIDlist[i]){
				//cout<<(*itr)->m_nLinkId<<endl;
				//cout<<(*itr)->m_hBand<<endl;
				(*itr)->m_linkCap = (*itr)->m_linkCap + bw;
				//cout<<(*itr)->m_nLinkId<<"-";
				//cout<<(*itr)->m_hBand<<"\n";
			}
			//(*itr)->m_hBand = BANDLINK;
			//i++;
		}
		//cout<<(*itr)->m_nLinkId<<"-";
		//cout<<(*itr)->m_hBand<<"\n";
...
//LA:function used for the case shared path protection (conflic set is used to identify sharing potential between backup paths)
