...
public:
	...
	//LA:added for the lighter version of DVNF
	void setBetweennessCentrality();
	...
	void extractPotentialSourcesMC_Nconn(bool, vector<UINT>&, double, double);
	...
	UniFiber* addUniFiberDCTopo(UINT, UINT, UINT, UINT, LINK_COST, UINT, float);	//LA:added to support DC topology with link capacity																		//	Each UniFiber object has a Channel array, whose size is the num of wavel/channels
	...
	void setChannelCapacity(double);												//-B: simply set the related var to the value passed as parameter
...
//LA:
	UINT				NumberOfWavelength;
	//LINK_COST DVNF_ShortestPath(list<AbstractLink*>&, UINT, UINT,Graph&m_hGraph);
