	...
	switch (m_eStatActive){
		...
		{												//ALMENO 300000 connessioni..
			...
			if((p_block->isconfsatisfied(stop_perc , conf) == true /*&& (connection >= 300000)*/) || (p_block->mean() == 0.0 && (connection) == 1000000))   //tolto per evitare che una volta raggiunta la confidenza si fermi la simulazione
			{
				...
				okstat = true;
				cout << "******************************* CONFIDENZA SODDISFATTA!!!" << endl << endl;
				//cin.get();
			}
			break;
		}
...
/*LA:commented as I need channel capacity to be double
...
/*LA:commented as i need capacity to be double
...
UINT NetMan::placeBBUClose(UINT src, vector<OXCNode*>&BBUsList)
{
	...
	for (int j = 0; j < BBUsList.size(); j++)
	{
		...
		{
			float lat ;
			//= computeLatency(pOXCdst->pPath,pCon);
			if (lat <= LATENCYBUDGET) //-B: -> should be unnecessary using DijkstraLatency -> IT IS ABSOLUTELY NECESSARY! (read at the end of DijkstraHelperLatency)
			{		
...
UINT NetMan::placeBBU_Metric(UINT src, vector<OXCNode*>&BBUsList)
{
	...
	for (itr = BBUsList.begin(); itr != BBUsList.end(); itr++)
	{
		...
		{
			float lat;
			//= computeLatency(pNode->pPath,pCon);
			if (lat <= LATENCYBUDGET) //-B: -> should be unnecessary using DijkstraLatency -> IT IS ABSOLUTELY NECESSARY! (read at the end of DijkstraHelperLatency)
			{
				//PoP node as BBU hotel node should be preferred
				//	(if not, we could have strange situation in which the connection
...
//LA:new function that takes as input also graph for reprovisioning version
void NS_OCH::NetMan::invalidateSimplexLinkDueToCapOrStatus(Graph* m_bkGraph,double bwd)
{
	list<AbstractLink*>::const_iterator itr, itrL;
	SimplexLink*pLink;
	UniFiber*pUniFiber;
	...
	for (itr = m_bkGraph->m_hLinkList.begin(); itr != m_bkGraph->m_hLinkList.end(); itr++)
	{
		pLink = (SimplexLink*)(*itr);
...
float NetMan::computeLatency(list<AbstractLink*>&path, Connection* pCon)
{
#ifdef DEBUGB
	cout << "-> computeLatency" << endl;
#endif // DEBUGB
	...
	Vertex *VSrc,*VDst;
	int counter=0;
	bool needsGrooming=false;
	list<Connection*>::const_iterator itrc;
	Connection* Conn;
	bool LatencyViolated=false;
	int auxval=0; //for the sake of if
	//-B: scan every simplex link of path
	while (itr != path.end())
	{
		SimplexLink *pLink = (SimplexLink*)(*itr);
		...
		VSrc = (Vertex*)pLink->getSrc();
		VDst = (Vertex*)pLink->getDst();
		...
		switch (pLink->m_eSimplexLinkType)
		{
			...
			if(VSrc->m_pOXCNode->m_NFVnode && counter==0){
				if(find(pCon->m_SC->SCnodes.begin(),pCon->m_SC->SCnodes.end(),VSrc->m_pOXCNode)!=pCon->m_SC->SCnodes.end())
				/*	latency += (float)FEC + (float) OEO;
				else*/

				//LA:not considering context sw
									auxval++;
			/*	latency += (float)FEC + (float) OEO + (float)((VSrc->m_pOXCNode->VNF.size())* CONTEXTSW);*/
				
			//LA:if more than 1 cpu core is used we consider context sw latency
				/*if(VCPU-(VSrc->m_pOXCNode->CPURes)>1)
					latency +=CONTEXTSW;*/
			}
			if(VDst->m_pOXCNode->m_NFVnode){
				if(find(pCon->m_SC->SCnodes.begin(),pCon->m_SC->SCnodes.end(),VDst->m_pOXCNode)!=pCon->m_SC->SCnodes.end())
				/*	latency += (float)FEC + (float) OEO;
				else*/
				
					//LA:not considering context sw
							auxval++;
					/*latency += (float)FEC + (float) OEO + (float)((VDst->m_pOXCNode->VNF.size())* CONTEXTSW);*/
				//LA:if more than 1 cpu core is used we consider context sw latency
				/*if(VCPU-(VDst->m_pOXCNode->CPURes)>1)
					latency +=CONTEXTSW;*/
			}
				counter++;
				break;
			...
			//LA:src node is not nfvnode so needs to consider oeo
			else{
				if (!VSrc->m_pOXCNode->m_NFVnode)
					if(needsGrooming & !pLink->m_pLightpath->GroomLatUpdated ){
						for (itrc =pLink->m_pLightpath->m_ConsOnLP.begin(); itrc !=pLink->m_pLightpath->m_ConsOnLP.end(); itrc++){
							...
							//if(Conn!=pCon){
								if(Conn->m_ConLat >Conn->m_SC->latency)
									LatencyViolated=true;
								Conn->m_ConLat+=(float) ELSWITCHLATENCY;
							//Conn->m_ConLat+=(float) OEO;
							//LA:check if latency will be violated now or not if yes log it 
							if(Conn->m_ConLat>Conn->m_SC->latency && !LatencyViolated)
								this->m_hLog.m_Latency_Violated++;
							//}
						}
					pLink->m_pLightpath->GroomLatUpdated=true;
					}
		...
		case SimplexLink::LT_Grooming:
			{
					//LA:if it is NFV-node we've once calculated OEO
				/*if(!VDst->m_pOXCNode->m_NFVnode)
					latency += (float)OEO;*/
...
void NetMan::consumeBandwOnSimplexLink(Lightpath*pLightpath, double BWToBeAllocated)
{
	list<AbstractLink*>::const_iterator itr;
	SimplexLink*pLink;
	
	//LA:to compare two double value in case of voip service
	...
	for (itr = this->m_hGraph.m_hLinkList.begin(); itr != this->m_hGraph.m_hLinkList.end(); itr++)
	{
		...
		if (pLink->m_pLightpath == pLightpath)
		{
			...
			assert((pLightpath->getFreeCapacity() - pLink->m_hFreeCap<epsilon));
			break;
		}
	}
}
//LA:to support reprovision
void NetMan::consumeBandwOnSimplexLink(Lightpath*pLightpath, double BWToBeAllocated, bool tempReprov )
{
	list<AbstractLink*>::const_iterator itr;
	SimplexLink*pLink;
	...
	for (itr = this->m_BackupGraph->m_hLinkList.begin(); itr != this->m_BackupGraph->m_hLinkList.end(); itr++)
	{
		pLink = (SimplexLink*)(*itr);
		if (pLink->m_pLightpath == pLightpath)
		{
...
bool NetMan :: DVNF_ProvisionSC(Connection * pCon, Circuit * pPCircuit){
	...
	if(pCon->m_nSrc!=pCon->m_nDst && pCon->m_SC->SCPath.size()==0)
		success=false;
	/*if(success){
	...
	if(success)
		{
			...
			if(pCon->m_nSrc!=pCon->m_nDst){
				DVNF_NewCircuit(*pPCircuit,pCon->m_SC->SCPath,-1,pCon);
				//LA:consume Bw
...
bool NetMan :: DVNF_ProvisionSCHelper(Connection * pCon)
{
	...
	//LA:having all nfv nodes as destinations
		...
		for(int i=0;i<NFVCapnodes.size();i++)
		{
			...
			pVSrc = m_hGraph.lookUpVertex(pCon->m_nSrc, Vertex::VT_Access_Out, -1);
			pVDst = m_hGraph.lookUpVertex(NFVCapnodes[i]->getId(), Vertex::VT_Access_In, -1);
...
//LA:depending of Latency requirements of SC assign costs to links
		if(pCon->m_SC->LatencyClass=="Stringent")
		m_hGraph.preferWavelPath();
		else if(pCon->m_SC->LatencyClass=="Relaxed")
	...
	if (bSuccess){
		pCon->m_ConLat=computeLatency(pCon->m_SC->SCPath, pCon);
			if (pCon->m_ConLat<=pCon->m_SC->latency){
...
cout << "->DVNF_ProvisionVNF" << endl;
	...
	list<AbstractLink*>::const_iterator itr,itrnew,itrSC,ShortestPathLA ;
	vector <OXCNode*> K_shPNFVnodes ;
	UINT MinNumVNF=UNREACHABLE;
	bool NodeExist=false,provisioned=false,linkRedun=false, DstRed=false;
...
cout << "\nProvisioning VNF of type :" << VN->VNFName<< endl;
	...
	if(checkVar!=0){
						...
						for(itr=pCon->m_SC->SCPath.begin();itr!=pCon->m_SC->SCPath.end();itr++){
							SimplexLink *pLink = (SimplexLink*)(*itr);
							Vertex * VDstpath= (Vertex *) pLink->getDst();
							Vertex * VSrcpath= (Vertex *) pLink->getSrc();
							if(VSrcpath->m_pOXCNode->m_NFVnode && find(NodeOnPath.begin(),NodeOnPath.end(),VSrcpath->m_pOXCNode)==NodeOnPath.end())
								NodeOnPath.push_back(VSrcpath->m_pOXCNode);
							if(find(NodeOnPath.begin(),NodeOnPath.end(),VDstpath->m_pOXCNode)==NodeOnPath.end() && VDstpath->m_pOXCNode->m_NFVnode)
								NodeOnPath.push_back(VDstpath->m_pOXCNode);
						}
							...
							if(((NFVnodesCap[j]->CPURes))>=(VN->CpuUsage)*(pCon->m_SC->NumofUsers)){
								if(find(pCon->m_SC->SCnodes.begin(),pCon->m_SC->SCnodes.end(),NFVnodesCap[j])==pCon->m_SC->SCnodes.end())
									if(find(NodeOnPath.begin(),NodeOnPath.end(),NFVnodesCap[j])==NodeOnPath.end()){
										...
										pVDst = m_hGraph.lookUpVertex(NFVnodesCap[j]->getId(), Vertex::VT_Access_In, -1);
										NFVnodesCap[j]->hFromSrcPath.clear();
										NFVnodesCap[j]->NFVnSrcReachCost=m_hGraph.DijkstraWP(NFVnodesCap[j]->hFromSrcPath,pVSrc,pVDst,AbstractGraph::LCF_ByOriginalLinkCost);
										if(NFVnodesCap[j]->NFVnSrcReachCost==UNREACHABLE)
											continue;
										for(itrSC=pCon->m_SC->SCPath.begin();itrSC!=pCon->m_SC->SCPath.end();itrSC++){
											for(itr=NFVnodesCap[j]->hFromSrcPath.begin();itr!=NFVnodesCap[j]->hFromSrcPath.end();itr++){
												...
												if((VSrcpath->m_pOXCNode==((Vertex*)pLinkSC->getSrc())->m_pOXCNode && VDstpath->m_pOXCNode==((Vertex*)pLinkSC->getDst())->m_pOXCNode&&VDstpath->m_pOXCNode!=VSrcpath->m_pOXCNode)||
													(VSrcpath->m_pOXCNode==((Vertex*)pLinkSC->getDst())->m_pOXCNode && VDstpath->m_pOXCNode==((Vertex*)pLinkSC->getSrc())->m_pOXCNode&&VDstpath->m_pOXCNode!=VSrcpath->m_pOXCNode))
													{
														linkRedun=true;
														break;
												}
										...
										if(!linkRedun && NFVnodesCap[j]->NFVnSrcReachCost!=UNREACHABLE)
										NFVnodes.push_back(NFVnodesCap[j]);
									}
								else{
									if(NFVnodesCap[j]->getId()==pCon->m_nDst && !linkRedun && NFVnodesCap[j]->NFVnSrcReachCost!=UNREACHABLE)
										NFVnodes.push_back(NFVnodesCap[j]);
								}
						}
								/*LA:not needed since I will need this later
								NFVnodesCap[j]->hFromSrcPath.clear();*/
						}
						...
						//LA: there is more than one already activated VNF instance  in the network with enough capacity
							...
							if(SrcID!=NFVnodes[j]->getId()){
								//LA: calculate the shortest path between this node and src -> redundant
								/*pVSrc = m_hGraph.lookUpVertex(SrcID, Vertex::VT_Access_Out, -1);
								pVDst = m_hGraph.lookUpVertex(NFVnodes[j]->getId(), Vertex::VT_Access_In, -1);
								//if(SrcID!=NFVnodes[j]->getId())
								NFVnodes[j]->hFromSrcPath.clear(); //LA:as each time src is changed i need to reset it otherwise path is just added
																	//to the previous one
								NFVnodes[j]->NFVnSrcReachCost= m_hGraph.Dijkstra(NFVnodes[j]->hFromSrcPath,pVSrc,pVDst,AbstractGraph::LCF_ByOriginalLinkCost);*/
							...
							pVSrc = m_hGraph.lookUpVertex(NFVnodes[j]->getId(), Vertex::VT_Access_Out, -1);
							pVDst = m_hGraph.lookUpVertex(pCon->m_nDst, Vertex::VT_Access_In, -1);
							NFVnodes[j]->htoDstPath.clear();
							NFVnodes[j]->NFVnDstReachCost=m_hGraph.DijkstraWP(NFVnodes[j]->htoDstPath,pVSrc,pVDst,AbstractGraph::LCF_ByOriginalLinkCost);
							...
							if(pCon->m_SC->SCnodes.size()!=0){
								for (itr=NFVnodes[j]->htoDstPath.begin(); itr!=NFVnodes[j]->htoDstPath.end(); itr++){
									...
									Vertex * VDstpath= (Vertex *) pLink->getDst();
									Vertex * VSrcpath= (Vertex *) pLink->getSrc();
									if((find(pCon->m_SC->SCnodes.begin(),pCon->m_SC->SCnodes.end(),VDstpath->m_pOXCNode)!=pCon->m_SC->SCnodes.end())||(find(pCon->m_SC->SCnodes.begin(),pCon->m_SC->SCnodes.end(),VSrcpath->m_pOXCNode)!=pCon->m_SC->SCnodes.end())){
										NodeExist=true;
										break;
									}
								}
				...
				/*	if(candidatenode!=NULL){
						...
						for(itr=candidatenode->hFromSrcPath.begin();itr!=candidatenode->hFromSrcPath.end();itr++){
							SimplexLink *plink=(SimplexLink*)(*itr);
							Vertex * LinkDst= (Vertex*) plink->getDst();
							if (LinkDst->m_pOXCNode->getId()==pCon->m_nDst&& (m_hGraph.lookUpVertex(pCon->m_nDst, Vertex::VT_Access_In,-1)->m_pOXCNode->CPURes)>=(VN->CpuUsage)*(pCon->m_SC->NumofUsers)){
							//LA>choosing connection dst as candidatenode and provision VNF on that 
								//candidatenode=(OXCNode*)m_hWDMNet.m_hNodeList.find(pCon->m_nDst);
								candidatenode= m_hGraph.lookUpVertex(pCon->m_nDst, Vertex::VT_Access_In,-1)->m_pOXCNode;
								pVSrc =m_hGraph.lookUpVertex(SrcID, Vertex::VT_Access_Out, -1);
								pVDst=m_hGraph.lookUpVertex(candidatenode->getId(), Vertex::VT_Access_In,-1);
								candidatenode->hFromSrcPath.clear();
								//candidatenode->htoDstPath.clear();
								candidatenode->NFVnSrcReachCost=m_hGraph.Dijkstra(candidatenode->hFromSrcPath,pVSrc,pVDst,AbstractGraph::LCF_ByOriginalLinkCost);
								
								//LA:follow activating new instance of VNF procedure->results in more NFV-nodes 
							/*	candidatenode->hFromSrcPath.clear();
								candidatenode->htoDstPath.clear();
								status=DVNF_ActivateNewInst(VN,pCon);
								provisioned=true;*/
					...
					if(candidatenode!=NULL && candidatenode->getId()!=pCon->m_nDst){
						...
						if(pCon->m_SC->SCPath.size()!=0){
							for(itr=candidatenode->htoDstPath.begin();itr!=candidatenode->htoDstPath.end();itr++){
								...
								Vertex * LinkDst= (Vertex*) plink->getDst();
								for(itrSC=pCon->m_SC->SCPath.begin();itrSC!=pCon->m_SC->SCPath.end();itrSC++){									
									SimplexLink *plinkPath=(SimplexLink*)(*itrSC);
									if((LinkDst->m_pOXCNode->getId()==((Vertex*)plinkPath->getSrc())->m_pOXCNode->getId()&& ((Vertex*)plink->getSrc())->m_pOXCNode->getId()==((Vertex*)plinkPath->getDst())->m_pOXCNode->getId())||
										//LA:same link in scpath and htodst path
										(LinkDst->m_pOXCNode->getId()==((Vertex*)plinkPath->getDst())->m_pOXCNode->getId()&& ((Vertex*)plink->getSrc())->m_pOXCNode->getId()==((Vertex*)plinkPath->getSrc())->m_pOXCNode->getId()))
									{
										...
										candidatenode->hFromSrcPath.clear();
										candidatenode->htoDstPath.clear();
										candidatenode->NFVnSrcReachCost=m_hGraph.DijkstraWP(candidatenode->hFromSrcPath,pVSrc,pVDst,AbstractGraph::LCF_ByOriginalLinkCost);
										DstRed=true;
										break;
									}
						...
						//LA:check if the link causing trombone effect exists in hfromsrc and htodst of candidatenode
							for(itrSC=candidatenode->hFromSrcPath.begin();itrSC!=candidatenode->hFromSrcPath.end();itrSC++){
								...
								for(itr=candidatenode->htoDstPath.begin();itr!=candidatenode->htoDstPath.end();itr++){
									SimplexLink *plink=(SimplexLink*)(*itr);									
									if((plink->m_hFreeCap<=CHANNEL_CAPACITY && plinkPath->m_hFreeCap<=CHANNEL_CAPACITY && ((Vertex*)plink->getDst())->m_pOXCNode==((Vertex*)plinkPath->getSrc())->m_pOXCNode && ((Vertex*)plink->getSrc())->m_pOXCNode==((Vertex*)plinkPath->getDst())->m_pOXCNode)||
										//LA:if the link with the same source and dst exists in both hfromsrc and htodst
										(plink->m_hFreeCap<=CHANNEL_CAPACITY && plinkPath->m_hFreeCap<=CHANNEL_CAPACITY && ((Vertex*)plink->getDst())->m_pOXCNode==((Vertex*)plinkPath->getDst())->m_pOXCNode && ((Vertex*)plink->getSrc())->m_pOXCNode==((Vertex*)plinkPath->getSrc())->m_pOXCNode))									
									{
										//LA:choose Dst as NFV-node
										candidatenode= m_hGraph.lookUpVertex(pCon->m_nDst, Vertex::VT_Access_In,-1)->m_pOXCNode;
										pVSrc =m_hGraph.lookUpVertex(SrcID, Vertex::VT_Access_Out, -1);
							...
							if(!DstRed){
							//LA:Check hfromsrc and scpath
								for(itrSC=candidatenode->hFromSrcPath.begin();itrSC!=candidatenode->hFromSrcPath.end();itrSC++){
									SimplexLink *plinkPath=(SimplexLink*)(*itrSC);
									for(itr=pCon->m_SC->SCPath.begin();itr!=pCon->m_SC->SCPath.end();itr++){
										SimplexLink *plink=(SimplexLink*)(*itr);									
										if((plink->m_hFreeCap<=CHANNEL_CAPACITY && plinkPath->m_hFreeCap<=CHANNEL_CAPACITY&&((Vertex*)plink->getDst())->m_pOXCNode->getId()==((Vertex*)plinkPath->getSrc())->m_pOXCNode->getId()&& ((Vertex*)plink->getSrc())->m_pOXCNode->getId()==((Vertex*)plinkPath->getDst())->m_pOXCNode->getId())||
										(plink->m_hFreeCap<=CHANNEL_CAPACITY && plinkPath->m_hFreeCap<=CHANNEL_CAPACITY&&((Vertex*)plink->getDst())->m_pOXCNode->getId()==((Vertex*)plinkPath->getDst())->m_pOXCNode->getId()&& ((Vertex*)plink->getSrc())->m_pOXCNode->getId()==((Vertex*)plinkPath->getSrc())->m_pOXCNode->getId()))
										{
											//LA:choose Dst as NFV-node
											candidatenode= m_hGraph.lookUpVertex(pCon->m_nDst, Vertex::VT_Access_In,-1)->m_pOXCNode;
											pVSrc =m_hGraph.lookUpVertex(SrcID, Vertex::VT_Access_Out, -1);
				...
				//LA:if candidatenode is not dst yet 
						if(candidatenode->getId()!=pCon->m_nDst){
							...
							for(itrnew=candidatenode->hFromSrcPath.begin();itrnew!=candidatenode->hFromSrcPath.end();itrnew++){
								SimplexLink *plink=(SimplexLink*)(*itrnew);
								Vertex * LinkDst= (Vertex*) plink->getDst();
								if (LinkDst->m_pOXCNode->getId()==pCon->m_nDst&& (m_hGraph.lookUpVertex(pCon->m_nDst, Vertex::VT_Access_In,-1)->m_pOXCNode->CPURes)>=(VN->CpuUsage)*(pCon->m_SC->NumofUsers)){
							//LA>choosing connection dst as candidatenode and provision VNF on that 
								//candidatenode=(OXCNode*)m_hWDMNet.m_hNodeList.find(pCon->m_nDst);
...
bool NetMan:: DVNF_ActivateNewInst(VNF* VN,Connection * pCon)
{
	...
	pVSrc=m_hGraph.lookUpVertex(SrcID, Vertex::VT_Access_Out,-1);
	pVDst=m_hGraph.lookUpVertex(pCon->m_nDst, Vertex::VT_Access_In,-1);
	/*m_hGraph.YenWP(listaPath,((Vertex*)pVSrc)->m_pOXCNode,((Vertex*)pVDst)->m_pOXCNode,1,AbstractGraph::LCF_ByOriginalLinkCost);*/
	//m_hGraph.YenWP(listaPath,pVSrc,pVDst,1,AbstractGraph::LCF_ByOriginalLinkCost);
	//LA:disabled for using Yen-WP for wavelength continuity 
	//LA:using disjkstrawp will be very strict as when there is NFV node wl continuity is not enforced
	SP_Cost= m_hGraph.Dijkstra(ShortestPath,pVSrc,pVDst,AbstractGraph::LCF_ByOriginalLinkCost);
	...
	if(SP_Cost==UNREACHABLE){
		...
		OXCNode* VSrc = pVSrc->m_pOXCNode;	
		/*if(VSrc->m_NFVnode && VSrc->CPURes>=(VN->CpuUsage)*(SC->NumofUsers))
				SPnodes.push_back(VSrc);*/
		/*LA: YEN
		WPPath=listaPath.back();
		ShortestPath=WPPath->m_hLinkList;*/

		for (itrSP=ShortestPath.begin(); itrSP!=ShortestPath.end(); itrSP++){
			SimplexLink *pLinkSP = (SimplexLink*)(*itrSP);
			
			//LA:lighter version
			//OXCNode * VDst=(OXCNode*)pLink->m_pDst;

			Vertex* VDst=(Vertex*)pLinkSP->getDst();
		
			if(VDst->m_pOXCNode->m_NFVnode && (pLinkSP->m_hFreeCap<=CHANNEL_CAPACITY)){
				
				if((VDst->m_pOXCNode->CPURes)>=(VN->CpuUsage)*(pCon->m_SC->NumofUsers)){		//if it has enough capacity add it to the list otherwise no
					...
					//LA:to avoid using the nfv-nodes already on SCpath
						for(itrSC=pCon->m_SC->SCPath.begin();itrSC!=pCon->m_SC->SCPath.end();itrSC++){
							SimplexLink *pLinkSC = (SimplexLink*)(*itrSC);
							Vertex * VDstpath= (Vertex *) pLinkSC->getDst();
							Vertex * VSrcpath= (Vertex *) pLinkSC->getSrc();
							if(VSrcpath->m_pOXCNode->m_NFVnode)
								NodeOnPath.push_back(VSrcpath->m_pOXCNode);
							if(find(NodeOnPath.begin(),NodeOnPath.end(),VDstpath->m_pOXCNode)==NodeOnPath.end() && VDstpath->m_pOXCNode->m_NFVnode)
								NodeOnPath.push_back(VDstpath->m_pOXCNode);
						}
					...
					if(find(pCon->m_SC->SCnodes.begin(),pCon->m_SC->SCnodes.end(),VDst->m_pOXCNode)==pCon->m_SC->SCnodes.end() || VDst->m_pOXCNode->getId()==pCon->m_nDst)
						if(	find(SPnodes.begin(),SPnodes.end(),VDst->m_pOXCNode)==SPnodes.end() )	
							if(find(NodeOnPath.begin(),NodeOnPath.end(),VDst->m_pOXCNode)==NodeOnPath.end()){
								linkRedun=false;
							//	while(!linkRedun){		
								//LA:check if there is redundant link for reaching this node from con src
										pVSrc = m_hGraph.lookUpVertex(SrcID, Vertex::VT_Access_Out, -1);
										pVDst = m_hGraph.lookUpVertex(VDst->m_pOXCNode->getId(), Vertex::VT_Access_In, -1);
										VDst->m_pOXCNode->hFromSrcPath.clear();
										VDst->m_pOXCNode->NFVnSrcReachCost=m_hGraph.DijkstraWP(VDst->m_pOXCNode->hFromSrcPath,pVSrc,pVDst,AbstractGraph::LCF_ByOriginalLinkCost);
										for(itrSC=pCon->m_SC->SCPath.begin();itrSC!=pCon->m_SC->SCPath.end();itrSC++){
											for(itr=VDst->m_pOXCNode->hFromSrcPath.begin();itr!=VDst->m_pOXCNode->hFromSrcPath.end();itr++){
												SimplexLink *pLink = (SimplexLink*)(*itr);
												Vertex * VDstpath= (Vertex *) pLink->getDst();
												Vertex * VSrcpath= (Vertex *) pLink->getSrc();
												SimplexLink *pLinkSC = (SimplexLink*)(*itrSC);
					...
					#ifdef DEBUGLA
					cout<<"\nFound a NFV node on the shortest path with ID: "<<VDst->m_pOXCNode->getId()<<endl;
					#endif
				//	}
				//else
		...
		for(int k=0;k<SPnodes.size();k++)
		{
			...
			//if(SPnodes[k]->CPURes>=VN->CpuUsage){
				...
				if(pCon->m_SC->SCnodes.size()==0){
					...
					#endif
					/*LA:hfrom src is already calculated so commented it
					SPnodes[k]->hFromSrcPath.clear();
					pVSrc = m_hGraph.lookUpVertex(SrcID,Vertex::VT_Access_Out,-1);
					pVDst = m_hGraph.lookUpVertex(SPnodes[k]->getId(),Vertex::VT_Access_In,-1);
					SP_Cost= m_hGraph.DijkstraWP(SPnodes[k]->hFromSrcPath,pVSrc,pVDst,AbstractGraph::LCF_ByOriginalLinkCost);
				...
				/*	for (itr=SPnodes[k]->hFromSrcPath.begin(); itr!=SPnodes[k]->hFromSrcPath.end(); itr++){
						...
						if(SPnodes[k]->NFVnSrcReachCost==UNREACHABLE){
						continue;
					}
				
					//LA:calculate htoDSt
					SPnodes[k]->htoDstPath.clear();
					pVSrc = m_hGraph.lookUpVertex(SPnodes[k]->getId(),Vertex::VT_Access_Out,-1);
					pVDst = m_hGraph.lookUpVertex(pCon->m_nDst,Vertex::VT_Access_In,-1);
					SPnodes[k]->NFVnDstReachCost=m_hGraph.DijkstraWP(SPnodes[k]->htoDstPath,pVSrc,pVDst,AbstractGraph::LCF_ByOriginalLinkCost);
				...
				//	if(SC->SCnodes.back()!=SPnodes[k]){	 //LA: the node chosen for provisioning of vnf is not chosen before in provisioning of previous VNF
					...
					//LA:hfromsrc is already calculated-> commented ->needed actually otherwise the path will be empty
					...
					pVSrc = m_hGraph.lookUpVertex(SrcID,Vertex::VT_Access_Out,-1);
					pVDst = m_hGraph.lookUpVertex(NewVNFCand->getId(),Vertex::VT_Access_In,-1);
					SP_Cost= m_hGraph.DijkstraWP(NewVNFCand->hFromSrcPath,pVSrc,pVDst,AbstractGraph::LCF_ByOriginalLinkCost);
				...
				/*	for (itr=NewVNFCand->hFromSrcPath.begin(); itr!=NewVNFCand->hFromSrcPath.end(); itr++){
					...
					if(this->VNFCounter==(pCon->m_SC->LengthOfSCs)-1){
						//LA:htodst is already calculated->it is needed actually otherwise the path will be empty
						NewVNFCand->htoDstPath.clear();
						pVSrc = m_hGraph.lookUpVertex(NewVNFCand->getId(),Vertex::VT_Access_Out,-1);
						pVDst = m_hGraph.lookUpVertex(pCon->m_nDst,Vertex::VT_Access_In,-1);
						SP_Cost= m_hGraph.DijkstraWP(NewVNFCand->htoDstPath,pVSrc,pVDst,AbstractGraph::LCF_ByOriginalLinkCost);
				...
				for(int l=0;l<NFVCapnodes.size();l++){
					...
					if(/*NFVCapnodes[l]->hasCap&&*/(NFVCapnodes[l]->CPURes)>((VN->CpuUsage)*(pCon->m_SC->NumofUsers)))
						if(find(pCon->m_SC->SCnodes.begin(),pCon->m_SC->SCnodes.end(),NFVCapnodes[l])==pCon->m_SC->SCnodes.end() )
							...
							pVSrc = m_hGraph.lookUpVertex(SrcID,Vertex::VT_Access_Out,-1);
							pVDst = m_hGraph.lookUpVertex(NFVCapnodes[l]->getId(),Vertex::VT_Access_In,-1);
							NFVCapnodes[l]->NFVnReachCost= m_hGraph.DijkstraWP(NFVCapnodes[l]->hFromSrcPath,pVSrc,pVDst,AbstractGraph::LCF_ByOriginalLinkCost);
					...
					//LA:calculate the shortest path between this node and dst
							...
							pVSrc = m_hGraph.lookUpVertex(NFVCapnodes[l]->getId(),Vertex::VT_Access_Out,-1);
							pVDst = m_hGraph.lookUpVertex(pCon->m_nDst,Vertex::VT_Access_In,-1);
							NFVCapnodes[l]->NFVnReachCost+= m_hGraph.DijkstraWP(NFVCapnodes[l]->htoDstPath,pVSrc,pVDst,AbstractGraph::LCF_ByOriginalLinkCost);
...
inline void NetMan::DVNF_Deprovision(Connection *pCon)
{
   ...
   //LA:not needed for the lighter Version
	...
	//LA:if I am doing connection reprovision don't need to touch VNFs and SCnodes and SCpath 
	if(!pCon->Reprov){
		DVNF_Deprovision_Helper(pCon);
		//LA:for lighter version
	...
	//LA:remove the connection from list of connections on the lightpath for each lightpath of the connection route
	for (itr = pCon->m_pPCircuit->m_hRoute.begin(); itr != pCon->m_pPCircuit->m_hRoute.end(); itr++){
		(*itr)->m_ConsOnLP.remove(pCon);
	}
	...
	while (itr != hLinkList.end())
	{
		...
		switch (pLink->m_eSimplexLinkType)
		{
			...
			case SimplexLink::LT_UniFiber:
			{
				...
				//LA:add the connection to the list of connections on this lightpath
				pLightpath->m_ConsOnLP.push_front(pCon);
...
#endif // DEBUGB
				//LA:add the connection to the list of connections on this lightpath
				pLink->m_pLightpath->m_ConsOnLP.push_front(pCon);
				hCircuit.addVirtualLink(pLink->m_pLightpath);
				break;
			}
			case SimplexLink::LT_Grooming:
...
//LA:to update the links of backup graph
void NetMan::DVNF_NewCircuit(Circuit& hCircuit, const list<AbstractLink*>& hLinkList, UINT channel, Connection*pCon,Graph m_BackupGraph)
{
#ifdef DEBUGLA
	cout << "\n-> DVNF_NewCircuit" << endl;
#endif // DEBUGB
	...
	while (itr != hLinkList.end())
	{
		...
		//-B: save source vertex to add a simplex link Lightpath
		Vertex*pVSrc = (Vertex*)pLink->getSrc();
		Vertex*pAVSrc = m_BackupGraph.lookUpVertex(pVSrc->m_pOXCNode->getId(), Vertex::VertexType::VT_Access_Out, -1);
		...
		switch (pLink->m_eSimplexLinkType)
		{
			...
			case SimplexLink::LT_UniFiber:
			{
				...
				//-B: save dst vertex to add a simplex link lightpath
				Vertex*pVDst = (Vertex*)pLink->getDst();
				Vertex*pAVDst = m_BackupGraph.lookUpVertex(pVDst->m_pOXCNode->getId(), Vertex::VertexType::VT_Access_In, -1);
				...
				//-B: ADD NEW SIMPLEX LINK LT_Lightpath TO THE GRAPH
				int sLinkId = this->m_BackupGraph->getNextLinkId();
				SimplexLink *pNewLink = m_BackupGraph.addSimplexLink(sLinkId, pAVSrc, pAVDst, cost, //oppure 1, oppure cost
					length, NULL, SimplexLink::LT_Lightpath, pLightpath->wlAssigned, pLightpath->m_nCapacity);
					//(pLightpath->m_nCapacity - hCircuit.m_eBW)); //DECREASING MOVED TO Unprotected_setUpLightpath
				assert(pNewLink && pNewLink->getSrc() && pNewLink->getDst());
...
void NetMan::DVNF_Dump_SC(ServiceChain *SC)
{
	...
	for (itrSC=SC->SCPath.begin(); itrSC!=SC->SCPath.end(); itrSC++){
				SimplexLink *pLink = (SimplexLink*)(*itrSC);
				if(pLink->m_hFreeCap<=CHANNEL_CAPACITY){
				Vertex*VSrc = (Vertex*)pLink->getSrc();
				Vertex*VDst = (Vertex*)pLink->getDst();
				outfile<<VSrc->m_pOXCNode->getId()<< "->"<<VDst->m_pOXCNode->getId()<<" ("<<pLink->m_nChannel<<")"<<", ";
				
			#ifdef DEBUGLA
				cout<<VSrc->m_pOXCNode->getId()<< "->"<<VDst->m_pOXCNode->getId()<<", ";
...
bool NetMan::DVNF_GroupingVNF(ServiceChain *SC, Connection * pCon)
{
	...
	latency=computeLatency(pCon->m_SC->SCPath,pCon);
	...
	{	
		 pVSrc = m_hGraph.lookUpVertex(SC->SCnodes[i]->getId(),Vertex::VT_Access_Out,-1);
		 pVDst = m_hGraph.lookUpVertex(SC->SCnodes[i+1]->getId(),Vertex::VT_Access_In,-1);
		linkcost= m_hGraph.DijkstraWP(ShortestPath,pVSrc,pVDst,AbstractGraph::LCF_ByOriginalLinkCost);
		pathlatency=computeLatency(ShortestPath,pCon);
	...
	cout<<"\nBuilding the path between Src of the connection and new node for the first VNF"<<endl;
		...
		for (itr=ShortestPath.begin(); itr!=ShortestPath.end(); itr++){
								...
								for(itrSC=pCon->m_SC->SCPath.begin(); itrSC!=pCon->m_SC->SCPath.end(); itrSC++){
									SimplexLink *pLinkSC = (SimplexLink*)(*itrSC);
									if(pLink->getSrc()==pLinkSC->getSrc() && pLink->getDst()==pLinkSC->getDst()&&VDstpath->m_pOXCNode!=VSrcpath->m_pOXCNode){
										linkRedun=true;
										break;
									}
								}
	...
	//LA:if the last scnode is dst nothing should be added otherwise path to dst should be added
	...
	#endif
		pVSrc =m_hGraph.lookUpVertex(SC->SCnodes.back()->getId(),Vertex::VT_Access_Out,-1);
		pVDst =  m_hGraph.lookUpVertex(pCon->m_nDst,Vertex::VT_Access_In, -1);
		linkcost= m_hGraph.DijkstraWP(SC->SCnodes.back()->htoDstPath,pVSrc,pVDst,AbstractGraph::LCF_ByOriginalLinkCost);
		//LA:if the path is not found 
		if(linkcost==UNREACHABLE){
		//pCon->m_bBlockedDueToLatency=true;
		...
		if(SC->SCnodes.back()!=pVDst->m_pOXCNode&& SC->SCnodes.back()->htoDstPath.size()!=0){
			for(itr=SC->SCnodes.back()->htoDstPath.begin();itr!=SC->SCnodes.back()->htoDstPath.end();itr++){
				SimplexLink *pLink = (SimplexLink*)(*itr);
				SC->SCPath.push_back(pLink);
			}
...
//	int SC3nodes []={5,6,8,2,3,4};
	...
	//if(SC->SCtype==MIoT)
	//	NodesID.insert(NodesID.end(),SC1nodes,SC1nodes+7);
	//else
	//	//LA:webservice
	//	if(SC->SCtype==AugmentedReality)
	//		NodesID.insert(NodesID.end(),SC2nodes,SC2nodes+7);
	//	else 
	...
	if(SC->SCtype==MIoT)
		NodesID.insert(NodesID.end(),SC1nodes,SC1nodes+14);
		//NodesID.insert(NodesID.end(),SC1nodes,SC1nodes+5);
	else
		//LA:webservice
		if(SC->SCtype==AugmentedReality)                  
			NodesID.insert(NodesID.end(),SC2nodes,SC2nodes+14);
		else 
			if(SC->SCtype==SmartFactory)
				NodesID.insert(NodesID.end(),SC3nodes,SC3nodes+14);
...
void NetMan::DVNF_EnableVNF(OXCNode* candidatenode, VNF * VN, ServiceChain * SC)
{
	...
	//LA:6- add the path to SCpath :if its not already there
				/*if(SC->SCPath.size()!=0){
					...
					Vertex *VSrcl=(Vertex*)pLast->getSrc();
					Vertex *VDstl=(Vertex*)pLast->getDst();
					PrevSrcID= VSrcl->m_pOXCNode->getId();
					PrevDstID=VDstl->m_pOXCNode->getId(); 	
					
					for (itr=candidatenode->hFromSrcPath.begin(); itr!=candidatenode->hFromSrcPath.end(); itr++){
						SimplexLink *pLinkcan = (SimplexLink*)(*itr);
						Vertex* VSrc = (Vertex*)pLinkcan->getSrc();
						Vertex* VDst = (Vertex*)pLinkcan->getDst();
						//if(find(SC->SCPath.begin(),SC->SCPath.end(),pLinkcan)==SC->SCPath.end()){
						if(PrevSrcID!=VSrc->m_pOXCNode->getId()&&PrevDstID!=VDst->m_pOXCNode->getId())
						{
				...
				if(this->VNFCounter==(SC->LengthOfSCs)-1) 
					SC->SCPath.insert(SC->SCPath.end(),candidatenode->htoDstPath.begin(),candidatenode->htoDstPath.end());
...
int NetMan::DVNF_Counthops(Connection* pCon)
{
	int NumHops= 0;
	SimplexLink* pLink;
	list<AbstractLink*>::const_iterator itr ;
	for(itr=pCon->m_SC->SCPath.begin();itr!=pCon->m_SC->SCPath.end();itr++){
		pLink= (SimplexLink*)(*itr);
	/*	if(pLink->getSimplexLinkType()==SimplexLink::LT_UniFiber ||pLink->getSimplexLinkType()==SimplexLink::LT_Channel)
			NumHops++;
		else */
			if(pLink->getSimplexLinkType()==SimplexLink::LT_Lightpath)
				NumHops+=pLink->m_pLightpath->getPhysicalHops();
			}
	return NumHops;
 }
//LA:to see where is the problem that dijkstra couldn't find a path
char* NetMan::DVNF_BottleNeck(Vertex* Src,Vertex* Dst)
{
	 char* bNeck="Null";
	 ...
	 SimplexLink *pLink ;
	 AbstractNode * BBNeck;
	 if (Src->m_pOXCNode==((Vertex*)BottelNeck)->m_pOXCNode)
		bNeck="Src";
	else
		if(Dst->m_pOXCNode==((Vertex*)BottelNeck)->m_pOXCNode)
			 bNeck="Dst";
		else{
			...
			{
				if(((Vertex*)(*itr)->getDst())->m_pOXCNode==Dst->m_pOXCNode){
					bNeck="Dst";
					Destination=true;
					break;
				}
			...
			{
				if(((Vertex*)(*itr)->getSrc())->m_pOXCNode==((Vertex*)BottelNeck)->m_pOXCNode && !(*itr)->m_bValid){
					bNeck="Dst";
					Destination=true;
					break;
				}
		...
		if(bNeck!="Dst"){
			...
			if(((Vertex*)BeforeBNeck.back())->m_pOXCNode==((Vertex*)BottelNeck)->m_pOXCNode)
				BBNeck=((Vertex*)BeforeBNeck[BeforeBNeck.size()-2])->m_pOXCNode;
			else
				BBNeck=((Vertex*)BeforeBNeck[BeforeBNeck.size()-1])->m_pOXCNode;
			

		pVSrc =m_hGraph.lookUpVertex(BBNeck->getId(),Vertex::VT_Access_Out,-1);
		pVDst = m_hGraph.lookUpVertex(((Vertex*)BottelNeck)->m_pOXCNode->getId(),Vertex::VT_Access_In, -1);
		linkcost= m_hGraph.Dijkstra(path,pVSrc,pVDst,AbstractGraph::LCF_ByOriginalLinkCost);
		for(itr=path.begin();itr!=path.end();itr++)
			{
				pLink = (SimplexLink*)(*itr);
				if(((Vertex*)(*itr)->getSrc())->m_pOXCNode==((Vertex*)BottelNeck)->m_pOXCNode && !pLink->m_eSimplexLinkType==SimplexLink::LT_Lightpath){
					bNeck="Dst";
					Destination=true;
					break;
				}
					...
					if(pLink->m_eSimplexLinkType==SimplexLink::LT_Lightpath){
						...
						for(itrPath=pLink->m_pLightpath->m_hPRoute.begin();itrPath!=pLink->m_pLightpath->m_hPRoute.end();itrPath++)
							if((*itrPath)->m_pSrc==((Vertex*)Dst)->m_pOXCNode){
							{
								bNeck="Dst";
								Destination=true;
								break;
