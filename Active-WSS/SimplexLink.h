...
public:
	typedef enum {
		...
		LT_UniFiber=1,		// a fiber, for link bundling
		LT_Lightpath=2,		// 2
		LT_Channel_Bypass=3,	// 3
		LT_Grooming=4,		// 4
		LT_Mux=5,				// 5
		LT_Demux=6,			// 6
		LT_Tx=7,				// 7
...
public:
	SimplexLink(int, Vertex*, Vertex*, LINK_COST, float, 
		UniFiber*, SimplexLinkType, int, double hCap = double(INFINITE_CAP));
	~SimplexLink();
	...
	void releaseBandwidth(double);
	void consumeBandwidth(double);
	void consumeBandwidthHelper(double);
...
//	UINT			m_nLength;
	...
	//LA:to support voip bandwidth 
	double			m_nBWToBeAllocated;
