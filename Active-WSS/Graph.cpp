...
void Graph::validateSimplexLinkDueToCap(double residualBWD)
{
#ifdef DEBUGB
				...
				UINT nNumberOfPaths, LinkCostFunction hLCF)
{
	...
	this->YenHelper(hKPaths, pSrc, pDst, nNumberOfPaths, hLCF);
}
		...
		itr!=pCurrentPath->m_hLinkList.end(); itr++) {
			...
			for (itrSeg=hFirstSeg.begin(); itrSeg!=hFirstSeg.end(); itrSeg++) {
				if (!((SimplexLink*)(*itrSeg))->auxVirtualLink())
					invalidateVNode(hDirtyNodes, (Vertex*)(*itrSeg)->m_pSrc);
			}
			// no branch
			...
			DijkstraHelper((*itr)->m_pSrc, hLCF);
			// enable nodes/links
							...
							Vertex* pVertex)
{
	...
	itr = m_hVertexMap.find(Vertex::VertexKey(nOXC, Vertex::VT_Channel_In, -1));
	assert(itr != m_hVertexMap.end());
	itr->second->invalidate();
	hNodeList.push_back(itr->second);
	itr = m_hVertexMap.find(Vertex::VertexKey(nOXC, Vertex::VT_Channel_Out, -1));
	assert(itr != m_hVertexMap.end());
	itr->second->invalidate();
	hNodeList.push_back(itr->second);
}
							...
							SimplexLink* pSLink)
{
	...
	Vertex *pLPOutVertex = (Vertex*)pSLink->m_pSrc;
	assert(pLPOutVertex);
...
void Graph::preferWavelPath()
{
	...
	for (itr = this->m_hLinkList.begin(); itr != this->m_hLinkList.end(); itr++)
	{
		...
		switch (pLink->m_eSimplexLinkType)
		{
			...
			case SimplexLink::LT_Converter:
			case SimplexLink::LT_Grooming:
				pLink->modifyCost(0.6); //-B: 0.6 and not 0.5, otherwise it could choose an already existing lightpath (assuming 1-hop lp --> cost = 0.5)
										//	using another wavelength instead of activating a new lp over the same wavelength. This is important because we care about latency!
				break;
			default:
				DEFAULT_SWITCH;
		}
...
//LA:assigns a higher cost to LPs
void Graph::preferWavelChannel()
{
#ifdef DEBUGB
	cout << "-> preferWavelPath" << endl;
#endif // DEBUGB
	...
	for (itr = this->m_hLinkList.begin(); itr != this->m_hLinkList.end(); itr++)
	{
		...
		switch (pLink->m_eSimplexLinkType)
		{
			...
			case SimplexLink::LT_Grooming:
				...
				break;
			case SimplexLink::LT_Lightpath:
				pLink->modifyCost( 1.1*(pLink->getCost()/0.5));
				break;
...
void Graph::releaseSimplexLinkLightpathBwd(double nBWToRelease, Lightpath*pLightpath)
{
#ifdef DEBUGB
	cout << "-> releaseSimplexLinkLightpathBwd" << endl;
#endif // DEBUGB
	
	//LA:for comparing double values
...
#ifdef DEBUGLA
	cout<<"\n The free capacity of simplex link is: "<<pSLink->m_hFreeCap<<" and free capacity of lightpath is: "<<pLightpath->getFreeCapacity();
//	assert((pSLink->m_hFreeCap - pLightpath->getFreeCapacity()<epsilon));
#endif
}
