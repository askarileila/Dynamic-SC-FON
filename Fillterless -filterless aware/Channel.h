...
public:
	Channel();
	Channel(UniFiber*, UINT, double, bool bFree = true, UpDownStatus eUpDown= UP);
	Channel(AbstractLink*, UINT, double, bool bFree = true, UpDownStatus eUpDown= UP);
	...
	void consumeCh(AbstractLink*, int, double); //LA
public:
	// immutable attributes
	UniFiber		*m_pUniFiber;	// the unifiber it belongs to
	UINT			m_nW;			// which wavelength
	double			m_nCapacity;	//LA:changed from UINT to double
	AbstractLink	*m_AbsLink;
	// mutable attributes
