...
WDMNetwork& WDMNetwork::operator=(const WDMNetwork& rhs)
{
	...
	for (itrLink=rhs.m_hLinkList.begin(); itrLink!=rhs.m_hLinkList.end(); itrLink++)
	{
		...
		UniFiber *pNewUniFiber = addUniFiber(pUniFiber->getId(), 
										...
										pUniFiber->m_nW,
										pUniFiber->m_hCost,
										//pUniFiber -> m_linkCap,  //LA:added for capacity
										pUniFiber->m_nLength,pUniFiber->tree);
		int  w;
...
//LA:modified
UniFiber* WDMNetwork::addUniFiberDCTopo(UINT nFiberId, UINT nSrc, UINT nDst, 
								  UINT nW, LINK_COST hCost, UINT nLinkCap, float nLength) 
{
	assert(nW);
	UniFiber *pUniFiber = new UniFiber(nFiberId, (OXCNode*)lookUpNodeById(nSrc),
			(OXCNode*)lookUpNodeById(nDst), hCost, nW, nLinkCap);
...
//LA:tree
UniFiber* WDMNetwork::addUniFiber(UINT nFiberId, UINT nSrc, UINT nDst, 
								  UINT nW, LINK_COST hCost, float nLength,int tree)  
{
	assert(nW);
	UniFiber *pUniFiber = new UniFiber(nFiberId, (OXCNode*)lookUpNodeById(nSrc),
			(OXCNode*)lookUpNodeById(nDst), hCost, nLength, nW,tree);
		...
		for (w = 0; w < pUniFiber->m_nW; w++)
		{
			if (pChannel[w].m_bFree)
			{
				pNewLink = hGraph.addLink(nLinkId++,
					...
					pUniFiber->getCost(), pUniFiber->getLength(),pUniFiber->getCapacity());  //LA:modified
				assert(pNewLink);
				break;
			}
		}
...
//----------t
		...
		if (UNREACHABLE != (*itrLinkM)->m_hCost) {
			AbstractLink *pNewLink = this->addLink(getNextLinkId(), nSrc, nDst, 
				...
				ccost, //----------t
				1,(*itrLinkM)->m_linkCap);		//LA:modified
			assert(pNewLink);
	...
	//Creazione Grafo Ausiliario
      ...
      for (itrCPPath; itrCPPath!=hPPath.m_hLinkList.end(); itrCPPath++) {
            ...
            AGBSP.addLink(new AbstractLink(id++,AGBSP.lookUpNodeById((*itrCPPath)->getDst()->m_nNodeId),
                           AGBSP.lookUpNodeById((*itrCPPath)->getSrc()->m_nNodeId),0,1,(*itrCPPath)->m_linkCap)); //LA:modified
...
*/
           ...
           break; }
			...
			AGBSP.addLink(new AbstractLink(id++, AGBSP.lookUpNodeById(pSrcL->getId()),
				             AGBSP.lookUpNodeById(pDstL->getId()), hCurrBCostL,1,(*itrLp)->m_linkCap));} //LA:modified
      ...
      DEFAULT_SWITCH;
            ...
            // for(ip_=hCurrBPathL.begin();ip_!=hCurrBPathL.end();ip_++){cout<<"{"<<(*ip_)->getSrc()->getId()<<"-"<<(*ip_)->getDst()->getId()<<"}";}cout.precision(12);cout<<"   C:"<<hCurrBCostL<<endl;          
			   if(hCurrBCostL!=UNREACHABLE){
				...
				AGBSP.addLink(new AbstractLink(id++,AGBSP.lookUpNodeById(pSrcL->getId()),
                             AGBSP.lookUpNodeById(pDstLnew->getId()), hCurrBCostL,1));  //LA: modified
		 }			
...
LINK_COST WDMNetwork::BBU_ComputeRoute(Circuit*hCircuit, NetMan *pNetMan, OXCNode* pSrc, OXCNode* pDst, int algorithm_type)
{
	...
	case 0:
	{
		...
		*/
		...
		Lightpath_Seg *pLPSeg1 = new Lightpath_Seg(0);
...
LINK_COST WDMNetwork::wpBBU_ComputeRoute(Circuit*hCircuit, NetMan *pNetMan, OXCNode* pSrc, OXCNode* pDst)
{
	...
	*/
	...
	if (UNREACHABLE != hBestCost)
	{
		...
		Lightpath *pLPSeg = new Lightpath(0);
		//-B: for each link of the pBestP path
		for (itr = pBestP->m_hLinkList.begin(); itr != pBestP->m_hLinkList.end(); itr++)
		{
	...
	for (itrLink = m_hLinkList.begin(); itrLink != m_hLinkList.end(); itrLink++) {
		...
		for (w = 0; w < pUniFiber->m_nW; w++)
			if (pNetMan->checkFreedomSimplexLink(pUniFiber->getSrc()->getId(), pUniFiber->getDst()->getId(), w))
			{
				pNewLink = hGraph.addLink(nLinkId++,
					pUniFiber->getSrc()->getId(), pUniFiber->getDst()->getId(),
					pUniFiber->getCost(), pUniFiber->getLength(), pUniFiber->m_linkCap);
				assert(pNewLink);
				break;
			}
	}
...
void WDMNetwork::printHotelNodes()
{
	...
	for (i = 0; i < this->BBUs.size(); i++)
	{
		cout << "\tHotel node " << BBUs[i]->getId() << ": " << BBUs[i]->m_nBBUs << " BBUs attive" << endl;
	}
	...
	for (itr = this->m_hNodeList.begin(); itr != this->m_hNodeList.end(); itr++)
	{
		...
		if (!pNode->getBBUHotel())
		{
			...
			if (pNode->m_nBBUs > 0)
			{
				cout << "\tHotel node " << pNode->getId() << ": " << pNode->m_nBBUs << " BBUs attive" << endl;
			}
		}
	}
	cout << endl;
...
void WDMNetwork::sortHotelsList(Graph&m_hGraph)
{
	...
	for (i = 0; i < this->hotelsList.size(); i++)
	{
		...
		pVSrc = m_hGraph.lookUpVertex(pNode->getId(), Vertex::VT_Access_Out, -1);
		pVDst = m_hGraph.lookUpVertex(this->DummyNode, Vertex::VT_Access_In, -1);
	...
	{
		pNode = ((OXCNode*)(*itr));
		pVDst = m_hGraph.lookUpVertex(pNode->getId(), Vertex::VT_Access_In, -1);
		...
		for (itrNode = this->m_hNodeList.m_hList.begin(); itrNode != this->m_hNodeList.m_hList.end(); itrNode++)
		{
			...
			{
				pVSrc = m_hGraph.lookUpVertex(pOXC->getId(), Vertex::VT_Access_Out, -1);
			...
			for (itrL = pNetNode->pPath.begin(); itrL != pNetNode->pPath.end(); itrL++)
			{
				...
				switch (pLink->getSimplexLinkType())
				{
					...
					case SimplexLink::LT_UniFiber:
					{
						...
						if(pOXC->m_NFVnode)
						//LA:commented as its replaced with the above condition :if (pOXC->getBBUHotel())
...
//void WDMNetwork::setBetweennessCentrality()
//{
//	vector<OXCNode*>::const_iterator itr;
//	OXCNode*pOXC, *pNetNode;
//	list<AbstractNode*>::const_iterator itrNode;
//	list<AbstractLink*>::const_iterator itrL;
//	SimplexLink *pLink;
//	AbstractNode*pAbstract ,*pVDst, *pVSrc;
//	this->m_nAvgNodesCrossed = 0;
//	UINT nodesCrossed = 0;
//	
//	pVDst=m_hNodeList.find(this->DummyNode);
//
//	
//
//	for (itrNode = m_hNodeList.begin(); itrNode != m_hNodeList.end(); itrNode++)
...
//		}
//		UINT hop = 0;
//		pVSrc=m_hNodeList.find(pNetNode->getId());
//		
//		hop = Dijkstra(pNetNode->pPath, pVSrc, pVDst, AbstractGraph::LCF_ByOriginalLinkCost); //cost not needed
//		nodesCrossed += (hop - 1);
//		//-B: scan all hotel nodes
//		//for (itr = hotelsList.begin(); itr != this->hotelsList.end(); itr++)
//		//{
...
//					case SimplexLink::LT_Grooming:
//					case SimplexLink::LT_Rx:
//						//LA : added to support the lighter version
//						pAbstract = pLink->getSrc();
//						//get soure vertex
//						//pVertex = (Vertex*)(pAbstract);
//						//get source OXC node
...
//	cin.get();
//#endif // DEBUGB*/
//	/*cout<<"\n this is the size of nodelist"<<m_hNodeList.size();
//	cin.get();*/
//}
...
{
	vector<OXCNode*>::const_iterator itr;
	OXCNode*pOXCSrc, *pNetNode,*pNetNode2,*pOXCDst;
	list<AbstractNode*>::const_iterator itrNode,itrNode2;
	list<AbstractLink*>::const_iterator itrL;
	//SimplexLink *pLink;
	AbstractLink *pLink;
	AbstractNode*pAbstract ,*pVDst, *pVSrc;
	this->m_nAvgNodesCrossed = 0;
	UINT nodesCrossed = 0,LinkCount=0;
	
//	pVDst=m_hNodeList.find(this->DummyNode);

	for (itrNode2 = this->m_hNodeList.begin(); itrNode2 != this->m_hNodeList.end(); itrNode2++){
		pNetNode2 = (OXCNode*)(*itrNode2);
		pVDst=m_hNodeList.find(pNetNode2->getId());
	...
	for (itrNode = m_hNodeList.begin(); itrNode != m_hNodeList.end(); itrNode++)
	{
		...
		//{
			...
			{
				LinkCount++;
				////LA:commented since it will change simplex link type in the case that we have more than 8wls
				//pLink = (SimplexLink*)(*itrL);
				pLink = (AbstractLink*)(*itrL);
				//switch (pLink->getSimplexLinkType())
				//{
				//	case SimplexLink::LT_Channel_Bypass:
				//	case SimplexLink::LT_Converter:
				...
				//	case SimplexLink::LT_Rx:
				//		//LA : added to support the lighter version
				//		pOXCSrc=(OXCNode*)pLink->getSrc();
				//		pOXCDst=(OXCNode*)pLink->getDst();
				//		
				//		//if(pOXCSrc->m_NFVnode)
				//		//LA:commented as its replaced with the above condition :if (pOXC->getBBUHotel())
				//			pOXCSrc->m_nBetweennessCentrality++;
				//		//LA:jsut if it is the last node calculate betweeness for the dst
				//			if(LinkCount==pNetNode->pPath.size())
				//			pOXCDst->m_nBetweennessCentrality++;
				//		break;
				//	case SimplexLink::LT_Lightpath:
				//	case SimplexLink::LT_Channel:
				//	case SimplexLink::LT_UniFiber:
	...
	/*for (itrNode = this->m_hNodeList.begin(); itrNode != this->m_hNodeList.end(); itrNode++){
		pNetNode = (OXCNode*)(*itrNode);
		cout<<"\n For node: "<<pNetNode->getId()<<" Betweenness centrality is:"<<pNetNode->m_nBetweennessCentrality<<endl;
	}*/
	this->m_nAvgNodesCrossed = ((double)nodesCrossed / ((double)(this->numberOfNodes - 1)));
	if ((ceil(this->m_nAvgNodesCrossed)) - this->m_nAvgNodesCrossed <= (this->m_nAvgNodesCrossed - floor(this->m_nAvgNodesCrossed)))
		this->m_nAvgNodesCrossed = ceil(this->m_nAvgNodesCrossed);
...
void WDMNetwork::updateTrafficPerNode(Event*pEvent)
{
	...
	if (pEvent->m_hEvent == Event::EVT_ARRIVAL)
	{
		...
		//LA:commented:there is a problem with it
...
void WDMNetwork::extractPotentialSourcesMC_Nconn(bool officeSource, vector<UINT>&potentialSources, double bwdMC,
	double bwdSC)
{
#ifdef DEBUGB
	...
	for (itr = this->m_hNodeList.begin(); itr != this->m_hNodeList.end(); itr++)
	{
		...
		if (officeSource)
		{
			...
			if (pNode->getArea() == OXCNode::Office)
			{
		...
		else
		{
			...
			if (pNode->getArea() == OXCNode::Residential)
			{
				//-B: if this node is a macro cell
				if (pNode->isMacroCell())
				{
...
void WDMNetwork::computeNumOfBBUSupported(UINT offeredTraffic_MC)
{
	...
	for (itr = this->hotelsList.begin(); itr != this->hotelsList.end(); itr++)
	{
		...
		{
			MC_per_wavel = ceil(double(this->m_nChannelCapacity / offeredTraffic_MC));
			if (MC_per_wavel == 0)
			{
...
void WDMNetwork::computeNumOfBBUSupported_CONSERVATIVE(UINT offeredTraffic_MC)
{
	...
	//-B: assuming monofiber link betweeen nodes and assuming each fiber has the same number of wavelenghts
	double fiberCapacity = this->numberOfChannels * this->m_nChannelCapacity;
...
//LA:
/*
LINK_COST DVNF_ShortestPath(list<AbstractLink*>& ShortestPath, UINT Src, UINT Dst, Graph&m_hGraph){
	Vertex*pVSrc, *pVDst;	
	LINK_COST cost=0;
	pVSrc = m_hGraph.lookUpVertex(Src, Vertex::VT_Access_Out, -1);
	pVDst = m_hGraph.lookUpVertex(Dst, Vertex::VT_Access_In, -1);
	cost= m_hGraph.Dijkstra(ShortestPath,pVSrc,pVDst,AbstractGraph::LCF_ByOriginalLinkCost);
#ifdef DEBUGB
	cout<< "\The cost of the shortest path from node"<<Src<<"-to-"<<Dst<<"is"<<cost ;
#endif
