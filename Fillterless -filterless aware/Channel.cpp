...
Channel::Channel(UniFiber *pUniFiber, UINT nW, double nCap, 
...
Channel::Channel(AbstractLink *abs_Link, UINT nW, double nCap, 
				 bool bFree, UpDownStatus eUpDown): m_AbsLink(abs_Link),
		m_nW(nW), m_nCapacity(nCap), m_bFree(bFree), m_eUpDownStatus(eUpDown),
		m_pLightpath(NULL),m_backup(false),isLPsaved(false),canaliProtetti(0)//-t
{
...
void Channel::consumeCh(AbstractLink* SClink, int wl,double bwd)
{
	if(bwd==-1)
		SClink->abs_m_pChannel[wl].m_nCapacity=-1;
	else{
		SClink->abs_m_pChannel[wl].m_nCapacity=SClink->abs_m_pChannel[wl].m_nCapacity-bwd;
			if(SClink->abs_m_pChannel[wl].m_nCapacity<0)
				cin.get();
	}
}
